...
class GlossaryLinkRequest(BaseModel):
    ...
    column_fqns: list[str] = Field(
        ..., description="List of catalog.schema.table.column FQNs to link"
    )
...
class DQRuleCreate(BaseModel):
    ...
    rule_type: str = Field(
        ...,
        description="not_null, unique, range, regex, custom_sql, freshness, row_count, referential",
    )
    expression: str = Field(default="", description="Custom SQL expression or regex pattern")
    params: dict = Field(
        default={}, description="Rule parameters: min, max, pattern, expected_count, etc."
    )
    threshold: float = Field(
        default=0.0, description="Max allowed failure rate (0.0 = zero tolerance)"
    )
...
class CertificationCreate(BaseModel):
    ...
    status: str = Field(
        ..., description="certified, deprecated, draft, pending_review, under_investigation"
    )
...
class DataContractCreate(BaseModel):
    ...
    expected_columns: list[dict] = Field(
        default=[], description="Expected column definitions [{name, type, nullable}]"
    )
...
class DQXCheckCreate(BaseModel):
    ...
    check_function: str = Field(
        ..., description="DQX function: is_not_null, is_in_range, regex_match, etc."
    )
    arguments: dict = Field(
        default={}, description="Function arguments: {column, min_value, max_value, pattern, etc.}"
    )
...
class DQXRunRequest(BaseModel):
    ...
    check_ids: list[str] = Field(
        default=[], description="Specific check IDs to run (empty = all enabled)"
    )
...
class DQXProfileRequest(BaseModel):
    ...
    max_in_count: int = Field(
        default=10, description="Generate is_in_list when distinct values below this count"
    )
    max_null_ratio: float = Field(
        default=0.01, description="Generate is_not_null when null ratio below this (0.0-1.0)"
    )
    remove_outliers: bool = Field(
        default=True, description="Remove statistical outliers from min/max calculations"
    )
    max_parallelism: int = Field(default=4, description="Max tables to profile in parallel (1-16)")
    auto_generate_checks: bool = Field(
        default=True, description="Auto-generate checks from profile results"
    )
...
class ChangeTrackRequest(BaseModel):
    entity_type: str = Field(
        ..., description="glossary, certification, dq_rule, sla_rule, contract, table, column"
    )
