...
def _auth(host: str | None, token: str | None):
    h = (host  or "").rstrip("/")
    t = (token or "")
    if not h or not t:
        raise HTTPException(
...
async def table_lineage(
    ...
    import httpx
    host, token = _auth(x_databricks_host, x_databricks_token)
    ...
    try:
        ...
        return {
            ...
            "upstream_tables":   _extract_all(data.get("upstreams")   or data.get("upstream_tables",   [])),
            "downstream_tables": _extract_all(data.get("downstreams")  or data.get("downstream_tables", [])),
        }
...
async def column_lineage(
    ...
    try:
        ...
        if r.status_code == 404:
            return {"table_name": table_name, "column_name": column_name, "upstream_cols": [], "downstream_cols": []}
        if r.status_code != 200:
            raise HTTPException(status_code=r.status_code, detail=r.text[:500])
        data = r.json()
        return {
        ...
        raise HTTPException(status_code=502, detail=f"Databricks column lineage API unreachable: {exc}")
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as c:
            r = await c.post(
                ...
                headers={"Authorization": f"Bearer {token}"},
                json={"statement": sql, "wait_timeout": "20s", "on_wait_timeout": "CANCEL"},
            )
        if r.status_code != 200:
            raise HTTPException(status_code=r.status_code, detail=r.text[:500])
        data = r.json()
