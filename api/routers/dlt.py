...
async def trigger_pipeline(
    pipeline_id: str, body: DLTTriggerRequest, client=Depends(get_db_client)
):
...
async def clone_pipeline_cross_workspace(
    pipeline_id: str, body: DLTCrossWorkspaceCloneRequest, client=Depends(get_db_client)
):
    ...
    try:
        ...
        if "401" in error_msg or "Unauthorized" in error_msg:
            raise HTTPException(
                401,
                f"Destination workspace authentication failed. Check the PAT token. Detail: {error_msg}",
            )
...
async def get_events(
    pipeline_id: str,
    max_events: int = Query(100, ge=1, le=1000),
    client=Depends(get_db_client),
):
