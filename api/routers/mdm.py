...
async def get_dashboard(client=Depends(get_db_client)):
    ...
    try:
        ...
        return {
            "entities": [],
            "pairs": [],
            "stewardship": {"total": 0, "open": 0, "high_priority": 0, "resolved": 0},
        }
...
async def list_entities(
    entity_type: str = None, status: str = None, limit: int = 100, client=Depends(get_db_client)
):
    config = await get_app_config()
    try:
...
async def update_entity(
    entity_id: str, req: GoldenRecordUpdateRequest, client=Depends(get_db_client)
):
    config = await get_app_config()
...
async def ingest_source_records(req: IngestRequest, client=Depends(get_db_client)):
    config = await get_app_config()
    return _manager(client, config).ingest_source_records(
        req.catalog, req.schema_name, req.table, req.entity_type, req.key_column, req.trust_score
    )
...
async def detect_duplicates(req: DetectDuplicatesRequest, client=Depends(get_db_client)):
    config = await get_app_config()
    return _manager(client, config).detect_duplicates(
        req.entity_type, req.auto_merge_threshold, req.review_threshold
    )


@router.get("/pairs")
async def list_match_pairs(
    entity_type: str = None, status: str = None, limit: int = 100, client=Depends(get_db_client)
):
    config = await get_app_config()
    try:
...
async def create_rule(req: MatchingRuleRequest, client=Depends(get_db_client)):
    config = await get_app_config()
    return _manager(client, config).create_rule(
        req.entity_type, req.name, req.field, req.match_type, req.weight, req.threshold, req.enabled
    )
...
async def list_stewardship(
    status: str = None, priority: str = None, limit: int = 50, client=Depends(get_db_client)
):
    config = await get_app_config()
    try:
...
async def add_hierarchy_node(
    hierarchy_id: str, req: HierarchyNodeRequest, client=Depends(get_db_client)
):
    config = await get_app_config()
    return _manager(client, config).add_node(
        hierarchy_id, req.entity_id, req.label, req.parent_node_id, req.level
    )
