...
async def validate(req: ReconciliationValidateRequest, client=Depends(get_db_client)):
    ...
    _logger.info(
        f"[Reconciliation] source={req.source_catalog} dest={req.destination_catalog} "
...
async def deep_validate(req: DeepReconciliationRequest, client=Depends(get_db_client)):
    ...
    try:
        ...
        adapted = {
            ...
            "mismatched": response.get("missing_in_dest", 0)
            + response.get("extra_in_dest", 0)
            + response.get("modified_rows", 0),
...
async def execute_sql_endpoint(req: dict, client=Depends(get_db_client)):
    ...
    if use_spark:
        try:
            ...
            if spark is None:
                raise HTTPException(
                    status_code=400, detail="Spark session not available. Connect first."
                )
...
async def create_schedule(
    req: dict, client=Depends(get_db_client), app_config=Depends(get_app_config)
):
...
async def delete_schedule(
    schedule_id: str, client=Depends(get_db_client), app_config=Depends(get_app_config)
):
