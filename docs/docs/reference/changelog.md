...
### Fixed
- **`CREATE RECIPIENT IF NOT EXISTS` silently swallowing real errors** — Databricks's `IF NOT EXISTS` returns success even when the create fails for unrelated reasons (cross-region/account constraint, missing entitlement, etc.). Clone-Xs now probes via `SHOW RECIPIENTS LIKE` first; if the recipient doesn't exist, it runs **bare** `CREATE RECIPIENT` (no `IF NOT EXISTS`) so the underlying Databricks error surfaces with full context. If the post-create visibility probe still can't see the recipient, the clone fails immediately with both metastore IDs and a copy-paste diagnostic SQL — no more proceeding to GRANT and emitting the misleading "phantom recipient" message.
