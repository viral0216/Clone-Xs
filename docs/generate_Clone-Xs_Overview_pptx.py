...
def add_textbox(
    slide, left, top, width, height, text="", size=14, bold=False, color=BLACK, align=PP_ALIGN.LEFT
):
...
def add_card_with_text(
    slide, left, top, width, height, title, body, accent=True, badge_text=None, badge_color=CRIMSON
):
    ...
    if badge_text:
        add_rounded_rect(
            slide, left + Inches(0.25), y_offset, Inches(0.8), Inches(0.22), badge_color
        )
        add_textbox(
            slide,
            left + Inches(0.25),
...
def add_phase_card(slide, left, top, width, height, header_text, body_lines):
    ...
    add_rounded_rect(
        slide,
        left,
        top + hdr_h - Inches(0.02),
        width,
        height - hdr_h + Inches(0.02),
        CARD_BG,
        CARD_BORDER,
    )
    ...
    add_textbox(
        slide,
        left + Inches(0.15),
        top + hdr_h + Inches(0.08),
        width - Inches(0.3),
        height - hdr_h - Inches(0.1),
        body,
        size=10,
        color=BODY_TEXT,
    )
...
cards = [
    (
        "Manual SQL Scripts",
        "Teams write custom SQL to clone tables one-by-one. No automation, no error handling, no rollback.",
    ),
    (
        "Missing Metadata",
        "Permissions, tags, properties, ownership, constraints, and comments are silently dropped.",
    ),
    (
        "No Validation",
        "No way to verify clone accuracy — row counts, checksums, and schema parity are unchecked.",
    ),
    (
        "No Audit Trail",
        "Nobody knows who cloned what, when, or whether it succeeded. Compliance gaps everywhere.",
    ),
]
col_w = Inches(5.7)
gap = Inches(0.3)
...
add_separator(s)
add_subtitle(
    s, "Enterprise-grade cloning toolkit — clone, compare, sync, and manage Unity Catalog catalogs"
)

kpis = [
    ("56", "CLI Commands", CRIMSON),
    ("88", "Python Modules", BLACK),
    ("31", "Web UI Pages", INFO),
    ("61+", "API Endpoints", SUCCESS),
]
kpi_w = Inches(2.75)
for i, (val, lbl, clr) in enumerate(kpis):
    add_kpi_box(
        s, Inches(0.7) + i * (kpi_w + Inches(0.2)), Inches(1.7), kpi_w, Inches(1.0), val, lbl, clr
    )
...
sol_cards = [
    (
        "CLI",
        "Command Line",
        "56 commands for cloning, diffing, syncing, rollback, validation, profiling, and more.",
        CRIMSON,
    ),
    (
        "WEB",
        "Web Dashboard",
        "31-page React UI with dark mode, dynamic dropdowns, real-time logs, and guided wizards.",
        INFO,
    ),
    (
        "API",
        "REST API",
        "61+ FastAPI endpoints for automation, CI/CD integration, and programmatic control.",
        SUCCESS,
    ),
]
card_w = Inches(3.75)
for i, (badge, title, body, clr) in enumerate(sol_cards):
    add_card_with_text(
        s,
        Inches(0.7) + i * (card_w + Inches(0.2)),
        Inches(3.0),
        card_w,
        Inches(1.3),
        title,
...
features = [
    (
        "CLONE",
        "Deep & Shallow Clone",
        "Full data copy or metadata-only. Incremental sync and time-travel support.",
        CRIMSON,
    ),
    (
        "METADATA",
        "Full Metadata Copy",
        "Permissions, ownership, tags, properties, security policies, constraints, comments.",
        CRIMSON,
    ),
    (
        "VALIDATE",
        "Post-Clone Validation",
        "Row count + checksum verification with automatic rollback on mismatch.",
        SUCCESS,
    ),
    (
        "SERVERLESS",
        "Serverless Compute",
        "Run clones without a SQL warehouse — uploads wheel, submits notebook job automatically.",
        INFO,
    ),
    (
        "AUDIT",
        "Run Logs to Delta",
        "Every operation persists logs, metrics, and audit trail to Unity Catalog Delta tables.",
        BLACK,
    ),
    (
        "SYNC",
        "Incremental Sync",
        "Sync only changed tables using Delta version history — no full re-clone needed.",
        WARN,
    ),
]
col_w = Inches(5.7)
...
for i, (title, desc, bg) in enumerate(arch_boxes):
    ...
    add_rounded_rect(
        s, x, Inches(1.7), box_w, Inches(1.6), bg, CARD_BORDER if bg == CARD_BG else None
    )
    ...
    add_textbox(
        ...
        desc,
        size=9,
        color=txt_color if bg != CARD_BG else BODY_TEXT,
        align=PP_ALIGN.CENTER,
    )
    if i < 3:
        add_textbox(
...
for i, (title, body) in enumerate(bottom_cards):
    ...
    add_textbox(
        s, x + Inches(0.15), Inches(4.0), Inches(3.5), Inches(0.8), body, size=9, color=BODY_TEXT
    )
...
table_data = [
    ...
    [
        "Operations",
        "Clone, Sync, Incremental Sync, Generate, Rollback, Templates, Schedule, Multi-Clone",
        "8",
    ],
    [
        "Discovery",
        "Explorer, Diff & Compare, Config Diff, Lineage, Dependencies, Impact Analysis, Data Preview",
        "7",
    ],
...
kpis_ui = [
    ("17", "Dynamic Dropdown Pages", CRIMSON),
    ("12", "Clone Templates", BLACK),
    ("4", "Clone Wizard Steps", SUCCESS),
]
for i, (val, lbl, clr) in enumerate(kpis_ui):
    add_kpi_box(
        s, Inches(0.7) + i * Inches(4.1), Inches(5.2), Inches(3.8), Inches(0.9), val, lbl, clr
    )
...
phases = [
    ...
    (
        "3. PREVIEW",
        ["Review all settings", "See object counts", "Dry-run mode", "Preflight checks"],
    ),
    (
        "4. EXECUTE",
        [
            "Real-time log stream",
            "Progress bar per schema",
            "Live status badges",
            "Databricks job link",
        ],
    ),
...
for i, (hdr, items) in enumerate(phases):
    add_phase_card(
        s, Inches(0.7) + i * (phase_w + Inches(0.2)), Inches(1.7), phase_w, Inches(1.8), hdr, items
    )
...
for i, (title, body) in enumerate(steps):
    add_card_with_text(
        s, Inches(0.7), Inches(2.0) + i * Inches(1.1), Inches(5.7), Inches(0.9), title, body
    )
...
add_kpi_box(
    s, Inches(6.8), Inches(2.0), Inches(5.8), Inches(0.8), "$0", "No Warehouse Costs", CRIMSON
)
add_kpi_box(
    s, Inches(6.8), Inches(2.95), Inches(5.8), Inches(0.8), "Auto", "Scales Automatically", BLACK
)
add_kpi_box(
    s, Inches(6.8), Inches(3.9), Inches(5.8), Inches(0.8), "Fast", "No Cluster Start Wait", SUCCESS
)
...
audit_cards = [
    ...
    (
        "logs.clone_operations",
        "Compliance audit trail",
        [
            "Operation ID & type",
            "Who cloned what, when",
...
sec_cards = [
    (
        ...
        "Databricks tokens stored in browser session only. Passed via headers — never persisted on server.",
        CRIMSON,
    ),
    (
        "AUDIT",
        "Sanitized Logging",
        "All tokens and secrets stripped from config before writing to audit Delta tables.",
        BLACK,
    ),
    (
        "RBAC",
        "Role-Based Access Control",
        "Define policies for who can clone which catalogs. Enforceable at API and UI level.",
        INFO,
    ),
    (
        "PII",
        "PII Scanner",
        "Scan catalogs for PII patterns (email, phone, SSN) before cloning to prevent data exposure.",
        SUCCESS,
    ),
]
col_w = Inches(5.7)
...
tech_data = [
    ...
    [
        "Frontend",
        "React 19 + TypeScript",
        "Vite build, TanStack Query, Tailwind CSS 4, shadcn/ui, Recharts",
    ],
    ...
    [
        "Deployment",
        "Docker + Wheel",
        "Docker Compose for full stack, wheel for Databricks notebook/serverless",
    ],
]
add_table(
    s,
...
install_options = [
    (
        ...
        'git clone github.com/viral0216/clone-xs\ncd clone-xs\npip install -e ".[dev]"\nmake web-start',
    ...
    (
        "OPTION 3: CLI ONLY",
        "pip install -e .\nclxs clone \\\n  --source my_catalog \\\n  --dest my_catalog_clone",
        "No server needed for CLI usage",
    ),
]
card_w = Inches(3.75)
...
add_textbox(
    ...
    Inches(4.3),
    Inches(0.4),
    "Open Source — Start Cloning Today",
    size=12,
    bold=True,
    color=WHITE,
    align=PP_ALIGN.CENTER,
