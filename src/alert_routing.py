...
def create_routing_rule(
    name: str, table_pattern: str = "*", severity_filter: str = "*",
    event_type_filter: str = "*", route_to_team: str = "",
    channel: str = "slack", channel_config: dict = None,
...
def list_routing_rules(client=None, warehouse_id: str = "", config: dict = None) -> list[dict]:
    ...
    schema = _get_schema(config)
    try:
        return _query_sql(f"SELECT * FROM {schema}.alert_routing_rules ORDER BY created_at DESC",
                          limit=100, client=client, warehouse_id=warehouse_id) or []
    except Exception:
        return []


def update_routing_rule(rule_id: str, updates: dict, client=None, warehouse_id: str = "", config: dict = None):
    config = config or {}
    schema = _get_schema(config)
    sets = []
    for k, v in updates.items():
        if k in ("name", "table_pattern", "severity_filter", "event_type_filter", "route_to_team", "channel"):
    ...
    if sets:
        try:
            _run_sql(f"UPDATE {schema}.alert_routing_rules SET {', '.join(sets)} WHERE rule_id = '{_esc(rule_id)}'",
                     client, warehouse_id)
        except Exception as e:
...
def delete_routing_rule(rule_id: str, client=None, warehouse_id: str = "", config: dict = None):
    ...
    schema = _get_schema(config)
    try:
        _run_sql(f"DELETE FROM {schema}.alert_routing_rules WHERE rule_id = '{_esc(rule_id)}'", client, warehouse_id)
    except Exception as e:
...
def route_alert(
    event_type: str, table_fqn: str, severity: str, title: str, message: str,
    client=None, warehouse_id: str = "", config: dict = None,
) -> dict:
    ...
    try:
        ...
        if existing:
            ...
            _run_sql(f"UPDATE {schema}.alert_inbox SET occurrence_count = {cnt} WHERE alert_id = '{aid}'",
                     client, warehouse_id)
    ...
    for rule in rules:
        if not rule.get("enabled", True):
            continue
        if (_match_pattern(rule.get("table_pattern", "*"), table_fqn) and
            _match_pattern(rule.get("severity_filter", "*"), severity) and
            _match_pattern(rule.get("event_type_filter", "*"), event_type)):
    ...
    if severity == "critical" and channel:
        try:
            ...
            dispatch_webhook(channel, {"alert_id": aid, "title": title, "message": message,
                                        "severity": severity, "table_fqn": table_fqn})
        except Exception:
            pass
...
def get_inbox(
    ...
    client=None, warehouse_id: str = "", config: dict = None, limit: int = 100,
) -> list[dict]:
    config = config or {}
    schema = _get_schema(config)
    where_parts = []
...
def snooze_alert(alert_id: str, hours: int = 4, client=None, warehouse_id: str = "", config: dict = None):
    config = config or {}
    schema = _get_schema(config)
    try:
        _run_sql(f"""
...
def get_alert_analytics(days: int = 30, client=None, warehouse_id: str = "", config: dict = None) -> dict:
    """Get alert volume, MTTR, and trends."""
    config = config or {}
    schema = _get_schema(config)
    try:
        # Total alerts
        total_rows = _query_sql(f"""
            ...
            GROUP BY severity
        """, limit=10, client=client, warehouse_id=warehouse_id) or []

        total = sum(int(r.get("total", 0)) for r in total_rows)
        by_severity = {r["severity"]: {"count": int(r["total"]), "avg_mttr_minutes": r.get("avg_mttr_minutes")}
                       for r in total_rows}
        ...
        """, limit=days, client=client, warehouse_id=warehouse_id) or []

        return {"total_alerts": total, "period_days": days, "by_severity": by_severity, "daily_trend": trend}
    except Exception as e:
...
def create_digest(recipient: str, frequency: str = "daily", filters: dict = None,
...
def list_digests(client=None, warehouse_id: str = "", config: dict = None) -> list[dict]:
    ...
    schema = _get_schema(config)
    try:
        return _query_sql(f"SELECT * FROM {schema}.alert_digests ORDER BY created_at DESC",
...
def delete_digest(digest_id: str, client=None, warehouse_id: str = "", config: dict = None):
    ...
    schema = _get_schema(config)
    try:
        _run_sql(f"DELETE FROM {schema}.alert_digests WHERE digest_id = '{_esc(digest_id)}'", client, warehouse_id)
    except Exception:
        pass
