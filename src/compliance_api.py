...
def generate_compliance_report_api(
    ...
    if report_type == "full_report":
        ...
        sections = report_runners[report_type](
            client, warehouse_id, config, catalog, from_date, to_date
        )
    else:
        sections = report_runners["data_governance"](
            client, warehouse_id, config, catalog, from_date, to_date
        )
...
def _report_data_governance(client, warehouse_id, config, catalog, from_date, to_date):
    ...
    if audit_cfg and isinstance(audit_cfg, dict):
        ...
        try:
            ...
            rows = execute_sql(
                client,
                warehouse_id,
                f"SELECT * FROM {fqn} {where} ORDER BY started_at DESC LIMIT 100",
            )
            total = len(rows)
            successful = sum(
                1 for r in rows if str(r.get("status", "")).upper() in ("SUCCESS", "COMPLETED")
            )
            failed = total - successful
            score = 100 if total > 0 else 50
            items = [
                {
                    "name": f"{total} clone operations recorded",
                    "status": "COMPLIANT" if total > 0 else "WARNING",
                },
                {
                    "name": f"{successful} successful, {failed} failed",
                    "status": "COMPLIANT" if failed == 0 else "WARNING",
                },
            ]
            sections.append(
                _section(
                    "Audit Trail Coverage",
                    f"Audit trail at {fqn} — {total} operations found.",
                    score,
                    items=items,
                )
            )
            ...
            sections.append(
                _section(
                    "Audit Trail Coverage",
                    f"Could not query audit trail: {e}",
                    30,
        ...
        sections.append(
            _section(
                "Audit Trail Coverage",
                "Audit trail not configured in settings.",
                0,
                items=[
                    {
                        "name": "Configure audit trail in Settings to enable tracking",
                        "status": "NON_COMPLIANT",
                    }
                ],
            )
    ...
    sections.append(
        _section(
            ...
            items=[
                {
                    "name": f"Post-clone validation: {'Enabled' if validate else 'Disabled'}",
                    "status": "COMPLIANT" if validate else "WARNING",
                },
                {
                    "name": f"Checksum validation: {'Enabled' if checksum else 'Disabled'}",
                    "status": "COMPLIANT" if checksum else "WARNING",
                },
            ],
        )
    )
    ...
    items = [
        {
            "name": f"{label}: {'Enabled' if config.get(key, False) else 'Disabled'}",
    ...
    sections.append(
        _section(
            "Clone Configuration Hygiene", "Best-practice clone settings.", hyg_score, items=items
        )
    )
...
def _report_pii_audit(client, warehouse_id, config, catalog, from_date, to_date):
    ...
    try:
        findings = detect_pii_by_column_names(
            client, warehouse_id, catalog, config.get("exclude_schemas", [])
        )
    except Exception as e:
        return [
            _section(
    ...
    table_data = [
        {
            ...
            "masking": f.get("suggested_masking", ""),
        }
    ...
    sections.append(
        _section(
            ...
            items=[
                {
                    "name": f"{pii_count} PII columns detected",
                    "status": "WARNING" if pii_count > 0 else "COMPLIANT",
                }
            ],
    ...
    mask_score = (
        100 if pii_count == 0 else max(0, 100 - int((len(unmasked) / max(pii_count, 1)) * 100))
    )
    sections.append(
        _section(
            "Masking Coverage",
            ...
            items=[
                {
                    "name": f"{len(masking_rules)} masking rules defined",
                    "status": "COMPLIANT" if masking_rules else "WARNING",
                },
                {
                    "name": f"{len(unmasked)} PII columns unmasked",
                    "status": "NON_COMPLIANT" if unmasked else "COMPLIANT",
                },
            ],
        )
    )
...
def _report_permission_audit(client, warehouse_id, config, catalog, from_date, to_date):
    ...
    try:
        ...
        broad_grants = [
            g for g in grants if "ALL" in str(g.get("ActionType", g.get("privilege", ""))).upper()
        ]
    ...
    try:
        ...
        sp_or_group = [
            r
            for r in rows
            if _classify_owner(r.get("table_owner", "")) in ("service_principal", "group")
        ]
        ...
        if needs_attention:
            table_data = [
                {
                    ...
                    "recommendation": _owner_recommendation(
                        _classify_owner(r.get("table_owner", ""))
                    ),
                }
            ...
            sections.append(
                _section(
                    "Table Ownership",
                    f"All {total} tables are properly owned by service principals or groups.",
                    own_score,
                    items=items,
                )
            )
...
def _report_tag_coverage(client, warehouse_id, config, catalog, from_date, to_date):
    ...
    try:
        ...
        sections.append(
            _section(
                "Table Tag Coverage",
                f"{pct}% of tables have at least one tag.",
                score,
                items=items,
                table_data=table_data,
                table_columns=table_columns,
    ...
    try:
        ...
        sections.append(
            _section(
                ...
                items=[
                    {
                        "name": f"{tagged_col_count} tagged columns found",
        ...
        sections.append(
            _section(
                "Column Tag Coverage",
                f"Could not query column tags: {e}",
                30,
                items=[{"name": str(e), "status": "WARNING"}],
            )
        )
...
def _report_ownership_audit(client, warehouse_id, config, catalog, from_date, to_date):
    ...
    try:
        ...
        user_owned = [r for r in rows if _classify_owner(r.get("table_owner", "")) == "user"]
        sp_owned = [
            r for r in rows if _classify_owner(r.get("table_owner", "")) == "service_principal"
        ]
        ...
        sections.append(
            _section(
                ...
                items=[
                    {
                        "name": f"{with_owner}/{total} tables have assigned owners",
                        "status": "COMPLIANT" if pct >= 80 else "WARNING",
                    },
                    {
                        "name": f"{len(no_owner)} tables missing owner",
                        "status": "NON_COMPLIANT" if no_owner else "COMPLIANT",
                    },
                    {
                    ...
                    {"name": f"{len(group_owned)} tables owned by groups", "status": "COMPLIANT"},
                ],
                table_data=table_data,
                table_columns=table_columns,
            )
    ...
    try:
        ...
        sp_or_group_s = [
            s
            for s in schemas
            if _classify_owner(s.get("schema_owner", "")) in ("service_principal", "group")
        ]
