...
FRAMEWORK_CONTROLS = {
    "SOC2": {
        ...
        "controls": [
            ...
            {
                "id": "CC7.2",
                "name": "Monitoring System Components",
                "category": "Operations",
            ...
            },
            {
                ...
                "category": "Operations",
                "evidence_source": "audit_trail",
            },
            {
                "id": "CC8.1",
                "name": "Data Quality Standards",
                "category": "Processing Integrity",
                "evidence_source": "dq_results",
            },
            {
                "id": "A1.2",
    ...
    "GDPR": {
        ...
        "controls": [
            ...
            },
            {
                ...
                "category": "Rights",
                "evidence_source": "rtbf",
            },
            {
                "id": "Art15",
                "name": "Right of Access (DSAR)",
                "category": "Rights",
                "evidence_source": "dsar",
            },
            {
                "id": "Art25",
    ...
    "DORA": {
        ...
        "controls": [
            ...
            {
                "id": "Art8",
                "name": "Identification of ICT Assets",
                "category": "Assets",
                "evidence_source": "audit_trail",
            },
            {
            ...
            {
                "id": "Art10",
                "name": "Detection of Anomalies",
                "category": "Detection",
                "evidence_source": "anomalies",
            },
            {
                "id": "Art11",
...
def _collect_evidence_for_source(source: str, config, client, warehouse_id) -> tuple[int, str]:
    ...
    try:
        if source == "dq_results":
            rows = _query_sql(
                f"SELECT COUNT(*) as cnt FROM {gov_schema}.dq_results WHERE executed_at >= DATEADD(DAY, -90, CURRENT_TIMESTAMP())",
                limit=1,
                client=client,
            ...
            rows = _query_sql(
                f"SELECT COUNT(*) as cnt FROM {get_schema_fqn(config, 'logs')}.clone_operations",
                limit=1,
                client=client,
                warehouse_id=warehouse_id,
            ) or [{"cnt": 0}]
            ...
            rows = _query_sql(
                f"SELECT COUNT(*) as cnt FROM {pii_schema}.pii_scans",
                limit=1,
                client=client,
                warehouse_id=warehouse_id,
            ) or [{"cnt": 0}]
            ...
            rows = _query_sql(
                f"SELECT COUNT(*) as cnt FROM {rtbf_schema}.rtbf_requests",
                limit=1,
                client=client,
                warehouse_id=warehouse_id,
            ) or [{"cnt": 0}]
            ...
            rows = _query_sql(
                f"SELECT COUNT(*) as cnt FROM {dsar_schema}.dsar_requests",
                limit=1,
                client=client,
                warehouse_id=warehouse_id,
            ) or [{"cnt": 0}]
            ...
            rows = _query_sql(
                f"SELECT COUNT(*) as cnt FROM {dq_schema}.monitoring_configs",
                limit=1,
                client=client,
                warehouse_id=warehouse_id,
            ) or [{"cnt": 0}]
            ...
            rows = _query_sql(
                f"SELECT COUNT(*) as cnt FROM {dq_schema}.metric_baselines WHERE is_anomaly = true",
                limit=1,
                client=client,
                warehouse_id=warehouse_id,
            ) or [{"cnt": 0}]
...
def collect_evidence(
    ...
    for control in fw["controls"]:
        count, summary = _collect_evidence_for_source(
            control["evidence_source"], config, client, warehouse_id
        )
        ...
        evidence_list.append(
            {
                ...
                "evidence_type": control["evidence_source"],
...
def get_gaps(
    framework_name: str, client=None, warehouse_id: str = "", config: dict = None
) -> list[dict]:
