...
def compute_coverage(
    ...
    try:
        ...
        """, limit=2000, client=client, warehouse_id=warehouse_id) or []
    except Exception as e:
    ...
    dq_rules = set()
    try:
        rows = _query_sql(f"SELECT DISTINCT table_fqn FROM {gov_schema}.dq_rules WHERE enabled = true",
    ...
    dq_counts = {}
    try:
        rows = _query_sql(f"SELECT table_fqn, COUNT(*) as cnt FROM {gov_schema}.dq_rules WHERE enabled = true GROUP BY table_fqn",
                          limit=5000, client=client, warehouse_id=warehouse_id) or []
    ...
    sla_tables = set()
    try:
        rows = _query_sql(f"SELECT DISTINCT table_fqn FROM {gov_schema}.sla_rules",
                          limit=5000, client=client, warehouse_id=warehouse_id) or []
    ...
    pii_tables = set()
    try:
        rows = _query_sql(f"SELECT DISTINCT table_fqn FROM {pii_schema}.pii_scans",
                          limit=5000, client=client, warehouse_id=warehouse_id) or []
    ...
    mon_tables = set()
    try:
        rows = _query_sql(f"SELECT DISTINCT table_fqn FROM {dq_schema}.monitoring_configs",
                          limit=5000, client=client, warehouse_id=warehouse_id) or []
    ...
    contract_tables = set()
    try:
        rows = _query_sql(f"SELECT DISTINCT table_fqn FROM {gov_schema}.data_contracts",
                          limit=5000, client=client, warehouse_id=warehouse_id) or []
    ...
    for i in range(0, len(value_rows), batch_size):
        ...
        try:
            _run_sql(f"INSERT INTO {gov_schema}.coverage_snapshots VALUES {', '.join(batch)}", client, warehouse_id)
        except Exception as e:
...
def get_coverage_summary(
    ...
    if not coverage:
        return {"total_tables": 0, "avg_coverage": 0, "fully_covered": 0, "no_coverage": 0,
                "with_dq_rules": 0, "with_sla": 0, "with_pii": 0, "with_monitoring": 0}

    total = len(coverage)
    return {
        "total_tables": total,
        "avg_coverage": round(sum(c.get("coverage_pct", 0) for c in coverage) / total, 1) if total else 0,
