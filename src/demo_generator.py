    ...
    "healthcare": {
        "tables": [
            # Large fact tables
            {
                "name": "claims",
                "rows": 100_000_000,
                "ddl_cols": "claim_id BIGINT, patient_id BIGINT, provider_id BIGINT, diagnosis_code STRING, procedure_code STRING, claim_amount DECIMAL(10,2), submitted_date DATE, status STRING, payer_id BIGINT, facility_id BIGINT",
            ...
            },
            {
                "name": "encounters",
                "rows": 50_000_000,
                "ddl_cols": "encounter_id BIGINT, patient_id BIGINT, provider_id BIGINT, facility_id BIGINT, encounter_date DATE, encounter_type STRING, duration_minutes INT, discharge_status STRING, primary_diagnosis STRING, admission_source STRING",
            ...
            },
            {
                "name": "prescriptions",
                "rows": 30_000_000,
                "ddl_cols": "rx_id BIGINT, patient_id BIGINT, provider_id BIGINT, drug_code STRING, drug_name STRING, quantity INT, days_supply INT, fill_date DATE, pharmacy_id BIGINT, refills_remaining INT",
            ...
            # Medium tables
            {
                "name": "lab_results",
                "rows": 5_000_000,
                "ddl_cols": "result_id BIGINT, patient_id BIGINT, encounter_id BIGINT, test_code STRING, test_name STRING, result_value DOUBLE, unit STRING, reference_range STRING, collected_date DATE, status STRING",
            ...
            },
            {
                "name": "vital_signs",
                "rows": 5_000_000,
                "ddl_cols": "vital_id BIGINT, patient_id BIGINT, encounter_id BIGINT, measurement_type STRING, value DOUBLE, unit STRING, recorded_at TIMESTAMP",
            ...
            # Dimension tables
            {
                "name": "patients",
                "rows": 1_000_000,
                "ddl_cols": "patient_id BIGINT, first_name STRING, last_name STRING, date_of_birth DATE, gender STRING, zip_code STRING, insurance_plan_id BIGINT, email STRING, phone STRING, created_at DATE",
            ...
            },
            {
                "name": "providers",
                "rows": 1_000_000,
                "ddl_cols": "provider_id BIGINT, npi STRING, first_name STRING, last_name STRING, specialty_code STRING, facility_id BIGINT, license_state STRING, active BOOLEAN",
            ...
            },
            {
                "name": "facilities",
                "rows": 1_000_000,
                "ddl_cols": "facility_id BIGINT, name STRING, facility_type STRING, address STRING, city STRING, state STRING, zip_code STRING, bed_count INT, npi STRING",
            ...
            },
            {
                ...
                "rows": 1_000_000,
                "ddl_cols": "plan_id BIGINT, carrier_name STRING, plan_type STRING, coverage_level STRING, monthly_premium DECIMAL(8,2), deductible DECIMAL(8,2), effective_date DATE",
            ...
            },
            {
                "name": "drug_catalog",
                "rows": 1_000_000,
                "ddl_cols": "drug_code STRING, drug_name STRING, generic_name STRING, drug_class STRING, manufacturer STRING, schedule STRING, unit_cost DECIMAL(8,2)",
            ...
            # Lookup tables
            {
                ...
                "rows": 500_000,
                "ddl_cols": "code STRING, description STRING, category STRING, icd_version STRING, is_chronic BOOLEAN",
            ...
            },
            {
                ...
                "rows": 500_000,
                "ddl_cols": "code STRING, description STRING, category STRING, cpt_version STRING, avg_duration_minutes INT",
            ...
            },
            {
                "name": "pharmacies",
                "rows": 200_000,
                "ddl_cols": "pharmacy_id BIGINT, name STRING, chain STRING, address STRING, city STRING, state STRING, is_24hr BOOLEAN",
            ...
            },
            {
                "name": "specialties",
                "rows": 100_000,
                "ddl_cols": "specialty_code STRING, description STRING, department STRING, requires_referral BOOLEAN",
            ...
            },
            {
                "name": "claim_lines",
                "rows": 500_000,
                "ddl_cols": "line_id BIGINT, claim_id BIGINT, service_code STRING, quantity INT, line_amount DECIMAL(10,2), modifier STRING",
            ...
            },
            {
                "name": "referrals",
                "rows": 300_000,
                "ddl_cols": "referral_id BIGINT, patient_id BIGINT, from_provider_id BIGINT, to_provider_id BIGINT, reason STRING, referral_date DATE, status STRING",
            ...
            },
            {
                "name": "appointments",
                "rows": 500_000,
                "ddl_cols": "appt_id BIGINT, patient_id BIGINT, provider_id BIGINT, scheduled_date DATE, scheduled_time STRING, status STRING, appt_type STRING, duration_minutes INT",
            ...
            },
            {
                "name": "allergies",
                "rows": 200_000,
                "ddl_cols": "allergy_id BIGINT, patient_id BIGINT, allergen STRING, severity STRING, reaction STRING, onset_date DATE",
            ...
            },
            {
                "name": "immunizations",
                "rows": 200_000,
                "ddl_cols": "imm_id BIGINT, patient_id BIGINT, vaccine_code STRING, vaccine_name STRING, administered_date DATE, lot_number STRING, site STRING",
            ...
            },
            {
                ...
                "rows": 500_000,
                "ddl_cols": "adj_id BIGINT, claim_id BIGINT, adjustment_type STRING, amount DECIMAL(10,2), reason_code STRING, adjusted_date DATE",
        ...
        "views": [
            (
                "v_claim_summary",
                "SELECT p.patient_id, p.first_name, p.last_name, count(*) AS total_claims, sum(c.claim_amount) AS total_amount, avg(c.claim_amount) AS avg_amount, min(c.submitted_date) AS first_claim, max(c.submitted_date) AS last_claim FROM {s}.claims c JOIN {s}.patients p ON c.patient_id = p.patient_id GROUP BY p.patient_id, p.first_name, p.last_name",
            ),
            (
                "v_provider_workload",
                "SELECT pr.provider_id, pr.first_name, pr.last_name, pr.specialty_code, count(DISTINCT e.patient_id) AS unique_patients, count(*) AS total_encounters, avg(e.duration_minutes) AS avg_duration FROM {s}.encounters e JOIN {s}.providers pr ON e.provider_id = pr.provider_id GROUP BY pr.provider_id, pr.first_name, pr.last_name, pr.specialty_code",
            ),
            (
                "v_monthly_claims",
                "SELECT date_trunc('month', submitted_date) AS month, status, count(*) AS claim_count, sum(claim_amount) AS total_amount FROM {s}.claims GROUP BY date_trunc('month', submitted_date), status",
            ),
            (
                "v_high_cost_patients",
                "SELECT patient_id, count(*) AS claim_count, sum(claim_amount) AS total_cost FROM {s}.claims GROUP BY patient_id HAVING sum(claim_amount) > 100000",
            ),
            (
                "v_facility_utilization",
                "SELECT f.facility_id, f.name, f.bed_count, count(*) AS total_encounters, count(DISTINCT e.patient_id) AS unique_patients FROM {s}.facilities f JOIN {s}.encounters e ON f.facility_id = e.facility_id GROUP BY f.facility_id, f.name, f.bed_count",
            ),
            (
                "v_prescription_trends",
                "SELECT date_trunc('month', fill_date) AS month, drug_name, count(*) AS rx_count, avg(quantity) AS avg_quantity FROM {s}.prescriptions GROUP BY date_trunc('month', fill_date), drug_name",
            ),
            (
                "v_lab_abnormal_results",
                "SELECT lr.*, p.first_name, p.last_name FROM {s}.lab_results lr JOIN {s}.patients p ON lr.patient_id = p.patient_id WHERE lr.result_value > 200",
            ),
            (
                "v_encounter_discharge",
                "SELECT encounter_type, discharge_status, count(*) AS cnt, avg(duration_minutes) AS avg_duration FROM {s}.encounters GROUP BY encounter_type, discharge_status",
            ),
            (
                "v_drug_utilization",
                "SELECT dc.drug_class, dc.drug_name, count(*) AS prescriptions, sum(rx.quantity) AS total_units FROM {s}.prescriptions rx JOIN {s}.drug_catalog dc ON rx.drug_code = dc.drug_code GROUP BY dc.drug_class, dc.drug_name",
            ),
            (
                "v_patient_demographics",
                "SELECT gender, floor(datediff(current_date(), date_of_birth)/365) AS age, count(*) AS patient_count FROM {s}.patients GROUP BY gender, floor(datediff(current_date(), date_of_birth)/365)",
            ),
            (
                "v_active_referrals",
                "SELECT r.*, p.first_name, p.last_name FROM {s}.referrals r JOIN {s}.patients p ON r.patient_id = p.patient_id WHERE r.status = 'pending'",
            ),
            (
                "v_appointment_no_shows",
                "SELECT provider_id, count(*) AS no_shows, count(*) * 100.0 / sum(count(*)) OVER (PARTITION BY provider_id) AS no_show_pct FROM {s}.appointments WHERE status = 'no_show' GROUP BY provider_id",
            ),
            (
                "v_allergy_prevalence",
                "SELECT allergen, severity, count(*) AS patient_count FROM {s}.allergies GROUP BY allergen, severity ORDER BY patient_count DESC",
            ),
            (
                "v_immunization_coverage",
                "SELECT vaccine_name, date_trunc('quarter', administered_date) AS quarter, count(DISTINCT patient_id) AS patients_vaccinated FROM {s}.immunizations GROUP BY vaccine_name, date_trunc('quarter', administered_date)",
            ),
            (
                "v_claim_denials",
                "SELECT c.diagnosis_code, c.procedure_code, count(*) AS denial_count, avg(c.claim_amount) AS avg_denied_amount FROM {s}.claims c WHERE c.status = 'denied' GROUP BY c.diagnosis_code, c.procedure_code",
            ),
            (
                "v_pharmacy_volume",
                "SELECT ph.chain, ph.state, count(*) AS rx_filled FROM {s}.prescriptions rx JOIN {s}.pharmacies ph ON rx.pharmacy_id = ph.pharmacy_id GROUP BY ph.chain, ph.state",
            ),
            (
                "v_billing_net",
                "SELECT cl.claim_id, cl.claim_amount, COALESCE(sum(ba.amount),0) AS total_adjustments, cl.claim_amount + COALESCE(sum(ba.amount),0) AS net_amount FROM {s}.claims cl LEFT JOIN {s}.billing_adjustments ba ON cl.claim_id = ba.claim_id GROUP BY cl.claim_id, cl.claim_amount",
            ),
            (
                "v_specialty_demand",
                "SELECT sp.description AS specialty, count(*) AS referral_count FROM {s}.referrals r JOIN {s}.providers p ON r.to_provider_id = p.provider_id JOIN {s}.specialties sp ON p.specialty_code = sp.specialty_code GROUP BY sp.description",
            ),
            (
                "v_vital_signs_latest",
                "SELECT patient_id, measurement_type, value, unit, recorded_at, ROW_NUMBER() OVER (PARTITION BY patient_id, measurement_type ORDER BY recorded_at DESC) AS rn FROM {s}.vital_signs",
            ),
            (
                "v_patient_360",
                "SELECT p.patient_id, p.first_name, p.last_name, p.gender, count(DISTINCT c.claim_id) AS claims, count(DISTINCT e.encounter_id) AS encounters, count(DISTINCT rx.rx_id) AS prescriptions FROM {s}.patients p LEFT JOIN {s}.claims c ON p.patient_id = c.patient_id LEFT JOIN {s}.encounters e ON p.patient_id = e.patient_id LEFT JOIN {s}.prescriptions rx ON p.patient_id = rx.patient_id GROUP BY p.patient_id, p.first_name, p.last_name, p.gender",
            ),
        ],
        "udfs": [
            (
                ...
                "id BIGINT",
                "STRING",
                "Masks patient ID for de-identification",
                "concat('PAT-',substring(sha2(cast(id AS STRING),256),1,12))",
            ),
            (
                "mask_ssn",
                "ssn STRING",
            ...
            (
                ...
                "floor(datediff(current_date(),dob)/365.25)",
            ),
            (
                "format_claim_amount",
                "amount DECIMAL(10,2)",
                "STRING",
                "Formats claim amount as currency",
            ...
            (
                ...
                "length(npi) = 10 AND npi RLIKE '^[0-9]+$'",
            ),
            (
                "risk_score_band",
                "score DOUBLE",
                "STRING",
                "Categorizes risk score into bands",
                "CASE WHEN score >= 80 THEN 'Critical' WHEN score >= 60 THEN 'High' WHEN score >= 40 THEN 'Medium' WHEN score >= 20 THEN 'Low' ELSE 'Minimal' END",
            ),
            (
                "icd_category",
                "code STRING",
                "STRING",
                "Extracts ICD category from code",
                "substring(code,5,2)",
            ),
            (
                "days_since",
            ...
            (
                ...
                "diagnosis STRING",
                "BOOLEAN",
                "Checks if diagnosis code indicates chronic condition",
                "diagnosis RLIKE '^ICD-(E|I|J4|M|N1)'",
            ),
            (
                "encounter_duration_category",
                "minutes INT",
                "STRING",
                "Categorizes encounter duration",
                "CASE WHEN minutes < 30 THEN 'Brief' WHEN minutes < 60 THEN 'Standard' WHEN minutes < 120 THEN 'Extended' ELSE 'Long' END",
            ),
            (
                "format_date_display",
                "dt DATE",
                ...
                "date_format(dt,'MMM dd, yyyy')",
            ),
            (
                "quarter_label",
                "dt DATE",
                "STRING",
                "Returns quarter label like Q1 2024",
            ...
            (
                ...
                "STRING",
                "BMI classification",
                "CASE WHEN bmi < 18.5 THEN 'Underweight' WHEN bmi < 25 THEN 'Normal' WHEN bmi < 30 THEN 'Overweight' ELSE 'Obese' END",
            ),
            (
                "claim_status_label",
                "status STRING",
            ...
            (
                ...
                "schedule STRING",
                "STRING",
                "Risk level from drug schedule",
                "CASE WHEN schedule = 'II' THEN 'High' WHEN schedule IN ('III','IV') THEN 'Medium' WHEN schedule = 'V' THEN 'Low' ELSE 'None' END",
            ),
            (
                "is_pediatric",
                "dob DATE",
                "BOOLEAN",
                "Checks if patient is under 18",
                "datediff(current_date(),dob) < 6570",
            ),
            (
                "insurance_tier",
                "coverage STRING",
                "INT",
                "Numeric tier from coverage level",
                "CASE WHEN coverage = 'platinum' THEN 4 WHEN coverage = 'gold' THEN 3 WHEN coverage = 'silver' THEN 2 ELSE 1 END",
            ),
            (
                "anonymize_name",
                ...
                "concat(left(first_name,1),'.',left(last_name,1),'.',substring(sha2(concat(first_name,last_name),256),1,6))",
            ),
        ],
    },
}
...
INDUSTRIES["financial"] = {
    "tables": [
        {
            "name": "transactions",
            "rows": 100_000_000,
            "ddl_cols": "txn_id BIGINT, account_id BIGINT, txn_type STRING, amount DECIMAL(12,2), currency STRING, txn_date DATE, merchant_id BIGINT, category STRING, channel STRING, status STRING",
        ...
        },
        {
            "name": "card_events",
            "rows": 50_000_000,
            "ddl_cols": "event_id BIGINT, card_id BIGINT, event_type STRING, amount DECIMAL(12,2), merchant_name STRING, mcc_code STRING, event_timestamp TIMESTAMP, country STRING, is_international BOOLEAN, auth_code STRING",
        ...
        },
        {
            "name": "loan_payments",
            "rows": 30_000_000,
            "ddl_cols": "payment_id BIGINT, loan_id BIGINT, payment_date DATE, principal_amount DECIMAL(12,2), interest_amount DECIMAL(12,2), total_amount DECIMAL(12,2), remaining_balance DECIMAL(12,2), payment_method STRING, status STRING, days_late INT",
        ...
        },
        {
            "name": "wire_transfers",
            "rows": 5_000_000,
            "ddl_cols": "wire_id BIGINT, sender_account BIGINT, receiver_account BIGINT, amount DECIMAL(14,2), currency STRING, wire_date DATE, swift_code STRING, status STRING, purpose STRING, fee DECIMAL(8,2)",
        ...
        },
        {
            "name": "trading_orders",
            "rows": 5_000_000,
            "ddl_cols": "order_id BIGINT, account_id BIGINT, symbol STRING, side STRING, quantity INT, price DECIMAL(12,4), order_type STRING, order_date DATE, filled_at TIMESTAMP, status STRING",
        ...
        },
        {
            "name": "accounts",
            "rows": 1_000_000,
            "ddl_cols": "account_id BIGINT, customer_id BIGINT, account_type STRING, balance DECIMAL(14,2), currency STRING, opened_date DATE, status STRING, branch_id BIGINT, interest_rate DECIMAL(5,4), overdraft_limit DECIMAL(10,2)",
        ...
        },
        {
            "name": "customers",
            "rows": 1_000_000,
            "ddl_cols": "customer_id BIGINT, first_name STRING, last_name STRING, email STRING, phone STRING, ssn_hash STRING, date_of_birth DATE, credit_score INT, income_bracket STRING, risk_rating STRING",
        ...
        },
        {
            "name": "branches",
            "rows": 1_000_000,
            "ddl_cols": "branch_id BIGINT, name STRING, address STRING, city STRING, state STRING, zip STRING, manager_id BIGINT, open_date DATE, branch_type STRING",
        ...
        },
        {
            "name": "cards",
            "rows": 1_000_000,
            "ddl_cols": "card_id BIGINT, account_id BIGINT, card_type STRING, card_network STRING, card_number_hash STRING, expiry_date DATE, credit_limit DECIMAL(10,2), issued_date DATE, status STRING",
        ...
        },
        {
            "name": "loans",
            "rows": 1_000_000,
            "ddl_cols": "loan_id BIGINT, customer_id BIGINT, loan_type STRING, principal DECIMAL(14,2), interest_rate DECIMAL(5,4), term_months INT, start_date DATE, status STRING, collateral_type STRING",
        ...
        },
        {
            "name": "fraud_alerts",
            "rows": 500_000,
            "ddl_cols": "alert_id BIGINT, account_id BIGINT, txn_id BIGINT, alert_type STRING, risk_score DOUBLE, alert_date DATE, resolution STRING, investigated_by STRING",
        ...
        },
        {
            "name": "merchants",
            "rows": 500_000,
            "ddl_cols": "merchant_id BIGINT, name STRING, mcc_code STRING, category STRING, city STRING, state STRING, country STRING, risk_level STRING",
        ...
        },
        {
            "name": "interest_rates",
            "rows": 200_000,
            "ddl_cols": "rate_id BIGINT, product_type STRING, term STRING, rate DECIMAL(5,4), effective_date DATE, published_by STRING",
        ...
        },
        {
            ...
            "rows": 300_000,
            "ddl_cols": "event_id BIGINT, account_id BIGINT, event_type STRING, description STRING, reported_date DATE, resolution_date DATE, status STRING",
        ...
        },
        {
            "name": "credit_scores",
            "rows": 300_000,
            "ddl_cols": "score_id BIGINT, customer_id BIGINT, score INT, score_date DATE, bureau STRING, factors STRING",
        ...
        },
        {
            ...
            "rows": 500_000,
            "ddl_cols": "atm_txn_id BIGINT, card_id BIGINT, atm_id STRING, txn_type STRING, amount DECIMAL(10,2), txn_date DATE, surcharge DECIMAL(6,2), success BOOLEAN",
        ...
        },
        {
            ...
            "rows": 200_000,
            "ddl_cols": "statement_id BIGINT, account_id BIGINT, statement_date DATE, opening_balance DECIMAL(14,2), closing_balance DECIMAL(14,2), total_debits DECIMAL(14,2), total_credits DECIMAL(14,2)",
        ...
        },
        {
            "name": "beneficiaries",
            "rows": 200_000,
            "ddl_cols": "beneficiary_id BIGINT, account_id BIGINT, name STRING, relationship STRING, bank_name STRING, account_number_hash STRING, added_date DATE",
        ...
        },
        {
            "name": "exchange_rates",
            "rows": 100_000,
            "ddl_cols": "rate_id BIGINT, from_currency STRING, to_currency STRING, rate DECIMAL(10,6), rate_date DATE, source STRING",
    ...
    "views": [
        (
            "v_account_summary",
            "SELECT a.account_id, a.account_type, a.balance, c.first_name, c.last_name, c.credit_score FROM {s}.accounts a JOIN {s}.customers c ON a.customer_id = c.customer_id",
        ),
        (
            "v_monthly_txn_volume",
            "SELECT date_trunc('month',txn_date) AS month, txn_type, count(*) AS txn_count, sum(amount) AS total_amount FROM {s}.transactions GROUP BY date_trunc('month',txn_date), txn_type",
        ),
        (
            "v_fraud_risk_summary",
            "SELECT f.alert_type, f.resolution, count(*) AS alert_count, avg(f.risk_score) AS avg_score FROM {s}.fraud_alerts f GROUP BY f.alert_type, f.resolution",
        ),
        (
            "v_loan_portfolio",
            "SELECT l.loan_type, l.status, count(*) AS loan_count, sum(l.principal) AS total_principal, avg(l.interest_rate) AS avg_rate FROM {s}.loans l GROUP BY l.loan_type, l.status",
        ),
        (
            "v_card_spending",
            "SELECT ce.country, ce.is_international, count(*) AS events, sum(ce.amount) AS total_spend FROM {s}.card_events ce GROUP BY ce.country, ce.is_international",
        ),
        (
            "v_high_value_customers",
            "SELECT c.customer_id, c.first_name, c.last_name, sum(a.balance) AS total_balance FROM {s}.customers c JOIN {s}.accounts a ON c.customer_id = a.customer_id GROUP BY c.customer_id, c.first_name, c.last_name HAVING sum(a.balance) > 100000",
        ),
        (
            "v_delinquent_loans",
            "SELECT l.*, c.first_name, c.last_name, c.credit_score FROM {s}.loans l JOIN {s}.customers c ON l.customer_id = c.customer_id WHERE l.status IN ('default','delinquent')",
        ),
        (
            "v_branch_performance",
            "SELECT b.name, b.city, b.state, count(DISTINCT a.account_id) AS accounts, sum(a.balance) AS total_deposits FROM {s}.branches b JOIN {s}.accounts a ON b.branch_id = a.branch_id GROUP BY b.name, b.city, b.state",
        ),
        (
            "v_payment_delinquency",
            "SELECT loan_id, count(*) AS late_payments, avg(days_late) AS avg_days_late, max(days_late) AS max_days_late FROM {s}.loan_payments WHERE days_late > 0 GROUP BY loan_id",
        ),
        (
            "v_merchant_risk",
            "SELECT m.name, m.category, m.risk_level, count(f.alert_id) AS fraud_alerts FROM {s}.merchants m LEFT JOIN {s}.fraud_alerts f ON m.merchant_id = f.account_id GROUP BY m.name, m.category, m.risk_level",
        ),
        (
            "v_compliance_dashboard",
            "SELECT event_type, status, count(*) AS event_count, avg(datediff(resolution_date,reported_date)) AS avg_resolution_days FROM {s}.compliance_events GROUP BY event_type, status",
        ),
        (
            "v_credit_score_trend",
            "SELECT cs.customer_id, cs.bureau, cs.score, cs.score_date, LAG(cs.score) OVER (PARTITION BY cs.customer_id, cs.bureau ORDER BY cs.score_date) AS prev_score FROM {s}.credit_scores cs",
        ),
        (
            "v_wire_transfer_volume",
            "SELECT date_trunc('month',wire_date) AS month, currency, count(*) AS transfers, sum(amount) AS total_amount FROM {s}.wire_transfers GROUP BY date_trunc('month',wire_date), currency",
        ),
        (
            "v_atm_usage",
            "SELECT atm_id, count(*) AS txn_count, sum(amount) AS total_withdrawn, avg(surcharge) AS avg_surcharge FROM {s}.atm_transactions GROUP BY atm_id",
        ),
        (
            "v_trading_performance",
            "SELECT symbol, side, count(*) AS orders, sum(quantity * price) AS total_value, avg(price) AS avg_price FROM {s}.trading_orders WHERE status = 'filled' GROUP BY symbol, side",
        ),
        (
            "v_customer_360",
        ...
        (
            "v_daily_balance",
            "SELECT account_id, statement_date, closing_balance, closing_balance - opening_balance AS net_change FROM {s}.account_statements",
        ),
        (
            "v_interest_rate_history",
            "SELECT product_type, effective_date, rate, LAG(rate) OVER (PARTITION BY product_type ORDER BY effective_date) AS prev_rate FROM {s}.interest_rates",
        ),
        (
            "v_beneficiary_network",
            "SELECT b.account_id, b.name, b.bank_name, a.account_type, a.balance FROM {s}.beneficiaries b JOIN {s}.accounts a ON b.account_id = a.account_id",
        ),
        (
            "v_exchange_rate_spread",
            "SELECT from_currency, to_currency, rate_date, rate, avg(rate) OVER (PARTITION BY from_currency, to_currency ORDER BY rate_date ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) AS moving_avg FROM {s}.exchange_rates",
        ),
    ],
    "udfs": [
        (
        ...
        (
            "format_currency",
            "amount DECIMAL(14,2), currency STRING",
            "STRING",
            "Formats amount with currency symbol",
            "CASE WHEN currency = 'USD' THEN concat('$',format_number(amount,2)) WHEN currency = 'EUR' THEN concat('€',format_number(amount,2)) WHEN currency = 'GBP' THEN concat('£',format_number(amount,2)) ELSE concat(currency,' ',format_number(amount,2)) END",
        ),
        (
            "credit_score_band",
            "score INT",
            ...
            "CASE WHEN score >= 800 THEN 'Exceptional' WHEN score >= 740 THEN 'Very Good' WHEN score >= 670 THEN 'Good' WHEN score >= 580 THEN 'Fair' ELSE 'Poor' END",
        ),
        (
            "calculate_apr",
            "rate DECIMAL(5,4), compounds_per_year INT",
            "DECIMAL(5,4)",
            "Converts nominal rate to APR",
            "round(power(1 + rate/compounds_per_year, compounds_per_year) - 1, 4)",
        ),
        (
            "is_high_risk_txn",
            "amount DECIMAL(12,2), is_international BOOLEAN",
            "BOOLEAN",
            "Flags high-risk transactions",
            "amount > 5000 OR is_international",
        ),
        (
            "txn_category_label",
            "cat STRING",
            "STRING",
            "Human-readable transaction category",
            "initcap(replace(cat,'_',' '))",
        ),
        (
            "loan_status_severity",
            "status STRING",
            "INT",
            "Numeric severity for loan status",
            "CASE WHEN status = 'default' THEN 5 WHEN status = 'delinquent' THEN 4 WHEN status = 'restructured' THEN 3 WHEN status = 'active' THEN 2 ELSE 1 END",
        ),
        (
            "days_to_maturity",
            "start_date DATE, term_months INT",
            "INT",
            "Calculates days until loan maturity",
            "datediff(add_months(start_date, term_months), current_date())",
        ),
        (
            "is_valid_swift",
            "code STRING",
            "BOOLEAN",
            "Validates SWIFT code format",
            "length(code) IN (8,11) AND code RLIKE '^[A-Z]{4}[A-Z]{2}[A-Z0-9]{2}([A-Z0-9]{3})?$'",
        ),
        (
            "risk_weighted_amount",
            "amount DECIMAL(14,2), risk_level STRING",
            "DECIMAL(14,2)",
            "Risk-weighted asset calculation",
            "amount * CASE WHEN risk_level = 'high' THEN 1.5 WHEN risk_level = 'medium' THEN 1.0 ELSE 0.5 END",
        ),
        (
            "format_iban",
            "iban STRING",
            ...
            "concat(substring(iban,1,4),' ',substring(iban,5,4),' ',substring(iban,9,4),' ',substring(iban,13))",
        ),
        (
            "card_expiry_status",
            "expiry DATE",
            "STRING",
            "Checks card expiry status",
            "CASE WHEN expiry < current_date() THEN 'Expired' WHEN expiry < date_add(current_date(),90) THEN 'Expiring Soon' ELSE 'Valid' END",
        ),
        (
            "monthly_payment",
            "principal DECIMAL(14,2), annual_rate DECIMAL(5,4), term_months INT",
            "DECIMAL(10,2)",
            "Calculates monthly loan payment",
            "round(principal * (annual_rate/12) / (1 - power(1 + annual_rate/12, -term_months)), 2)",
        ),
        (
            "fraud_risk_label",
            "score DOUBLE",
            "STRING",
            "Labels fraud risk score",
            "CASE WHEN score >= 80 THEN 'Critical' WHEN score >= 60 THEN 'High' WHEN score >= 40 THEN 'Medium' WHEN score >= 20 THEN 'Low' ELSE 'Minimal' END",
        ),
        (
            "anonymize_customer",
            "first_name STRING, last_name STRING, customer_id BIGINT",
            "STRING",
            "Creates anonymized customer ref",
            "concat('CUST-',substring(sha2(concat(first_name,last_name,cast(customer_id as STRING)),256),1,10))",
        ),
        (
            "is_business_hours",
            "ts TIMESTAMP",
            "BOOLEAN",
            "Checks if timestamp is during business hours",
            "hour(ts) BETWEEN 9 AND 17 AND dayofweek(ts) BETWEEN 2 AND 6",
        ),
        (
            "currency_convert",
            "amount DECIMAL(14,2), rate DECIMAL(10,6)",
            "DECIMAL(14,2)",
        ...
        (
            ...
            "event_type STRING",
            "INT",
            "Priority level for compliance events",
            "CASE WHEN event_type IN ('SAR','OFAC_hit') THEN 1 WHEN event_type IN ('AML_alert','PEP_match') THEN 2 WHEN event_type = 'CTR' THEN 3 ELSE 4 END",
        ),
    ],
}
...
INDUSTRIES["retail"] = {
    "tables": [
        {
            "name": "order_items",
            "rows": 100_000_000,
            "ddl_cols": "item_id BIGINT, order_id BIGINT, product_id BIGINT, quantity INT, unit_price DECIMAL(10,2), discount DECIMAL(5,2), total DECIMAL(10,2), shipped_date DATE, status STRING, warehouse_id BIGINT",
        ...
        },
        {
            "name": "clickstream",
            "rows": 50_000_000,
            "ddl_cols": "event_id BIGINT, session_id STRING, customer_id BIGINT, page_url STRING, event_type STRING, product_id BIGINT, event_timestamp TIMESTAMP, device_type STRING, referrer STRING, duration_seconds INT",
        ...
        },
        {
            "name": "reviews",
            "rows": 30_000_000,
            "ddl_cols": "review_id BIGINT, product_id BIGINT, customer_id BIGINT, rating INT, title STRING, review_date DATE, verified_purchase BOOLEAN, helpful_votes INT, status STRING",
        ...
        },
        {
            "name": "orders",
            "rows": 5_000_000,
            "ddl_cols": "order_id BIGINT, customer_id BIGINT, order_date DATE, total_amount DECIMAL(10,2), shipping_cost DECIMAL(8,2), tax DECIMAL(8,2), payment_method STRING, shipping_method STRING, status STRING, promo_code STRING",
        ...
        },
        {
            "name": "cart_events",
            "rows": 5_000_000,
            "ddl_cols": "event_id BIGINT, session_id STRING, customer_id BIGINT, product_id BIGINT, action STRING, quantity INT, event_timestamp TIMESTAMP",
        ...
        },
        {
            "name": "customers",
            "rows": 1_000_000,
            "ddl_cols": "customer_id BIGINT, first_name STRING, last_name STRING, email STRING, phone STRING, address STRING, city STRING, state STRING, zip STRING, loyalty_tier STRING, created_date DATE",
        ...
        },
        {
            "name": "products",
            "rows": 1_000_000,
            "ddl_cols": "product_id BIGINT, name STRING, category STRING, subcategory STRING, brand STRING, price DECIMAL(10,2), cost DECIMAL(10,2), weight_kg DECIMAL(6,2), is_active BOOLEAN, created_date DATE",
        ...
        },
        {
            "name": "stores",
            "rows": 1_000_000,
            "ddl_cols": "store_id BIGINT, name STRING, store_type STRING, address STRING, city STRING, state STRING, zip STRING, manager STRING, sqft INT, opened_date DATE",
        ...
        },
        {
            "name": "inventory",
            "rows": 1_000_000,
            "ddl_cols": "inventory_id BIGINT, product_id BIGINT, warehouse_id BIGINT, quantity_on_hand INT, quantity_reserved INT, reorder_point INT, last_received DATE, last_counted DATE",
        ...
        },
        {
            "name": "categories",
            "rows": 1_000_000,
            "ddl_cols": "category_id BIGINT, name STRING, parent_category STRING, description STRING, is_active BOOLEAN, display_order INT",
        ...
        },
        {
            "name": "promotions",
            "rows": 500_000,
            "ddl_cols": "promo_id BIGINT, code STRING, discount_type STRING, discount_value DECIMAL(8,2), start_date DATE, end_date DATE, min_purchase DECIMAL(8,2), max_uses INT, used_count INT",
        ...
        },
        {
            "name": "warehouses",
            "rows": 500_000,
            "ddl_cols": "warehouse_id BIGINT, name STRING, address STRING, city STRING, state STRING, capacity_sqft INT, current_utilization DECIMAL(5,2), manager STRING",
        ...
        },
        {
            "name": "suppliers",
            "rows": 200_000,
            "ddl_cols": "supplier_id BIGINT, name STRING, contact_email STRING, country STRING, lead_time_days INT, rating DECIMAL(3,1), category STRING",
        ...
        },
        {
            "name": "shipping",
            "rows": 300_000,
            "ddl_cols": "shipment_id BIGINT, order_id BIGINT, carrier STRING, tracking_number STRING, shipped_date DATE, delivered_date DATE, status STRING, cost DECIMAL(8,2)",
        ...
        },
        {
            "name": "returns",
            "rows": 200_000,
            "ddl_cols": "return_id BIGINT, order_id BIGINT, product_id BIGINT, reason STRING, return_date DATE, refund_amount DECIMAL(10,2), condition_received STRING, status STRING",
        ...
        },
        {
            "name": "wishlists",
            "rows": 300_000,
            "ddl_cols": "wishlist_id BIGINT, customer_id BIGINT, product_id BIGINT, added_date DATE, priority STRING, is_public BOOLEAN",
        ...
        },
        {
            "name": "loyalty_points",
            "rows": 200_000,
            "ddl_cols": "txn_id BIGINT, customer_id BIGINT, points INT, txn_type STRING, txn_date DATE, order_id BIGINT, expiry_date DATE",
        ...
        },
        {
            "name": "price_history",
            "rows": 500_000,
            "ddl_cols": "history_id BIGINT, product_id BIGINT, old_price DECIMAL(10,2), new_price DECIMAL(10,2), change_date DATE, reason STRING",
        ...
        },
        {
            "name": "gift_cards",
            "rows": 100_000,
            "ddl_cols": "card_id BIGINT, card_number STRING, original_balance DECIMAL(8,2), current_balance DECIMAL(8,2), purchased_date DATE, expiry_date DATE, status STRING",
    ...
    "views": [
        (
            "v_order_summary",
            "SELECT o.order_id, o.customer_id, o.order_date, o.total_amount, count(oi.item_id) AS items FROM {s}.orders o JOIN {s}.order_items oi ON o.order_id = oi.order_id GROUP BY o.order_id, o.customer_id, o.order_date, o.total_amount",
        ),
        (
            "v_product_performance",
            "SELECT p.product_id, p.name, p.category, count(oi.item_id) AS units_sold, sum(oi.total) AS revenue, avg(r.rating) AS avg_rating FROM {s}.products p LEFT JOIN {s}.order_items oi ON p.product_id = oi.product_id LEFT JOIN {s}.reviews r ON p.product_id = r.product_id GROUP BY p.product_id, p.name, p.category",
        ),
        (
            "v_customer_ltv",
            "SELECT c.customer_id, c.first_name, c.last_name, c.loyalty_tier, count(DISTINCT o.order_id) AS orders, sum(o.total_amount) AS lifetime_value FROM {s}.customers c JOIN {s}.orders o ON c.customer_id = o.customer_id GROUP BY c.customer_id, c.first_name, c.last_name, c.loyalty_tier",
        ),
        (
            "v_monthly_revenue",
            "SELECT date_trunc('month',order_date) AS month, count(*) AS orders, sum(total_amount) AS revenue, avg(total_amount) AS aov FROM {s}.orders GROUP BY date_trunc('month',order_date)",
        ),
        (
            "v_inventory_alerts",
            "SELECT i.*, p.name, p.category FROM {s}.inventory i JOIN {s}.products p ON i.product_id = p.product_id WHERE i.quantity_on_hand < i.reorder_point",
        ),
        (
            "v_top_reviewers",
            "SELECT customer_id, count(*) AS review_count, avg(rating) AS avg_rating FROM {s}.reviews WHERE verified_purchase = true GROUP BY customer_id HAVING count(*) > 5",
        ),
        (
            "v_conversion_funnel",
            "SELECT event_type, count(*) AS events FROM {s}.clickstream GROUP BY event_type",
        ),
        (
            "v_return_rate",
            "SELECT p.category, count(DISTINCT ret.return_id) AS returns, count(DISTINCT oi.item_id) AS sold, round(count(DISTINCT ret.return_id)*100.0/count(DISTINCT oi.item_id),2) AS return_pct FROM {s}.products p JOIN {s}.order_items oi ON p.product_id = oi.product_id LEFT JOIN {s}.returns ret ON oi.order_id = ret.order_id AND oi.product_id = ret.product_id GROUP BY p.category",
        ),
        (
            "v_shipping_performance",
            "SELECT carrier, avg(datediff(delivered_date,shipped_date)) AS avg_delivery_days, count(*) AS shipments FROM {s}.shipping WHERE status = 'delivered' GROUP BY carrier",
        ),
        (
            "v_promo_effectiveness",
            "SELECT pr.code, pr.discount_type, pr.used_count, count(o.order_id) AS orders_with_promo FROM {s}.promotions pr LEFT JOIN {s}.orders o ON o.promo_code = pr.code GROUP BY pr.code, pr.discount_type, pr.used_count",
        ),
        ("v_store_performance", "SELECT s.name, s.city, s.store_type, s.sqft FROM {s}.stores s"),
        (
            "v_category_trends",
            "SELECT p.category, date_trunc('month',oi.shipped_date) AS month, sum(oi.total) AS revenue FROM {s}.order_items oi JOIN {s}.products p ON oi.product_id = p.product_id GROUP BY p.category, date_trunc('month',oi.shipped_date)",
        ),
        (
            "v_supplier_lead_times",
            "SELECT s.name, s.country, s.lead_time_days, s.rating FROM {s}.suppliers s ORDER BY s.lead_time_days",
        ),
        (
            "v_warehouse_utilization",
            "SELECT w.name, w.city, w.capacity_sqft, w.current_utilization, count(i.inventory_id) AS products_stored FROM {s}.warehouses w LEFT JOIN {s}.inventory i ON w.warehouse_id = i.warehouse_id GROUP BY w.name, w.city, w.capacity_sqft, w.current_utilization",
        ),
        (
            "v_loyalty_balance",
            "SELECT customer_id, sum(CASE WHEN txn_type IN ('earned','bonus') THEN points ELSE 0 END) AS earned, sum(CASE WHEN txn_type = 'redeemed' THEN abs(points) ELSE 0 END) AS redeemed FROM {s}.loyalty_points GROUP BY customer_id",
        ),
        (
            "v_price_volatility",
            "SELECT product_id, count(*) AS changes, avg(abs(new_price-old_price)) AS avg_change, max(abs(new_price-old_price)) AS max_change FROM {s}.price_history GROUP BY product_id",
        ),
        (
            "v_gift_card_liability",
            "SELECT status, count(*) AS cards, sum(current_balance) AS total_liability FROM {s}.gift_cards GROUP BY status",
        ),
        (
            "v_wishlist_demand",
            "SELECT product_id, count(*) AS wishlist_count FROM {s}.wishlists GROUP BY product_id ORDER BY wishlist_count DESC",
        ),
        (
            "v_device_analytics",
            "SELECT device_type, count(*) AS events, count(DISTINCT session_id) AS sessions FROM {s}.clickstream GROUP BY device_type",
        ),
        (
            "v_customer_segments",
            "SELECT loyalty_tier, count(*) AS customers, avg(datediff(current_date(),created_date)) AS avg_tenure_days FROM {s}.customers GROUP BY loyalty_tier",
        ),
    ],
    "udfs": [
        (
        ...
        (
            ...
            "original DECIMAL(10,2), discounted DECIMAL(10,2)",
            "STRING",
            "Calculates discount percentage",
            "concat(cast(round((1 - discounted/original)*100,0) as STRING),'% off')",
        ),
        (
            "loyalty_tier_level",
            "tier STRING",
            "INT",
            "Numeric level for loyalty tier",
            "CASE WHEN tier = 'diamond' THEN 5 WHEN tier = 'platinum' THEN 4 WHEN tier = 'gold' THEN 3 WHEN tier = 'silver' THEN 2 ELSE 1 END",
        ),
        (
            "shipping_eta",
            "method STRING",
            ...
            "CASE WHEN method = 'same_day' THEN 'Today' WHEN method = 'next_day' THEN '1 business day' WHEN method = 'express' THEN '2-3 business days' WHEN method = 'standard' THEN '5-7 business days' ELSE '7-14 business days' END",
        ),
        (
            "product_margin",
            "price DECIMAL(10,2), cost DECIMAL(10,2)",
            "DECIMAL(5,2)",
            "Calculates product margin percentage",
        ...
        (
            ...
            "order_date DATE",
            "BOOLEAN",
            "Checks if within 30-day return window",
            "datediff(current_date(), order_date) <= 30",
        ),
        (
            "cart_abandonment_risk",
            "events INT, duration INT",
            "STRING",
            "Risk of cart abandonment",
            "CASE WHEN events < 3 AND duration > 300 THEN 'High' WHEN events < 5 THEN 'Medium' ELSE 'Low' END",
        ),
        (
            "format_tracking",
            "carrier STRING, tracking STRING",
            "STRING",
            "Formats tracking display",
            "concat(carrier, ': ', tracking)",
        ),
        (
            "inventory_status",
            "on_hand INT, reorder_point INT",
            "STRING",
            "Inventory status label",
            "CASE WHEN on_hand = 0 THEN 'Out of Stock' WHEN on_hand < reorder_point THEN 'Low Stock' WHEN on_hand < reorder_point * 3 THEN 'Adequate' ELSE 'Overstocked' END",
        ),
        (
            "promo_discount_display",
            "discount_type STRING, value DECIMAL(8,2)",
            "STRING",
            "Display promo discount",
            "CASE WHEN discount_type = 'percentage' THEN concat(cast(value as STRING),'% off') WHEN discount_type = 'fixed' THEN concat('$',format_number(value,2),' off') WHEN discount_type = 'bogo' THEN 'Buy One Get One' ELSE 'Free Shipping' END",
        ),
        (
            "gift_card_status_label",
            "status STRING, balance DECIMAL(8,2)",
            "STRING",
            "Gift card status display",
            "CASE WHEN status = 'expired' THEN 'Expired' WHEN balance = 0 THEN 'Used' ELSE concat('$',format_number(balance,2),' remaining') END",
        ),
        (
            "category_icon",
            "category STRING",
            ...
            "CASE WHEN category = 'Electronics' THEN '📱' WHEN category = 'Clothing' THEN '👕' WHEN category = 'Home' THEN '🏠' WHEN category = 'Sports' THEN '⚽' WHEN category = 'Books' THEN '📚' ELSE '🛍️' END",
        ),
        (
            "review_sentiment",
            "rating INT",
        ...
        (
            "anonymize_shopper",
            "first_name STRING, customer_id BIGINT",
            "STRING",
            "Anonymized shopper ID",
            "concat('SHOP-',substring(sha2(concat(first_name,cast(customer_id as STRING)),256),1,8))",
        ),
    ],
}
...
INDUSTRIES["telecom"] = {
    "tables": [
        {
            "name": "cdr_records",
            "rows": 100_000_000,
            "ddl_cols": "cdr_id BIGINT, subscriber_id BIGINT, call_type STRING, called_number STRING, duration_seconds INT, start_time TIMESTAMP, end_time TIMESTAMP, cell_tower_id BIGINT, cost DECIMAL(8,4), status STRING",
        ...
        },
        {
            "name": "data_usage",
            "rows": 50_000_000,
            "ddl_cols": "usage_id BIGINT, subscriber_id BIGINT, session_start TIMESTAMP, session_end TIMESTAMP, bytes_uploaded BIGINT, bytes_downloaded BIGINT, app_category STRING, network_type STRING, cell_tower_id BIGINT, throttled BOOLEAN",
        ...
        },
        {
            "name": "billing",
            "rows": 30_000_000,
            "ddl_cols": "bill_id BIGINT, subscriber_id BIGINT, billing_period DATE, base_charge DECIMAL(8,2), data_charge DECIMAL(8,2), overage_charge DECIMAL(8,2), taxes DECIMAL(8,2), total_amount DECIMAL(8,2), payment_status STRING, due_date DATE",
        ...
        },
        {
            "name": "network_events",
            "rows": 5_000_000,
            "ddl_cols": "event_id BIGINT, tower_id BIGINT, event_type STRING, severity STRING, start_time TIMESTAMP, end_time TIMESTAMP, affected_subscribers INT, root_cause STRING",
        ...
        },
        {
            ...
            "rows": 5_000_000,
            "ddl_cols": "ticket_id BIGINT, subscriber_id BIGINT, category STRING, priority STRING, created_date DATE, resolved_date DATE, status STRING, channel STRING, satisfaction_score INT",
        ...
        },
        {
            "name": "subscribers",
            "rows": 1_000_000,
            "ddl_cols": "subscriber_id BIGINT, first_name STRING, last_name STRING, email STRING, phone STRING, plan_id BIGINT, activation_date DATE, status STRING, credit_class STRING, contract_end DATE",
        ...
        },
        {
            "name": "plans",
            "rows": 1_000_000,
            "ddl_cols": "plan_id BIGINT, name STRING, plan_type STRING, monthly_cost DECIMAL(8,2), data_limit_gb INT, voice_minutes INT, sms_limit INT, is_unlimited BOOLEAN, is_active BOOLEAN",
        ...
        },
        {
            "name": "towers",
            "rows": 1_000_000,
            "ddl_cols": "tower_id BIGINT, tower_name STRING, latitude DOUBLE, longitude DOUBLE, tower_type STRING, technology STRING, capacity INT, city STRING, state STRING, status STRING",
        ...
        },
        {
            "name": "devices",
            "rows": 1_000_000,
            "ddl_cols": "device_id BIGINT, subscriber_id BIGINT, imei STRING, make STRING, model STRING, os STRING, os_version STRING, activation_date DATE, is_5g_capable BOOLEAN",
        ...
        },
        {
            "name": "roaming_usage",
            "rows": 1_000_000,
            "ddl_cols": "roaming_id BIGINT, subscriber_id BIGINT, country STRING, roaming_type STRING, data_mb DECIMAL(10,2), voice_minutes INT, cost DECIMAL(8,2), usage_date DATE",
        ...
        },
        {
            ...
            "rows": 500_000,
            "ddl_cols": "port_id BIGINT, phone_number STRING, from_carrier STRING, to_carrier STRING, request_date DATE, completion_date DATE, status STRING",
        ...
        {
            "name": "promotions_telecom",
            "rows": 500_000,
            "ddl_cols": "promo_id BIGINT, name STRING, promo_type STRING, discount_amount DECIMAL(8,2), start_date DATE, end_date DATE, eligibility STRING, redemptions INT",
        ...
        },
        {
            "name": "coverage_areas",
            "rows": 200_000,
            "ddl_cols": "area_id BIGINT, zip_code STRING, city STRING, state STRING, coverage_type STRING, signal_strength INT, technology STRING, last_tested DATE",
        ...
        {
            "name": "tower_maintenance",
            "rows": 300_000,
            "ddl_cols": "maintenance_id BIGINT, tower_id BIGINT, maintenance_type STRING, scheduled_date DATE, completed_date DATE, cost DECIMAL(10,2), technician STRING, status STRING",
        ...
        },
        {
            "name": "sim_cards",
            "rows": 200_000,
            "ddl_cols": "sim_id BIGINT, iccid STRING, subscriber_id BIGINT, sim_type STRING, activation_date DATE, status STRING, puk_hash STRING",
        ...
        },
        {
            ...
            "rows": 200_000,
            "ddl_cols": "prediction_id BIGINT, subscriber_id BIGINT, churn_probability DOUBLE, risk_factors STRING, predicted_date DATE, model_version STRING",
        ...
        },
        {
            ...
            "rows": 300_000,
            "ddl_cols": "service_id BIGINT, subscriber_id BIGINT, service_name STRING, monthly_fee DECIMAL(6,2), activated_date DATE, status STRING",
        ...
        {
            "name": "spectrum_allocation",
            "rows": 100_000,
            "ddl_cols": "allocation_id BIGINT, band STRING, frequency_mhz INT, bandwidth_mhz INT, region STRING, license_expiry DATE, technology STRING",
    ...
    "views": [
        (
            "v_subscriber_usage",
            "SELECT s.subscriber_id, s.first_name, s.last_name, count(DISTINCT c.cdr_id) AS calls, sum(c.duration_seconds) AS total_duration, sum(d.bytes_downloaded) AS total_download FROM {s}.subscribers s LEFT JOIN {s}.cdr_records c ON s.subscriber_id = c.subscriber_id LEFT JOIN {s}.data_usage d ON s.subscriber_id = d.subscriber_id GROUP BY s.subscriber_id, s.first_name, s.last_name",
        ),
        (
            "v_monthly_revenue_telecom",
            "SELECT date_trunc('month',billing_period) AS month, count(*) AS bills, sum(total_amount) AS revenue, avg(total_amount) AS arpu FROM {s}.billing GROUP BY date_trunc('month',billing_period)",
        ),
        (
            "v_network_health",
            "SELECT t.tower_id, t.tower_name, t.technology, count(ne.event_id) AS incidents, sum(ne.affected_subscribers) AS total_affected FROM {s}.towers t LEFT JOIN {s}.network_events ne ON t.tower_id = ne.tower_id GROUP BY t.tower_id, t.tower_name, t.technology",
        ),
        (
            "v_churn_risk",
            "SELECT s.subscriber_id, s.first_name, s.last_name, s.plan_id, cp.churn_probability, cp.risk_factors FROM {s}.subscribers s JOIN {s}.churn_predictions cp ON s.subscriber_id = cp.subscriber_id WHERE cp.churn_probability > 0.7",
        ),
        (
            "v_data_consumption",
            "SELECT app_category, network_type, count(*) AS sessions, sum(bytes_downloaded)/1073741824 AS total_gb FROM {s}.data_usage GROUP BY app_category, network_type",
        ),
        (
            "v_billing_overdue",
            "SELECT b.*, s.first_name, s.last_name, s.email FROM {s}.billing b JOIN {s}.subscribers s ON b.subscriber_id = s.subscriber_id WHERE b.payment_status = 'overdue'",
        ),
        (
            "v_tower_load",
            "SELECT tower_id, date_trunc('hour',start_time) AS hour, count(*) AS calls, sum(duration_seconds) AS total_seconds FROM {s}.cdr_records GROUP BY tower_id, date_trunc('hour',start_time)",
        ),
        (
            "v_device_distribution",
            "SELECT make, model, os, count(*) AS devices, sum(CASE WHEN is_5g_capable THEN 1 ELSE 0 END) AS five_g_capable FROM {s}.devices GROUP BY make, model, os",
        ),
        (
            "v_roaming_revenue",
            "SELECT country, roaming_type, count(*) AS events, sum(cost) AS revenue FROM {s}.roaming_usage GROUP BY country, roaming_type",
        ),
        (
            "v_plan_popularity",
            "SELECT p.name, p.plan_type, p.monthly_cost, count(s.subscriber_id) AS subscribers FROM {s}.plans p LEFT JOIN {s}.subscribers s ON p.plan_id = s.plan_id GROUP BY p.name, p.plan_type, p.monthly_cost",
        ),
        (
            "v_ticket_resolution",
            "SELECT category, priority, avg(datediff(resolved_date,created_date)) AS avg_resolution_days, avg(satisfaction_score) AS avg_satisfaction FROM {s}.service_tickets WHERE status IN ('resolved','closed') GROUP BY category, priority",
        ),
        (
            "v_port_in_out",
            "SELECT to_carrier, from_carrier, count(*) AS ports, status FROM {s}.number_portability GROUP BY to_carrier, from_carrier, status",
        ),
        (
            "v_coverage_gaps",
            "SELECT city, state, technology, avg(signal_strength) AS avg_signal FROM {s}.coverage_areas WHERE signal_strength <= 2 GROUP BY city, state, technology",
        ),
        (
            "v_maintenance_costs",
            "SELECT date_trunc('month',scheduled_date) AS month, maintenance_type, count(*) AS jobs, sum(cost) AS total_cost FROM {s}.tower_maintenance GROUP BY date_trunc('month',scheduled_date), maintenance_type",
        ),
        (
            "v_vas_revenue",
            "SELECT service_name, count(*) AS subscribers, sum(monthly_fee) AS monthly_revenue FROM {s}.value_added_services WHERE status = 'active' GROUP BY service_name",
        ),
        (
            "v_sim_inventory",
            "SELECT sim_type, status, count(*) AS sims FROM {s}.sim_cards GROUP BY sim_type, status",
        ),
        (
            "v_call_quality",
            "SELECT status, count(*) AS calls, avg(duration_seconds) AS avg_duration FROM {s}.cdr_records GROUP BY status",
        ),
        (
            "v_spectrum_utilization",
            "SELECT band, technology, sum(bandwidth_mhz) AS total_bandwidth FROM {s}.spectrum_allocation GROUP BY band, technology",
        ),
        (
            "v_promo_performance",
            "SELECT name, promo_type, discount_amount, redemptions FROM {s}.promotions_telecom ORDER BY redemptions DESC",
        ),
        (
            "v_subscriber_360",
            "SELECT s.subscriber_id, s.first_name, s.last_name, s.status, p.name AS plan, d.make AS device_make, d.model AS device_model FROM {s}.subscribers s LEFT JOIN {s}.plans p ON s.plan_id = p.plan_id LEFT JOIN {s}.devices d ON s.subscriber_id = d.subscriber_id",
        ),
    ],
    "udfs": [
        (
        ...
        (
            ...
            "CASE WHEN bytes >= 1073741824 THEN concat(round(bytes/1073741824,2),' GB') WHEN bytes >= 1048576 THEN concat(round(bytes/1048576,2),' MB') WHEN bytes >= 1024 THEN concat(round(bytes/1024,2),' KB') ELSE concat(bytes,' B') END",
        ),
        (
            "format_duration",
            "seconds INT",
            ...
            "concat(lpad(cast(floor(seconds/3600) as STRING),2,'0'),':',lpad(cast(floor((seconds%3600)/60) as STRING),2,'0'),':',lpad(cast(seconds%60 as STRING),2,'0'))",
        ),
        (
            "call_cost_estimate",
            "duration_seconds INT, rate_per_min DECIMAL(6,4)",
            "DECIMAL(8,4)",
            "Estimates call cost",
            "round(duration_seconds / 60.0 * rate_per_min, 4)",
        ),
        (
            "churn_risk_label",
            "probability DOUBLE",
            "STRING",
            "Labels churn risk",
            "CASE WHEN probability >= 0.8 THEN 'Critical' WHEN probability >= 0.6 THEN 'High' WHEN probability >= 0.4 THEN 'Medium' WHEN probability >= 0.2 THEN 'Low' ELSE 'Minimal' END",
        ),
        (
            "network_generation",
            "tech STRING",
            "INT",
            "Returns network generation number",
            "CASE WHEN tech LIKE '5G%' THEN 5 WHEN tech LIKE '4G%' THEN 4 WHEN tech LIKE '3G%' THEN 3 ELSE 2 END",
        ),
        (
            "signal_quality",
            "strength INT",
            ...
            "CASE WHEN strength >= 4 THEN 'Excellent' WHEN strength >= 3 THEN 'Good' WHEN strength >= 2 THEN 'Fair' ELSE 'Poor' END",
        ),
        (
            "is_roaming",
            "country STRING",
            "BOOLEAN",
            "Checks if country is roaming",
            "country != 'US'",
        ),
        (
            "billing_status_priority",
            "status STRING",
            "INT",
            "Priority for billing status",
            "CASE WHEN status = 'overdue' THEN 1 WHEN status = 'partial' THEN 2 WHEN status = 'pending' THEN 3 WHEN status = 'paid' THEN 4 ELSE 5 END",
        ),
        (
            ...
            "months_between(current_date(), activation_date)",
        ),
        (
            "data_plan_usage_pct",
            "used_gb DOUBLE, limit_gb INT",
            "DECIMAL(5,2)",
            "Plan usage percentage",
            "CASE WHEN limit_gb = -1 THEN 0 ELSE round(used_gb / limit_gb * 100, 2) END",
        ),
        (
            "device_age_months",
            "activation_date DATE",
        ...
        (
            ...
            "datediff(end_date, current_date()) BETWEEN 0 AND 60",
        ),
        (
            "tower_distance_km",
            ...
            "round(6371 * acos(cos(radians(lat1)) * cos(radians(lat2)) * cos(radians(lon2) - radians(lon1)) + sin(radians(lat1)) * sin(radians(lat2))), 2)",
        ),
        (
            "anonymize_subscriber",
            "first_name STRING, subscriber_id BIGINT",
            "STRING",
            "Anonymized subscriber ref",
            "concat('SUB-',substring(sha2(concat(first_name,cast(subscriber_id as STRING)),256),1,8))",
        ),
        (
            ...
            "concat('(',substring(phone,3,3),') ',substring(phone,6,3),'-',substring(phone,9,4))",
        ),
        (
            "ticket_sla_status",
            "created DATE, resolved DATE, priority STRING",
            "STRING",
            "SLA compliance check",
            "CASE WHEN resolved IS NULL THEN 'Open' WHEN datediff(resolved,created) <= CASE WHEN priority='critical' THEN 1 WHEN priority='high' THEN 3 WHEN priority='medium' THEN 5 ELSE 7 END THEN 'Met' ELSE 'Breached' END",
        ),
        (
            "spectrum_band_category",
            "frequency_mhz INT",
            "STRING",
            "Categorizes spectrum band",
            "CASE WHEN frequency_mhz < 1000 THEN 'Low Band' WHEN frequency_mhz < 6000 THEN 'Mid Band' ELSE 'High Band (mmWave)' END",
        ),
        (
            "monthly_arpu",
            "total DECIMAL(8,2), months INT",
            "DECIMAL(8,2)",
            "Average revenue per user per month",
            "round(total / GREATEST(months, 1), 2)",
        ),
    ],
}
...
INDUSTRIES["manufacturing"] = {
    "tables": [
        {
            ...
            "rows": 100_000_000,
            "ddl_cols": "reading_id BIGINT, equipment_id BIGINT, sensor_type STRING, value DOUBLE, unit STRING, reading_timestamp TIMESTAMP, quality_flag STRING, line_id BIGINT, shift STRING, alert_level STRING",
        ...
        },
        {
            ...
            "rows": 50_000_000,
            "ddl_cols": "event_id BIGINT, order_id BIGINT, product_code STRING, line_id BIGINT, event_type STRING, quantity INT, event_timestamp TIMESTAMP, operator_id BIGINT, batch_id STRING, cycle_time_seconds DOUBLE",
        ...
        },
        {
            "name": "quality_checks",
            "rows": 30_000_000,
            "ddl_cols": "check_id BIGINT, batch_id STRING, product_code STRING, check_type STRING, result STRING, measurement DOUBLE, spec_min DOUBLE, spec_max DOUBLE, inspector_id BIGINT, check_timestamp TIMESTAMP",
        ...
        },
        {
            "name": "work_orders",
            "rows": 5_000_000,
            "ddl_cols": "order_id BIGINT, product_code STRING, quantity_ordered INT, quantity_produced INT, quantity_scrapped INT, start_date DATE, due_date DATE, completion_date DATE, status STRING, priority STRING",
        ...
        },
        {
            ...
            "rows": 5_000_000,
            "ddl_cols": "downtime_id BIGINT, equipment_id BIGINT, line_id BIGINT, reason STRING, start_time TIMESTAMP, end_time TIMESTAMP, duration_minutes INT, category STRING, planned BOOLEAN, cost_impact DECIMAL(10,2)",
        ...
        },
        {
            "name": "equipment",
            "rows": 1_000_000,
            "ddl_cols": "equipment_id BIGINT, name STRING, equipment_type STRING, manufacturer STRING, model STRING, serial_number STRING, install_date DATE, line_id BIGINT, status STRING, maintenance_interval_days INT",
        ...
        },
        {
            "name": "materials",
            "rows": 1_000_000,
            "ddl_cols": "material_id BIGINT, name STRING, material_type STRING, unit STRING, unit_cost DECIMAL(10,4), supplier_id BIGINT, lead_time_days INT, min_order_qty INT, safety_stock INT",
        ...
        },
        {
            "name": "suppliers_mfg",
            "rows": 1_000_000,
            "ddl_cols": "supplier_id BIGINT, name STRING, country STRING, certification STRING, quality_rating DECIMAL(3,1), on_time_delivery_pct DECIMAL(5,2), contact_email STRING, lead_time_avg_days INT",
        ...
        },
        {
            ...
            "rows": 1_000_000,
            "ddl_cols": "line_id BIGINT, name STRING, plant STRING, capacity_per_hour INT, product_family STRING, shift_pattern STRING, status STRING, oee_target DECIMAL(5,2)",
        ...
        },
        {
            "name": "products_mfg",
            "rows": 1_000_000,
            "ddl_cols": "product_code STRING, name STRING, category STRING, weight_kg DECIMAL(8,3), dimensions STRING, bom_cost DECIMAL(10,2), selling_price DECIMAL(10,2), active BOOLEAN",
        ...
        },
        {
            ...
            "rows": 500_000,
            "ddl_cols": "log_id BIGINT, equipment_id BIGINT, maintenance_type STRING, description STRING, start_time TIMESTAMP, end_time TIMESTAMP, cost DECIMAL(10,2), technician STRING, parts_used STRING",
        ...
        },
        {
            "name": "scrap_records",
            "rows": 500_000,
            "ddl_cols": "scrap_id BIGINT, batch_id STRING, product_code STRING, quantity INT, reason STRING, scrap_date DATE, cost DECIMAL(10,2), disposition STRING",
        ...
        },
        {
            ...
            "rows": 300_000,
            "ddl_cols": "record_id BIGINT, line_id BIGINT, meter_type STRING, value DOUBLE, unit STRING, reading_date DATE, shift STRING, cost DECIMAL(8,2)",
        ...
        },
        {
            ...
            "rows": 200_000,
            "ddl_cols": "incident_id BIGINT, location STRING, incident_type STRING, severity STRING, incident_date DATE, description STRING, lost_time_hours DECIMAL(6,1), osha_recordable BOOLEAN",
        ...
        },
        {
            "name": "inventory_mfg",
            "rows": 300_000,
            "ddl_cols": "inventory_id BIGINT, material_id BIGINT, warehouse STRING, quantity DECIMAL(12,2), unit STRING, last_counted DATE, min_level DECIMAL(12,2), max_level DECIMAL(12,2)",
        ...
        },
        {
            ...
            "rows": 200_000,
            "ddl_cols": "po_id BIGINT, supplier_id BIGINT, material_id BIGINT, quantity DECIMAL(12,2), unit_price DECIMAL(10,4), total DECIMAL(14,2), order_date DATE, delivery_date DATE, status STRING",
        ...
        },
        {
            "name": "bom",
            "rows": 200_000,
            "ddl_cols": "bom_id BIGINT, product_code STRING, material_id BIGINT, quantity_per_unit DECIMAL(8,4), unit STRING, level INT, is_critical BOOLEAN",
        ...
        },
        {
            ...
            "rows": 100_000,
            "ddl_cols": "schedule_id BIGINT, operator_id BIGINT, line_id BIGINT, shift_date DATE, shift STRING, start_time STRING, end_time STRING, status STRING",
        ...
        },
        {
            ...
            "rows": 100_000,
            "ddl_cols": "metric_id BIGINT, plant STRING, metric_type STRING, value DOUBLE, unit STRING, measurement_date DATE, compliant BOOLEAN",
    ...
    "views": [
        (
            "v_oee_by_line",
            "SELECT pl.line_id, pl.name, count(DISTINCT pe.order_id) AS orders, sum(pe.quantity) AS total_produced, avg(pe.cycle_time_seconds) AS avg_cycle_time FROM {s}.production_lines pl JOIN {s}.production_events pe ON pl.line_id = pe.line_id WHERE pe.event_type = 'complete' GROUP BY pl.line_id, pl.name",
        ),
        (
            "v_quality_summary",
            "SELECT product_code, check_type, count(*) AS checks, sum(CASE WHEN result='pass' THEN 1 ELSE 0 END) AS passed, round(sum(CASE WHEN result='pass' THEN 1 ELSE 0 END)*100.0/count(*),2) AS pass_rate FROM {s}.quality_checks GROUP BY product_code, check_type",
        ),
        (
            "v_equipment_health",
            "SELECT e.equipment_id, e.name, e.equipment_type, e.status, count(dt.downtime_id) AS downtime_events, sum(dt.duration_minutes) AS total_downtime_min FROM {s}.equipment e LEFT JOIN {s}.downtime_events dt ON e.equipment_id = dt.equipment_id GROUP BY e.equipment_id, e.name, e.equipment_type, e.status",
        ),
        (
            "v_sensor_alerts",
            "SELECT sr.equipment_id, sr.sensor_type, sr.alert_level, count(*) AS alerts, avg(sr.value) AS avg_value FROM {s}.sensor_readings sr WHERE sr.alert_level IN ('high','critical') GROUP BY sr.equipment_id, sr.sensor_type, sr.alert_level",
        ),
        (
            "v_work_order_status",
            "SELECT status, priority, count(*) AS orders, sum(quantity_ordered) AS total_ordered, sum(quantity_produced) AS total_produced, sum(quantity_scrapped) AS total_scrapped FROM {s}.work_orders GROUP BY status, priority",
        ),
        (
            "v_scrap_analysis",
            "SELECT reason, count(*) AS incidents, sum(quantity) AS total_scrapped, sum(cost) AS total_cost FROM {s}.scrap_records GROUP BY reason ORDER BY total_cost DESC",
        ),
        (
        ...
        (
            ...
            "SELECT s.name, s.country, s.quality_rating, s.on_time_delivery_pct, count(po.po_id) AS total_orders FROM {s}.suppliers_mfg s LEFT JOIN {s}.purchase_orders po ON s.supplier_id = po.supplier_id GROUP BY s.name, s.country, s.quality_rating, s.on_time_delivery_pct",
        ),
        (
            "v_energy_by_line",
            "SELECT line_id, meter_type, date_trunc('month',reading_date) AS month, sum(value) AS total_consumption, sum(cost) AS total_cost FROM {s}.energy_consumption GROUP BY line_id, meter_type, date_trunc('month',reading_date)",
        ),
        (
            "v_safety_dashboard",
            "SELECT date_trunc('month',incident_date) AS month, incident_type, severity, count(*) AS incidents, sum(lost_time_hours) AS total_lost_hours FROM {s}.safety_incidents GROUP BY date_trunc('month',incident_date), incident_type, severity",
        ),
        (
            "v_production_daily",
            "SELECT cast(event_timestamp as DATE) AS production_date, line_id, count(*) AS events, sum(CASE WHEN event_type='complete' THEN quantity ELSE 0 END) AS completed FROM {s}.production_events GROUP BY cast(event_timestamp as DATE), line_id",
        ),
        (
            "v_material_usage",
            "SELECT m.name, m.material_type, sum(b.quantity_per_unit * wo.quantity_produced) AS total_used FROM {s}.materials m JOIN {s}.bom b ON m.material_id = b.material_id JOIN {s}.work_orders wo ON b.product_code = wo.product_code GROUP BY m.name, m.material_type",
        ),
        (
            "v_inventory_status",
            "SELECT i.warehouse, count(*) AS items, sum(CASE WHEN i.quantity < i.min_level THEN 1 ELSE 0 END) AS below_min, sum(CASE WHEN i.quantity > i.max_level THEN 1 ELSE 0 END) AS above_max FROM {s}.inventory_mfg i GROUP BY i.warehouse",
        ),
        (
            "v_downtime_pareto",
            "SELECT reason, count(*) AS occurrences, sum(duration_minutes) AS total_minutes, sum(cost_impact) AS total_cost FROM {s}.downtime_events WHERE planned = false GROUP BY reason ORDER BY total_minutes DESC",
        ),
        (
            "v_product_profitability",
            "SELECT p.product_code, p.name, p.selling_price, p.bom_cost, round((p.selling_price - p.bom_cost)/p.selling_price*100,2) AS margin_pct FROM {s}.products_mfg p WHERE p.active = true",
        ),
        (
            "v_po_delivery",
            "SELECT s.name AS supplier, count(po.po_id) AS orders, avg(datediff(po.delivery_date,po.order_date)) AS avg_lead_days FROM {s}.purchase_orders po JOIN {s}.suppliers_mfg s ON po.supplier_id = s.supplier_id GROUP BY s.name",
        ),
        (
            "v_shift_productivity",
            "SELECT ss.shift, ss.line_id, count(DISTINCT ss.operator_id) AS operators, count(DISTINCT pe.event_id) AS events FROM {s}.shift_schedules ss LEFT JOIN {s}.production_events pe ON ss.line_id = pe.line_id AND ss.shift = pe.shift GROUP BY ss.shift, ss.line_id",
        ),
        (
            "v_environmental_compliance",
            "SELECT plant, metric_type, count(*) AS measurements, sum(CASE WHEN compliant THEN 1 ELSE 0 END) AS compliant_count, round(sum(CASE WHEN compliant THEN 1 ELSE 0 END)*100.0/count(*),2) AS compliance_pct FROM {s}.environmental_metrics GROUP BY plant, metric_type",
        ),
        (
            "v_critical_bom_items",
            "SELECT b.product_code, m.name AS material, m.material_type, b.quantity_per_unit, m.unit_cost, s.on_time_delivery_pct FROM {s}.bom b JOIN {s}.materials m ON b.material_id = m.material_id JOIN {s}.suppliers_mfg s ON m.supplier_id = s.supplier_id WHERE b.is_critical = true",
        ),
        (
            "v_equipment_lifecycle",
            "SELECT equipment_type, avg(datediff(current_date(),install_date)/365) AS avg_age_years, count(*) AS total, sum(CASE WHEN status='decommissioned' THEN 1 ELSE 0 END) AS decommissioned FROM {s}.equipment GROUP BY equipment_type",
        ),
    ],
    "udfs": [
        (
            "oee_calculate",
            "availability DOUBLE, performance DOUBLE, quality DOUBLE",
            "DOUBLE",
        ...
        (
            "sensor_status",
            "value DOUBLE, min_val DOUBLE, max_val DOUBLE",
            "STRING",
            "Sensor reading status",
            "CASE WHEN value < min_val OR value > max_val THEN 'Out of Spec' WHEN value < min_val*1.1 OR value > max_val*0.9 THEN 'Warning' ELSE 'Normal' END",
        ),
        (
            "cycle_time_efficiency",
            "actual DOUBLE, target DOUBLE",
            "DECIMAL(5,2)",
            "Cycle time efficiency percentage",
            "round(target / actual * 100, 2)",
        ),
        (
            "scrap_rate",
            "scrapped INT, produced INT",
            "DECIMAL(5,2)",
            "Scrap rate percentage",
            "round(scrapped * 100.0 / GREATEST(produced, 1), 2)",
        ),
        (
            "yield_rate",
            "good INT, total INT",
            "DECIMAL(5,2)",
            "First pass yield",
            "round(good * 100.0 / GREATEST(total, 1), 2)",
        ),
        (
            "maintenance_urgency",
            "days_since INT, interval_days INT",
            "STRING",
            "Maintenance urgency level",
            "CASE WHEN days_since > interval_days * 1.5 THEN 'Overdue - Critical' WHEN days_since > interval_days THEN 'Overdue' WHEN days_since > interval_days * 0.9 THEN 'Due Soon' ELSE 'OK' END",
        ),
        (
            "downtime_cost_per_hour",
            "cost DECIMAL(10,2), minutes INT",
            "DECIMAL(10,2)",
            "Cost per hour of downtime",
            "round(cost / GREATEST(minutes, 1) * 60, 2)",
        ),
        (
            "format_duration_mfg",
            "minutes INT",
            ...
            "concat(cast(floor(minutes/60) as STRING),'h ',cast(minutes%60 as STRING),'m')",
        ),
        (
            "safety_severity_score",
            "severity STRING, lost_hours DECIMAL(6,1)",
            "INT",
            "Numeric safety severity score",
            "CASE WHEN severity = 'critical' THEN 100 WHEN severity = 'serious' THEN 75 WHEN severity = 'moderate' THEN 50 ELSE 25 END + cast(lost_hours as INT)",
        ),
        (
            "inventory_health",
            "quantity DECIMAL(12,2), min_level DECIMAL(12,2), max_level DECIMAL(12,2)",
            "STRING",
            "Inventory health status",
            "CASE WHEN quantity < min_level THEN 'Critical Low' WHEN quantity < min_level * 1.5 THEN 'Low' WHEN quantity > max_level THEN 'Overstocked' ELSE 'Healthy' END",
        ),
        (
            "energy_cost_per_unit",
            "energy_cost DECIMAL(8,2), units_produced INT",
            "DECIMAL(8,4)",
            "Energy cost per unit produced",
            "round(energy_cost / GREATEST(units_produced, 1), 4)",
        ),
        (
            "supplier_grade",
            "quality DECIMAL(3,1), delivery DECIMAL(5,2)",
            "STRING",
            "Supplier grade from metrics",
            "CASE WHEN quality >= 4.5 AND delivery >= 95 THEN 'A' WHEN quality >= 3.5 AND delivery >= 85 THEN 'B' WHEN quality >= 2.5 AND delivery >= 75 THEN 'C' ELSE 'D' END",
        ),
        (
            "batch_traceability",
            "batch_id STRING",
            "STRING",
            "Extracts trace info from batch ID",
            "concat('Batch: ',batch_id,' | Generated: ',current_date())",
        ),
        (
            "is_overdue_po",
            "delivery_date DATE",
            "BOOLEAN",
            "Checks if PO is overdue",
        ...
        (
            ...
            "CASE WHEN shift = 'morning' THEN '1st Shift (6AM-2PM)' WHEN shift = 'afternoon' THEN '2nd Shift (2PM-10PM)' ELSE '3rd Shift (10PM-6AM)' END",
        ),
        (
            ...
            "CASE WHEN compliance_pct >= 98 THEN 'Excellent' WHEN compliance_pct >= 90 THEN 'Good' WHEN compliance_pct >= 80 THEN 'Acceptable' ELSE 'Needs Improvement' END",
        ),
        (
            "mask_serial",
            "serial STRING",
            "STRING",
            "Masks equipment serial number",
            "concat('SN-***',right(serial,4))",
        ),
        (
            "production_status",
            "produced INT, ordered INT",
            "STRING",
            "Production completion status",
            "CASE WHEN produced >= ordered THEN 'Complete' WHEN produced >= ordered * 0.75 THEN 'Nearly Complete' WHEN produced >= ordered * 0.5 THEN 'In Progress' ELSE 'Early Stage' END",
        ),
        (
            "quality_cpk",
            "measurement DOUBLE, spec_min DOUBLE, spec_max DOUBLE",
            "DOUBLE",
            "Simplified process capability",
            "round(LEAST(measurement - spec_min, spec_max - measurement) / ((spec_max - spec_min) / 6), 3)",
        ),
        (
            "anonymize_operator",
            "operator_id BIGINT",
            "STRING",
            "Anonymized operator reference",
            "concat('OP-',substring(sha2(cast(operator_id as STRING),256),1,8))",
        ),
    ],
}
...
INDUSTRIES["energy"] = {
    "tables": [
        _quick_table(
            "meter_readings",
            100_000_000,
            "reading_id BIGINT, meter_id BIGINT, reading_type STRING, value DOUBLE, unit STRING, reading_timestamp TIMESTAMP, quality STRING, tariff_code STRING, is_estimated BOOLEAN, location_id BIGINT",
        ...
        _quick_table(
            "grid_events",
            50_000_000,
            "event_id BIGINT, station_id BIGINT, event_type STRING, severity STRING, duration_minutes INT, affected_customers INT, event_timestamp TIMESTAMP, cause STRING, restoration_time TIMESTAMP, outage_mw DOUBLE",
        ...
        _quick_table(
            "generation_output",
            30_000_000,
            "output_id BIGINT, plant_id BIGINT, fuel_type STRING, output_mwh DOUBLE, capacity_factor DECIMAL(5,2), co2_tons DOUBLE, generation_date DATE, price_per_mwh DECIMAL(8,2), dispatch_order INT, availability DECIMAL(5,2)",
        ...
        _quick_table(
            "billing_energy",
            5_000_000,
            "bill_id BIGINT, account_id BIGINT, billing_period DATE, usage_kwh DOUBLE, demand_kw DOUBLE, base_charge DECIMAL(8,2), usage_charge DECIMAL(8,2), demand_charge DECIMAL(8,2), taxes DECIMAL(8,2), total DECIMAL(10,2)",
        ...
        _quick_table(
            "demand_forecast",
            5_000_000,
            "forecast_id BIGINT, region STRING, forecast_date DATE, hour INT, predicted_mw DOUBLE, actual_mw DOUBLE, temperature_f DOUBLE, model_version STRING, error_pct DECIMAL(5,2), confidence DECIMAL(5,2)",
        ...
        _quick_table(
            "customers_energy",
            1_000_000,
            "customer_id BIGINT, name STRING, customer_type STRING, address STRING, city STRING, state STRING, zip STRING, rate_class STRING, meter_id BIGINT, enrolled_date DATE",
        ...
        _quick_table(
            "power_plants",
            1_000_000,
            "plant_id BIGINT, name STRING, fuel_type STRING, capacity_mw DOUBLE, location STRING, state STRING, commissioned DATE, status STRING, operator STRING, emission_rate DOUBLE",
        ...
        _quick_table(
            ...
            1_000_000,
            "cert_id BIGINT, plant_id BIGINT, cert_type STRING, mwh_generated DOUBLE, vintage_year INT, state STRING, status STRING, price DECIMAL(8,2), buyer_id BIGINT, created_date DATE",
        ...
        _quick_table(
            "substations",
            500_000,
            "station_id BIGINT, name STRING, voltage_kv INT, station_type STRING, latitude DOUBLE, longitude DOUBLE, capacity_mva DOUBLE, status STRING, last_inspection DATE, transformer_count INT",
        ...
        _quick_table(
            "tariff_rates",
            500_000,
            "rate_id BIGINT, rate_class STRING, rate_type STRING, price_per_kwh DECIMAL(8,6), effective_date DATE, season STRING, time_of_use STRING, tier INT",
        ...
        _quick_table(
            "outage_reports",
            300_000,
            "report_id BIGINT, station_id BIGINT, reported_by STRING, report_time TIMESTAMP, cause STRING, customers_affected INT, estimated_restoration TIMESTAMP, actual_restoration TIMESTAMP, status STRING",
        ...
        _quick_table(
            "emissions",
            300_000,
            "emission_id BIGINT, plant_id BIGINT, pollutant STRING, amount_tons DOUBLE, measurement_date DATE, reporting_period STRING, method STRING, compliant BOOLEAN",
        ...
        _quick_table(
            "ev_charging",
            200_000,
            "session_id BIGINT, station_id BIGINT, connector_type STRING, energy_kwh DOUBLE, duration_minutes INT, cost DECIMAL(8,2), start_time TIMESTAMP, vehicle_type STRING, payment_method STRING",
        ...
        _quick_table(
            "solar_panels",
            200_000,
            "panel_id BIGINT, installation_id BIGINT, manufacturer STRING, capacity_kw DECIMAL(8,2), efficiency DECIMAL(5,2), install_date DATE, warranty_expiry DATE, degradation_rate DECIMAL(5,4), status STRING",
        ...
        _quick_table(
            "battery_storage",
            200_000,
            "battery_id BIGINT, site_id BIGINT, technology STRING, capacity_mwh DOUBLE, power_mw DOUBLE, soc_pct DECIMAL(5,2), cycles INT, install_date DATE, status STRING, degradation_pct DECIMAL(5,2)",
        ...
        _quick_table(
            "transmission_lines",
            100_000,
            "line_id BIGINT, from_station BIGINT, to_station BIGINT, voltage_kv INT, length_miles DOUBLE, capacity_mw DOUBLE, conductor_type STRING, year_built INT, status STRING",
        ...
        _quick_table(
            "smart_thermostats",
            100_000,
            "device_id BIGINT, customer_id BIGINT, set_temp_f DOUBLE, actual_temp_f DOUBLE, mode STRING, reading_time TIMESTAMP, energy_saved_kwh DOUBLE, is_dr_enrolled BOOLEAN",
        ...
        _quick_table(
            "demand_response",
            100_000,
            "event_id BIGINT, program STRING, event_date DATE, start_hour INT, end_hour INT, target_reduction_mw DOUBLE, actual_reduction_mw DOUBLE, participants INT, compensation_per_mw DECIMAL(8,2)",
        ...
        _quick_table(
            "carbon_credits",
            100_000,
            "credit_id BIGINT, project_type STRING, vintage_year INT, tons_co2 DOUBLE, price_per_ton DECIMAL(8,2), registry STRING, status STRING, buyer STRING, retirement_date DATE",
    ...
    "views": [
        (
            "v_daily_generation",
            "SELECT generation_date, fuel_type, sum(output_mwh) AS total_mwh, avg(capacity_factor) AS avg_cf FROM {s}.generation_output GROUP BY generation_date, fuel_type",
        ),
        (
            "v_outage_summary",
            "SELECT cause, count(*) AS outages, sum(customers_affected) AS total_affected, avg(duration_minutes) AS avg_duration FROM {s}.grid_events WHERE event_type = 'outage' GROUP BY cause",
        ),
        (
            "v_billing_summary",
            "SELECT date_trunc('month',billing_period) AS month, count(*) AS bills, sum(total) AS revenue, avg(usage_kwh) AS avg_usage FROM {s}.billing_energy GROUP BY date_trunc('month',billing_period)",
        ),
        (
            "v_emissions_by_plant",
            "SELECT pp.name, pp.fuel_type, e.pollutant, sum(e.amount_tons) AS total_tons FROM {s}.emissions e JOIN {s}.power_plants pp ON e.plant_id = pp.plant_id GROUP BY pp.name, pp.fuel_type, e.pollutant",
        ),
        (
            "v_renewable_mix",
            "SELECT fuel_type, sum(capacity_mw) AS total_capacity, count(*) AS plants FROM {s}.power_plants WHERE fuel_type IN ('solar','wind','hydro','geothermal','biomass') GROUP BY fuel_type",
        ),
        (
            "v_ev_charging_trends",
            "SELECT connector_type, date_trunc('month',start_time) AS month, count(*) AS sessions, sum(energy_kwh) AS total_kwh FROM {s}.ev_charging GROUP BY connector_type, date_trunc('month',start_time)",
        ),
        (
            "v_forecast_accuracy",
            "SELECT region, model_version, avg(abs(error_pct)) AS mae, count(*) AS forecasts FROM {s}.demand_forecast GROUP BY region, model_version",
        ),
        (
            "v_battery_fleet",
            "SELECT technology, status, count(*) AS units, sum(capacity_mwh) AS total_mwh FROM {s}.battery_storage GROUP BY technology, status",
        ),
        (
            "v_carbon_portfolio",
            "SELECT project_type, registry, sum(tons_co2) AS total_tons, avg(price_per_ton) AS avg_price FROM {s}.carbon_credits WHERE status = 'active' GROUP BY project_type, registry",
        ),
        (
            "v_customer_by_type",
            "SELECT customer_type, rate_class, count(*) AS customers FROM {s}.customers_energy GROUP BY customer_type, rate_class",
        ),
        (
            "v_grid_reliability",
            "SELECT station_id, count(*) AS events, avg(duration_minutes) AS avg_duration FROM {s}.grid_events GROUP BY station_id",
        ),
        (
            "v_solar_performance",
            "SELECT manufacturer, avg(efficiency) AS avg_efficiency, count(*) AS panels FROM {s}.solar_panels WHERE status = 'active' GROUP BY manufacturer",
        ),
        (
            "v_demand_response_performance",
            "SELECT program, count(*) AS events, sum(actual_reduction_mw) AS total_mw_reduced FROM {s}.demand_response GROUP BY program",
        ),
        (
            "v_tariff_comparison",
            "SELECT rate_class, season, time_of_use, avg(price_per_kwh) AS avg_price FROM {s}.tariff_rates GROUP BY rate_class, season, time_of_use",
        ),
        (
            "v_thermostat_savings",
            "SELECT mode, count(*) AS readings, avg(energy_saved_kwh) AS avg_saved FROM {s}.smart_thermostats GROUP BY mode",
        ),
        (
            "v_cert_market",
            "SELECT cert_type, vintage_year, count(*) AS certs, avg(price) AS avg_price FROM {s}.renewable_certificates GROUP BY cert_type, vintage_year",
        ),
        (
            "v_transmission_age",
            "SELECT voltage_kv, status, count(*) AS lines, avg(2025-year_built) AS avg_age FROM {s}.transmission_lines GROUP BY voltage_kv, status",
        ),
        (
            "v_meter_quality",
            "SELECT reading_type, quality, count(*) AS readings FROM {s}.meter_readings GROUP BY reading_type, quality",
        ),
        (
            "v_plant_utilization",
            "SELECT pp.fuel_type, pp.status, avg(go.capacity_factor) AS avg_cf FROM {s}.power_plants pp JOIN {s}.generation_output go ON pp.plant_id = go.plant_id GROUP BY pp.fuel_type, pp.status",
        ),
        (
            "v_substation_load",
            "SELECT station_type, status, count(*) AS stations, avg(capacity_mva) AS avg_capacity FROM {s}.substations GROUP BY station_type, status",
        ),
    ],
    "udfs": [
        (
            ...
            "STRING",
            "Formats MWh",
            "CASE WHEN mwh >= 1000 THEN concat(round(mwh/1000,1),' GWh') ELSE concat(round(mwh,1),' MWh') END",
        ),
        (
            "carbon_intensity",
            "co2_tons DOUBLE, mwh DOUBLE",
            "DECIMAL(8,4)",
            "CO2 intensity kg/MWh",
            "round(co2_tons*1000/GREATEST(mwh,1),4)",
        ),
        (
            "rate_class_label",
            "code STRING",
            "STRING",
            "Rate class label",
            "CASE WHEN code LIKE 'R%' THEN 'Residential' WHEN code LIKE 'C%' THEN 'Commercial' WHEN code LIKE 'I%' THEN 'Industrial' ELSE 'Other' END",
        ),
        (
            ...
            "CASE WHEN abs(freq-60) < 0.02 THEN 'Normal' WHEN abs(freq-60) < 0.05 THEN 'Alert' ELSE 'Emergency' END",
        ),
        (
            "solar_degradation",
            "capacity DECIMAL(8,2), rate DECIMAL(5,4), years INT",
            "DECIMAL(8,2)",
            "Degraded solar capacity",
            "round(capacity * power(1-rate, years), 2)",
        ),
        (
            "battery_soh",
            "cycles INT, max_cycles INT",
            "DECIMAL(5,2)",
            "Battery state of health",
            "round((1 - cycles*1.0/GREATEST(max_cycles,1)) * 100, 2)",
        ),
        (
            "peak_pricing",
            "hour INT, season STRING",
            "STRING",
            "Peak pricing tier",
            "CASE WHEN season='summer' AND hour BETWEEN 14 AND 19 THEN 'Super Peak' WHEN hour BETWEEN 7 AND 21 THEN 'Peak' ELSE 'Off-Peak' END",
        ),
        (
            "emission_compliance",
            "actual DOUBLE, limit_val DOUBLE",
            "STRING",
            "Compliance status",
            "CASE WHEN actual <= limit_val*0.8 THEN 'Well Below' WHEN actual <= limit_val THEN 'Compliant' ELSE 'Exceeds Limit' END",
        ),
        (
            "ev_charge_cost",
            "kwh DOUBLE, rate DECIMAL(8,6)",
            "DECIMAL(8,2)",
            "EV charging cost",
            "round(kwh*rate, 2)",
        ),
        (
            "thermostat_mode_icon",
            "mode STRING",
            "STRING",
            "Thermostat mode",
            "CASE WHEN mode='heat' THEN '🔥 Heat' WHEN mode='cool' THEN '❄️ Cool' WHEN mode='eco' THEN '🌿 Eco' WHEN mode='auto' THEN '🔄 Auto' ELSE '⏸ Off' END",
        ),
        (
            "outage_severity",
            "customers INT, duration INT",
            "STRING",
            "Outage severity",
            "CASE WHEN customers > 10000 OR duration > 240 THEN 'Major' WHEN customers > 1000 OR duration > 60 THEN 'Significant' ELSE 'Minor' END",
        ),
        (
            "voltage_level",
            "kv INT",
            ...
            "CASE WHEN kv >= 345 THEN 'Extra High' WHEN kv >= 115 THEN 'High' WHEN kv >= 35 THEN 'Medium' ELSE 'Low' END",
        ),
        (
            "fuel_color",
            "fuel STRING",
            "STRING",
            "Fuel type color code",
            "CASE WHEN fuel IN ('solar','wind','hydro','geothermal') THEN 'Green' WHEN fuel = 'nuclear' THEN 'Blue' WHEN fuel = 'natural_gas' THEN 'Gray' ELSE 'Brown' END",
        ),
        (
            "capacity_factor_grade",
            "cf DECIMAL(5,2)",
            "STRING",
            "Capacity factor grade",
            "CASE WHEN cf >= 80 THEN 'A' WHEN cf >= 60 THEN 'B' WHEN cf >= 40 THEN 'C' WHEN cf >= 20 THEN 'D' ELSE 'F' END",
        ),
        (
            "format_co2",
            "tons DOUBLE",
            "STRING",
            "Format CO2 display",
            "CASE WHEN tons >= 1000000 THEN concat(round(tons/1000000,1),' MT') WHEN tons >= 1000 THEN concat(round(tons/1000,1),' KT') ELSE concat(round(tons,1),' T') END",
        ),
        (
            "is_renewable",
            "fuel STRING",
            ...
            "fuel IN ('solar','wind','hydro','geothermal','biomass')",
        ),
        (
            "demand_response_savings",
            "reduction_mw DOUBLE, hours INT, price DECIMAL(8,2)",
            "DECIMAL(10,2)",
            "DR event savings",
            "round(reduction_mw * hours * price, 2)",
        ),
        (
            "line_loss_estimate",
            "distance_miles DOUBLE, voltage_kv INT",
            "DECIMAL(5,2)",
            "Estimated line loss %",
            "round(distance_miles * 0.01 * (765.0 / GREATEST(voltage_kv, 1)), 2)",
        ),
        (
            "meter_read_quality",
            "is_estimated BOOLEAN, quality STRING",
            "STRING",
            "Meter read quality label",
            "CASE WHEN is_estimated THEN 'Estimated' WHEN quality = 'validated' THEN 'Validated' ELSE 'Actual' END",
        ),
        (
            "anonymize_meter",
            "meter_id BIGINT",
            "STRING",
            "Anonymized meter ref",
            "concat('MTR-',substring(sha2(cast(meter_id as STRING),256),1,8))",
        ),
    ],
}
...
INDUSTRIES["education"] = {
    "tables": [
        _quick_table(
            "enrollments",
            100_000_000,
            "enrollment_id BIGINT, student_id BIGINT, course_id BIGINT, semester STRING, grade STRING, credits INT, enrollment_date DATE, status STRING, gpa DECIMAL(3,2), instructor_id BIGINT",
        ...
        _quick_table(
            "learning_events",
            50_000_000,
            "event_id BIGINT, student_id BIGINT, course_id BIGINT, event_type STRING, content_id BIGINT, duration_seconds INT, event_timestamp TIMESTAMP, device STRING, score DECIMAL(5,2), completion_pct DECIMAL(5,2)",
        ...
        _quick_table(
            "assessments",
            30_000_000,
            "assessment_id BIGINT, student_id BIGINT, course_id BIGINT, assessment_type STRING, score DECIMAL(5,2), max_score DECIMAL(5,2), submitted_at TIMESTAMP, graded_at TIMESTAMP, grader_id BIGINT, feedback STRING",
        ...
        _quick_table(
            "students",
            1_000_000,
            "student_id BIGINT, first_name STRING, last_name STRING, email STRING, date_of_birth DATE, major STRING, enrollment_year INT, status STRING, gpa DECIMAL(3,2), financial_aid BOOLEAN",
        ...
        _quick_table(
            "courses",
            1_000_000,
            "course_id BIGINT, code STRING, title STRING, department STRING, credits INT, level STRING, capacity INT, instructor_id BIGINT, is_online BOOLEAN, prerequisite_id BIGINT",
        ...
        _quick_table(
            "instructors",
            500_000,
            "instructor_id BIGINT, first_name STRING, last_name STRING, email STRING, department STRING, title STRING, tenure BOOLEAN, hire_date DATE, research_area STRING, rating DECIMAL(3,1)",
        ...
        _quick_table(
            "financial_aid",
            500_000,
            "aid_id BIGINT, student_id BIGINT, aid_type STRING, amount DECIMAL(10,2), academic_year STRING, status STRING, disbursement_date DATE, gpa_requirement DECIMAL(3,2)",
        ...
        _quick_table(
            "facilities_edu",
            300_000,
            "facility_id BIGINT, name STRING, building STRING, facility_type STRING, capacity INT, has_av BOOLEAN, is_accessible BOOLEAN, floor_num INT",
        ...
        _quick_table(
            "research_grants",
            200_000,
            "grant_id BIGINT, pi_id BIGINT, title STRING, funding_agency STRING, amount DECIMAL(12,2), start_date DATE, end_date DATE, status STRING, department STRING",
        ...
        _quick_table(
            "library_checkouts",
            200_000,
            "checkout_id BIGINT, student_id BIGINT, item_id BIGINT, item_type STRING, checkout_date DATE, due_date DATE, return_date DATE, renewals INT, status STRING",
        ...
        _quick_table(
            "alumni",
            200_000,
            "alumni_id BIGINT, student_id BIGINT, graduation_year INT, degree STRING, major STRING, employer STRING, job_title STRING, salary_range STRING, donated BOOLEAN, donation_total DECIMAL(10,2)",
        ...
        _quick_table(
            "campus_events",
            100_000,
            "event_id BIGINT, title STRING, event_type STRING, organizer STRING, venue_id BIGINT, event_date DATE, attendees INT, budget DECIMAL(8,2), status STRING",
        ...
        _quick_table(
            "dining_transactions",
            300_000,
            "txn_id BIGINT, student_id BIGINT, location STRING, amount DECIMAL(6,2), txn_date DATE, meal_type STRING, payment_type STRING",
        ...
        _quick_table(
            "housing",
            200_000,
            "assignment_id BIGINT, student_id BIGINT, building STRING, room_number STRING, room_type STRING, semester STRING, monthly_rate DECIMAL(8,2), status STRING",
        ...
        _quick_table(
            "parking_permits",
            100_000,
            "permit_id BIGINT, holder_id BIGINT, holder_type STRING, lot STRING, vehicle_plate STRING, start_date DATE, end_date DATE, cost DECIMAL(6,2), status STRING",
        ...
        _quick_table(
            "athletics",
            100_000,
            "event_id BIGINT, sport STRING, event_date DATE, opponent STRING, home_score INT, away_score INT, attendance INT, venue STRING, season STRING, result STRING",
        ...
        _quick_table(
            "it_helpdesk",
            100_000,
            "ticket_id BIGINT, requester_id BIGINT, requester_type STRING, category STRING, priority STRING, created_date DATE, resolved_date DATE, status STRING, satisfaction INT",
        ...
        _quick_table(
            "departments",
            100_000,
            "dept_id BIGINT, name STRING, college STRING, head_id BIGINT, budget DECIMAL(12,2), faculty_count INT, student_count INT, established_year INT",
    ...
    "views": [
        (
            "v_student_gpa",
            "SELECT student_id, avg(gpa) AS avg_gpa, count(*) AS courses FROM {s}.enrollments WHERE status = 'completed' GROUP BY student_id",
        ),
        (
            "v_course_enrollment",
            "SELECT c.code, c.title, c.department, count(e.enrollment_id) AS enrolled, c.capacity FROM {s}.courses c LEFT JOIN {s}.enrollments e ON c.course_id = e.course_id GROUP BY c.code, c.title, c.department, c.capacity",
        ),
        (
            "v_department_summary",
            "SELECT department, count(DISTINCT student_id) AS students, count(DISTINCT course_id) AS courses, avg(gpa) AS avg_gpa FROM {s}.enrollments e JOIN {s}.courses c ON e.course_id = c.course_id GROUP BY department",
        ),
        (
            "v_instructor_ratings",
            "SELECT i.first_name, i.last_name, i.department, i.rating, count(DISTINCT e.student_id) AS students_taught FROM {s}.instructors i LEFT JOIN {s}.courses c ON i.instructor_id = c.instructor_id LEFT JOIN {s}.enrollments e ON c.course_id = e.course_id GROUP BY i.first_name, i.last_name, i.department, i.rating",
        ),
        (
            "v_financial_aid_summary",
            "SELECT aid_type, academic_year, count(*) AS awards, sum(amount) AS total_awarded FROM {s}.financial_aid GROUP BY aid_type, academic_year",
        ),
        (
            "v_research_funding",
            "SELECT department, funding_agency, count(*) AS grants, sum(amount) AS total_funding FROM {s}.research_grants WHERE status = 'active' GROUP BY department, funding_agency",
        ),
        (
            "v_retention_rate",
            "SELECT enrollment_year, status, count(*) AS students FROM {s}.students GROUP BY enrollment_year, status",
        ),
        (
            "v_learning_engagement",
            "SELECT event_type, device, count(*) AS events, avg(duration_seconds) AS avg_duration FROM {s}.learning_events GROUP BY event_type, device",
        ),
        (
            "v_assessment_results",
            "SELECT assessment_type, avg(score) AS avg_score, avg(score/max_score*100) AS avg_pct FROM {s}.assessments GROUP BY assessment_type",
        ),
        (
            "v_alumni_outcomes",
            "SELECT major, degree, avg(CASE salary_range WHEN '200k+' THEN 225000 WHEN '150-200k' THEN 175000 WHEN '100-150k' THEN 125000 WHEN '50-100k' THEN 75000 ELSE 25000 END) AS est_avg_salary FROM {s}.alumni GROUP BY major, degree",
        ),
        (
            "v_library_usage",
            "SELECT item_type, status, count(*) AS checkouts FROM {s}.library_checkouts GROUP BY item_type, status",
        ),
        (
            "v_housing_occupancy",
            "SELECT building, room_type, count(*) AS rooms, sum(CASE WHEN status='assigned' THEN 1 ELSE 0 END) AS occupied FROM {s}.housing GROUP BY building, room_type",
        ),
        (
            "v_dining_revenue",
            "SELECT location, meal_type, count(*) AS transactions, sum(amount) AS revenue FROM {s}.dining_transactions GROUP BY location, meal_type",
        ),
        (
            "v_athletic_record",
            "SELECT sport, result, count(*) AS games FROM {s}.athletics GROUP BY sport, result",
        ),
        (
            "v_it_support_metrics",
            "SELECT category, priority, avg(datediff(resolved_date,created_date)) AS avg_resolution_days, avg(satisfaction) AS avg_satisfaction FROM {s}.it_helpdesk WHERE status IN ('resolved','closed') GROUP BY category, priority",
        ),
        (
            "v_campus_events_calendar",
            "SELECT event_type, date_trunc('month',event_date) AS month, count(*) AS events, sum(attendees) AS total_attendees FROM {s}.campus_events GROUP BY event_type, date_trunc('month',event_date)",
        ),
        (
            "v_parking_utilization",
            "SELECT lot, holder_type, count(*) AS permits FROM {s}.parking_permits WHERE status='active' GROUP BY lot, holder_type",
        ),
        (
            "v_major_popularity",
            "SELECT major, count(*) AS students, avg(gpa) AS avg_gpa FROM {s}.students WHERE status='active' GROUP BY major",
        ),
        (
            "v_grant_by_agency",
            "SELECT funding_agency, count(*) AS grants, sum(amount) AS total, avg(amount) AS avg_grant FROM {s}.research_grants GROUP BY funding_agency",
        ),
        (
            "v_facility_usage",
            "SELECT facility_type, count(*) AS rooms, avg(capacity) AS avg_capacity FROM {s}.facilities_edu GROUP BY facility_type",
        ),
    ],
    "udfs": [
        (
            ...
            "CASE WHEN gpa >= 3.7 THEN 'A' WHEN gpa >= 3.3 THEN 'A-' WHEN gpa >= 3.0 THEN 'B+' WHEN gpa >= 2.7 THEN 'B' WHEN gpa >= 2.3 THEN 'B-' WHEN gpa >= 2.0 THEN 'C+' WHEN gpa >= 1.7 THEN 'C' WHEN gpa >= 1.0 THEN 'D' ELSE 'F' END",
        ),
        (
            "academic_standing",
            "gpa DECIMAL(3,2)",
            "STRING",
            "Academic standing",
            "CASE WHEN gpa >= 3.5 THEN 'Deans List' WHEN gpa >= 2.0 THEN 'Good Standing' WHEN gpa >= 1.5 THEN 'Probation' ELSE 'Suspension Risk' END",
        ),
        (
            "credit_hours_status",
            "credits INT, year INT",
            "STRING",
            "Credit progress",
            "CASE WHEN credits >= 120 THEN 'Senior' WHEN credits >= 90 THEN 'Junior' WHEN credits >= 60 THEN 'Sophomore' ELSE 'Freshman' END",
        ),
        (
            "mask_student_email",
            "email STRING",
            "STRING",
            "Masks student email",
            "concat(left(email,3),'***@university.edu')",
        ),
        ...
        (
            ...
            "BOOLEAN",
            "STEM major check",
            "major IN ('Computer Science','Engineering','Mathematics','Physics','Chemistry','Biology')",
        ),
        (
            "aid_per_semester",
            "amount DECIMAL(10,2), semesters INT",
            "DECIMAL(10,2)",
            "Aid per semester",
            "round(amount / GREATEST(semesters, 1), 2)",
        ),
        (
            "course_level",
            "code STRING",
            "STRING",
            "Course level from code",
            "CASE WHEN cast(right(code,3) as INT) >= 500 THEN 'Graduate' WHEN cast(right(code,3) as INT) >= 400 THEN 'Senior' WHEN cast(right(code,3) as INT) >= 300 THEN 'Junior' WHEN cast(right(code,3) as INT) >= 200 THEN 'Sophomore' ELSE 'Freshman' END",
        ),
        (
            "graduation_eligible",
            "credits INT, gpa DECIMAL(3,2)",
            "BOOLEAN",
        ...
        (
            ...
            "concat(cast(round(score/max_score*100,1) as STRING),'%')",
        ),
        (
            "research_status",
            ...
            "STRING",
            "Grant status",
            "CASE WHEN current_date() < start_date THEN 'Not Started' WHEN current_date() > end_date THEN 'Completed' ELSE 'Active' END",
        ),
        (
            "housing_cost_semester",
            "monthly DECIMAL(8,2)",
            "DECIMAL(8,2)",
        ...
        (
            "meal_plan_remaining",
            "total DECIMAL(8,2), spent DECIMAL(8,2)",
            "STRING",
            "Meal plan balance",
            "concat('$',format_number(total-spent,2),' remaining')",
        ),
        (
            "anonymize_student",
            "first_name STRING, student_id BIGINT",
            "STRING",
            "Anonymized student ref",
            "concat('STU-',substring(sha2(concat(first_name,cast(student_id as STRING)),256),1,8))",
        ),
        (
            "instructor_title_rank",
            "title STRING",
            "INT",
            "Rank by title",
            "CASE WHEN title = 'Professor' THEN 5 WHEN title = 'Associate Professor' THEN 4 WHEN title = 'Assistant Professor' THEN 3 WHEN title = 'Lecturer' THEN 2 ELSE 1 END",
        ),
        (
            "is_overdue",
            "due_date DATE, return_date DATE",
            "BOOLEAN",
            "Check if library item overdue",
            "return_date IS NULL OR return_date > due_date",
        ),
        (
            "event_size",
            "attendees INT",
            "STRING",
            "Event size category",
            "CASE WHEN attendees > 5000 THEN 'Large' WHEN attendees > 500 THEN 'Medium' WHEN attendees > 50 THEN 'Small' ELSE 'Micro' END",
        ),
        (
            "ticket_sla",
            "priority STRING, days INT",
            "BOOLEAN",
            "SLA met check",
            "days <= CASE WHEN priority = 'critical' THEN 1 WHEN priority = 'high' THEN 3 WHEN priority = 'medium' THEN 5 ELSE 10 END",
        ),
        (
            "donation_tier",
            "total DECIMAL(10,2)",
            "STRING",
            "Donor tier",
            "CASE WHEN total >= 10000 THEN 'Platinum' WHEN total >= 5000 THEN 'Gold' WHEN total >= 1000 THEN 'Silver' ELSE 'Bronze' END",
        ),
        (
            "class_year",
            "enrollment_year INT",
            "STRING",
            "Expected graduation",
            "concat('Class of ',enrollment_year + 4)",
        ),
    ],
}
...
INDUSTRIES["real_estate"] = {
    "tables": [
        _quick_table(
            "listings",
            100_000_000,
            "listing_id BIGINT, property_id BIGINT, list_price DECIMAL(12,2), list_date DATE, status STRING, agent_id BIGINT, property_type STRING, bedrooms INT, bathrooms DECIMAL(3,1), sqft INT",
        ...
        _quick_table(
            "transactions_re",
            50_000_000,
            "txn_id BIGINT, listing_id BIGINT, buyer_id BIGINT, seller_id BIGINT, sale_price DECIMAL(12,2), closing_date DATE, mortgage_amount DECIMAL(12,2), down_payment_pct DECIMAL(5,2), agent_commission DECIMAL(10,2), days_on_market INT",
        ...
        _quick_table(
            "property_views",
            30_000_000,
            "view_id BIGINT, listing_id BIGINT, viewer_id BIGINT, view_type STRING, view_date DATE, duration_minutes INT, source STRING, saved BOOLEAN, inquired BOOLEAN",
        ...
        _quick_table(
            "properties",
            1_000_000,
            "property_id BIGINT, address STRING, city STRING, state STRING, zip STRING, property_type STRING, year_built INT, lot_size_sqft INT, assessed_value DECIMAL(12,2), last_sale_date DATE",
        ...
        _quick_table(
            "agents",
            1_000_000,
            "agent_id BIGINT, first_name STRING, last_name STRING, brokerage STRING, license_number STRING, email STRING, phone STRING, specialization STRING, years_experience INT, listings_active INT",
        ...
        _quick_table(
            "mortgages",
            1_000_000,
            "mortgage_id BIGINT, property_id BIGINT, borrower_id BIGINT, lender STRING, loan_amount DECIMAL(12,2), interest_rate DECIMAL(5,4), term_years INT, loan_type STRING, origination_date DATE, status STRING",
        ...
        _quick_table(
            "appraisals",
            500_000,
            "appraisal_id BIGINT, property_id BIGINT, appraised_value DECIMAL(12,2), appraisal_date DATE, appraiser_id BIGINT, purpose STRING, condition STRING, market_trend STRING",
        ...
        _quick_table(
            "inspections",
            500_000,
            "inspection_id BIGINT, property_id BIGINT, inspection_date DATE, inspector_id BIGINT, inspection_type STRING, overall_rating STRING, issues_found INT, estimated_repair_cost DECIMAL(10,2), report_url STRING",
        ...
        _quick_table(
            "rental_payments",
            500_000,
            "payment_id BIGINT, lease_id BIGINT, tenant_id BIGINT, amount DECIMAL(8,2), payment_date DATE, due_date DATE, payment_method STRING, status STRING, late_fee DECIMAL(6,2)",
        ...
        _quick_table(
            "neighborhoods",
            500_000,
            "neighborhood_id BIGINT, name STRING, city STRING, state STRING, zip STRING, median_price DECIMAL(12,2), avg_sqft_price DECIMAL(8,2), school_rating INT, crime_index DECIMAL(5,2), walkability_score INT",
        ...
        _quick_table(
            "open_houses",
            300_000,
            "oh_id BIGINT, listing_id BIGINT, agent_id BIGINT, oh_date DATE, start_time STRING, end_time STRING, attendees INT, offers_received INT",
        ...
        _quick_table(
            "market_trends",
            200_000,
            "trend_id BIGINT, zip_code STRING, month DATE, median_price DECIMAL(12,2), avg_days_on_market INT, inventory INT, new_listings INT, closed_sales INT, price_per_sqft DECIMAL(8,2)",
        ...
        _quick_table(
            "hoa_dues",
            200_000,
            "hoa_id BIGINT, property_id BIGINT, community STRING, monthly_dues DECIMAL(8,2), special_assessment DECIMAL(8,2), reserve_fund DECIMAL(12,2), paid_through DATE, status STRING",
        ...
        _quick_table(
            "property_taxes",
            200_000,
            "tax_id BIGINT, property_id BIGINT, tax_year INT, assessed_value DECIMAL(12,2), tax_rate DECIMAL(6,4), tax_amount DECIMAL(10,2), paid_date DATE, status STRING",
        ...
        _quick_table(
            ...
            100_000,
            "permit_id BIGINT, property_id BIGINT, permit_type STRING, description STRING, estimated_cost DECIMAL(12,2), issue_date DATE, expiry_date DATE, status STRING, contractor STRING",
        ...
        _quick_table(
            "comparables",
            100_000,
            "comp_id BIGINT, subject_property_id BIGINT, comp_property_id BIGINT, distance_miles DOUBLE, price_diff_pct DECIMAL(5,2), sqft_diff INT, sale_date DATE, adjustment DECIMAL(10,2)",
    ...
    "views": [
        (
            "v_market_summary",
            "SELECT p.city, p.state, count(l.listing_id) AS listings, avg(l.list_price) AS avg_price, avg(l.sqft) AS avg_sqft FROM {s}.listings l JOIN {s}.properties p ON l.property_id = p.property_id GROUP BY p.city, p.state",
        ),
        (
            "v_agent_performance",
            "SELECT a.first_name, a.last_name, a.brokerage, count(t.txn_id) AS sales, sum(t.sale_price) AS total_volume FROM {s}.agents a LEFT JOIN {s}.transactions_re t ON a.agent_id = t.buyer_id OR a.agent_id = t.seller_id GROUP BY a.first_name, a.last_name, a.brokerage",
        ),
        (
            "v_price_trends",
            "SELECT mt.zip_code, mt.month, mt.median_price, mt.avg_days_on_market FROM {s}.market_trends mt ORDER BY mt.zip_code, mt.month",
        ),
        (
            "v_listing_funnel",
            "SELECT l.status, count(*) AS listings, avg(l.list_price) AS avg_price FROM {s}.listings l GROUP BY l.status",
        ),
        (
            "v_property_by_type",
            "SELECT property_type, count(*) AS properties, avg(assessed_value) AS avg_value FROM {s}.properties GROUP BY property_type",
        ),
        (
            "v_mortgage_portfolio",
            "SELECT lender, loan_type, count(*) AS loans, avg(interest_rate) AS avg_rate, sum(loan_amount) AS total FROM {s}.mortgages GROUP BY lender, loan_type",
        ),
        (
            "v_rental_collection",
            "SELECT status, count(*) AS payments, sum(amount) AS total, sum(late_fee) AS total_fees FROM {s}.rental_payments GROUP BY status",
        ),
        (
            "v_neighborhood_rankings",
            "SELECT name, city, median_price, school_rating, walkability_score FROM {s}.neighborhoods ORDER BY median_price DESC",
        ),
        (
            "v_open_house_conversion",
            "SELECT oh.listing_id, count(*) AS open_houses, sum(oh.attendees) AS total_visitors, sum(oh.offers_received) AS total_offers FROM {s}.open_houses oh GROUP BY oh.listing_id",
        ),
        (
            "v_tax_assessment",
            "SELECT tax_year, count(*) AS properties, avg(assessed_value) AS avg_assessed, avg(tax_amount) AS avg_tax FROM {s}.property_taxes GROUP BY tax_year",
        ),
        (
            "v_construction_activity",
            "SELECT permit_type, status, count(*) AS permits, sum(estimated_cost) AS total_value FROM {s}.construction_permits GROUP BY permit_type, status",
        ),
        (
            "v_appraisal_accuracy",
            "SELECT purpose, avg(abs(a.appraised_value - l.list_price)/l.list_price*100) AS avg_diff_pct FROM {s}.appraisals a JOIN {s}.listings l ON a.property_id = l.property_id GROUP BY purpose",
        ),
        (
            "v_inspection_issues",
            "SELECT inspection_type, avg(issues_found) AS avg_issues, avg(estimated_repair_cost) AS avg_repair_cost FROM {s}.inspections GROUP BY inspection_type",
        ),
        (
            "v_hoa_delinquency",
            "SELECT community, status, count(*) AS units, sum(monthly_dues) AS monthly_revenue FROM {s}.hoa_dues GROUP BY community, status",
        ),
        (
            "v_days_on_market",
            "SELECT l.property_type, avg(t.days_on_market) AS avg_dom, min(t.days_on_market) AS min_dom, max(t.days_on_market) AS max_dom FROM {s}.transactions_re t JOIN {s}.listings l ON t.listing_id = l.listing_id GROUP BY l.property_type",
        ),
        (
            "v_comp_analysis",
            "SELECT subject_property_id, count(*) AS comps_used, avg(price_diff_pct) AS avg_price_diff, avg(distance_miles) AS avg_distance FROM {s}.comparables GROUP BY subject_property_id",
        ),
        (
            "v_monthly_closings",
            "SELECT date_trunc('month',closing_date) AS month, count(*) AS sales, avg(sale_price) AS avg_price FROM {s}.transactions_re GROUP BY date_trunc('month',closing_date)",
        ),
        (
            "v_price_per_sqft",
            "SELECT l.property_type, avg(l.list_price/l.sqft) AS avg_price_per_sqft FROM {s}.listings l WHERE l.sqft > 0 GROUP BY l.property_type",
        ),
        (
            "v_viewing_conversion",
            "SELECT view_type, source, count(*) AS views, sum(CASE WHEN inquired THEN 1 ELSE 0 END) AS inquiries FROM {s}.property_views GROUP BY view_type, source",
        ),
        (
            "v_brokerage_market_share",
            "SELECT a.brokerage, count(DISTINCT a.agent_id) AS agents, count(t.txn_id) AS transactions FROM {s}.agents a LEFT JOIN {s}.transactions_re t ON a.agent_id = t.buyer_id GROUP BY a.brokerage",
        ),
    ],
    "udfs": [
        (
            ...
            "price DECIMAL(12,2)",
            "STRING",
            "Format real estate price",
            "CASE WHEN price >= 1000000 THEN concat('$',round(price/1000000,2),'M') WHEN price >= 1000 THEN concat('$',round(price/1000,0),'K') ELSE concat('$',format_number(price,0)) END",
        ),
        (
            "price_per_sqft",
            "price DECIMAL(12,2), sqft INT",
            "DECIMAL(8,2)",
            "Price per square foot",
            "round(price / GREATEST(sqft, 1), 2)",
        ),
        (
            "mortgage_monthly",
            "principal DECIMAL(12,2), annual_rate DECIMAL(5,4), term_years INT",
            "DECIMAL(10,2)",
            "Monthly mortgage payment",
            "round(principal * (annual_rate/12) / (1 - power(1 + annual_rate/12, -term_years*12)), 2)",
        ),
        (
            "cap_rate",
            "noi DECIMAL(10,2), value DECIMAL(12,2)",
            "DECIMAL(5,2)",
            "Capitalization rate",
            "round(noi / GREATEST(value, 1) * 100, 2)",
        ),
        (
            "property_age",
            "year_built INT",
        ...
        (
            ...
            "STRING",
            "Market status",
            "CASE WHEN days_on_market < 7 THEN 'Hot' WHEN days_on_market < 30 THEN 'Active' WHEN days_on_market < 90 THEN 'Slow' ELSE 'Stale' END",
        ),
        (
        ...
        (
            "tax_estimate",
            "value DECIMAL(12,2), rate DECIMAL(6,4)",
            "DECIMAL(10,2)",
        ...
        (
            "roi_rental",
            "monthly_rent DECIMAL(8,2), purchase_price DECIMAL(12,2)",
            "DECIMAL(5,2)",
            "Gross rental ROI %",
            "round(monthly_rent * 12 / GREATEST(purchase_price, 1) * 100, 2)",
        ),
        (
            "school_rating_label",
            "rating INT",
            "STRING",
            "School rating label",
            "CASE WHEN rating >= 9 THEN 'Excellent' WHEN rating >= 7 THEN 'Good' WHEN rating >= 5 THEN 'Average' ELSE 'Below Average' END",
        ),
        (
            "mask_address",
            "address STRING",
            ...
            "concat(left(address,4),'*** ',split(address,' ')[size(split(address,' '))-1])",
        ),
        (
            "down_payment",
            "price DECIMAL(12,2), pct DECIMAL(5,2)",
            "DECIMAL(12,2)",
            "Down payment amount",
        ...
        (
            "commission_split",
            "sale_price DECIMAL(12,2), rate DECIMAL(5,2)",
            "DECIMAL(10,2)",
            "Agent commission",
            "round(sale_price * rate / 100, 2)",
        ),
        (
            "is_luxury",
            "price DECIMAL(12,2)",
        ...
        (
            ...
            "STRING",
            "Walkability label",
            "CASE WHEN score >= 90 THEN 'Walkers Paradise' WHEN score >= 70 THEN 'Very Walkable' WHEN score >= 50 THEN 'Somewhat Walkable' ELSE 'Car-Dependent' END",
        ),
        (
            "appreciation_estimate",
            "current_value DECIMAL(12,2), annual_rate DECIMAL(5,4), years INT",
            "DECIMAL(12,2)",
            "Estimated future value",
            "round(current_value * power(1 + annual_rate, years), 2)",
        ),
        (
            "ltv_ratio",
            "loan DECIMAL(12,2), value DECIMAL(12,2)",
            "DECIMAL(5,2)",
            "Loan-to-value ratio",
            "round(loan / GREATEST(value, 1) * 100, 2)",
        ),
        (
            "inspection_grade",
            "issues INT",
            "STRING",
            "Inspection grade",
            "CASE WHEN issues = 0 THEN 'A' WHEN issues <= 3 THEN 'B' WHEN issues <= 7 THEN 'C' WHEN issues <= 15 THEN 'D' ELSE 'F' END",
        ),
        (
            "hoa_status_label",
            "status STRING",
        ...
        (
            ...
            "property_id BIGINT",
            "STRING",
            "Anonymized property ref",
            "concat('PROP-',substring(sha2(cast(property_id as STRING),256),1,8))",
        ),
    ],
}
...
INDUSTRIES["logistics"] = {
    "tables": [
        _quick_table(
            "shipments",
            100_000_000,
            "shipment_id BIGINT, order_id BIGINT, origin STRING, destination STRING, carrier STRING, weight_kg DECIMAL(10,2), volume_m3 DECIMAL(8,2), ship_date DATE, delivery_date DATE, status STRING",
        ...
        _quick_table(
            "tracking_events",
            50_000_000,
            "event_id BIGINT, shipment_id BIGINT, event_type STRING, location STRING, event_timestamp TIMESTAMP, description STRING, latitude DOUBLE, longitude DOUBLE, facility_id BIGINT, handler STRING",
        ...
        _quick_table(
            "fleet_telemetry",
            30_000_000,
            "telemetry_id BIGINT, vehicle_id BIGINT, latitude DOUBLE, longitude DOUBLE, speed_mph DOUBLE, fuel_level_pct DECIMAL(5,2), engine_temp_f DOUBLE, odometer_miles BIGINT, timestamp_utc TIMESTAMP, idle_minutes INT",
        ...
        _quick_table(
            "warehouse_inventory",
            5_000_000,
            "inventory_id BIGINT, warehouse_id BIGINT, sku STRING, quantity INT, location_code STRING, received_date DATE, expiry_date DATE, lot_number STRING, status STRING, weight_kg DECIMAL(8,2)",
        ...
        _quick_table(
            "route_plans",
            5_000_000,
            "route_id BIGINT, vehicle_id BIGINT, driver_id BIGINT, planned_date DATE, stops INT, total_distance_miles DOUBLE, estimated_hours DOUBLE, actual_hours DOUBLE, fuel_cost DECIMAL(8,2), status STRING",
        ...
        _quick_table(
            "vehicles",
            1_000_000,
            "vehicle_id BIGINT, plate STRING, vehicle_type STRING, make STRING, model STRING, year INT, capacity_kg DECIMAL(10,2), fuel_type STRING, status STRING, last_service_date DATE",
        ...
        _quick_table(
            "drivers",
            1_000_000,
            "driver_id BIGINT, first_name STRING, last_name STRING, license_class STRING, hire_date DATE, status STRING, total_miles BIGINT, safety_score DECIMAL(5,2), home_base STRING, certifications STRING",
        ...
        _quick_table(
            "warehouses_lg",
            1_000_000,
            "warehouse_id BIGINT, name STRING, address STRING, city STRING, state STRING, sqft INT, warehouse_type STRING, temperature_controlled BOOLEAN, dock_doors INT, status STRING",
        ...
        _quick_table(
            "customs_entries",
            500_000,
            "entry_id BIGINT, shipment_id BIGINT, entry_type STRING, hs_code STRING, declared_value DECIMAL(12,2), duty_amount DECIMAL(10,2), origin_country STRING, dest_country STRING, filing_date DATE, clearance_date DATE",
        ...
        _quick_table(
            "freight_rates",
            500_000,
            "rate_id BIGINT, origin STRING, destination STRING, carrier STRING, mode STRING, rate_per_kg DECIMAL(8,4), rate_per_m3 DECIMAL(8,4), min_charge DECIMAL(8,2), effective_date DATE, currency STRING",
        ...
        _quick_table(
            "delivery_exceptions",
            300_000,
            "exception_id BIGINT, shipment_id BIGINT, exception_type STRING, description STRING, reported_at TIMESTAMP, resolved_at TIMESTAMP, impact STRING, resolution STRING",
        ...
        _quick_table(
            "fuel_purchases",
            300_000,
            "purchase_id BIGINT, vehicle_id BIGINT, fuel_type STRING, gallons DECIMAL(8,2), price_per_gallon DECIMAL(6,4), total_cost DECIMAL(8,2), station STRING, purchase_date DATE, odometer BIGINT",
        ...
        _quick_table(
            "dock_schedules",
            200_000,
            "schedule_id BIGINT, warehouse_id BIGINT, dock_number INT, carrier STRING, scheduled_time TIMESTAMP, actual_time TIMESTAMP, load_type STRING, status STRING, duration_minutes INT",
        ...
        _quick_table(
            "claims_lg",
            200_000,
            "claim_id BIGINT, shipment_id BIGINT, claim_type STRING, amount DECIMAL(10,2), filed_date DATE, resolved_date DATE, status STRING, evidence STRING, carrier_liable BOOLEAN",
        ...
        _quick_table(
            "packaging",
            100_000,
            "package_id BIGINT, shipment_id BIGINT, package_type STRING, length_cm DOUBLE, width_cm DOUBLE, height_cm DOUBLE, weight_kg DECIMAL(8,2), is_fragile BOOLEAN, is_hazmat BOOLEAN",
        ...
        _quick_table(
            "returns_lg",
            100_000,
            "return_id BIGINT, shipment_id BIGINT, reason STRING, return_date DATE, condition STRING, refund_amount DECIMAL(10,2), restocking_fee DECIMAL(8,2), status STRING",
    ...
    "views": [
        (
            "v_shipment_summary",
            "SELECT carrier, status, count(*) AS shipments, avg(weight_kg) AS avg_weight FROM {s}.shipments GROUP BY carrier, status",
        ),
        (
            "v_on_time_delivery",
            "SELECT carrier, count(*) AS total, sum(CASE WHEN delivery_date <= date_add(ship_date,7) THEN 1 ELSE 0 END) AS on_time FROM {s}.shipments WHERE status='delivered' GROUP BY carrier",
        ),
        (
            "v_fleet_utilization",
            "SELECT v.vehicle_type, v.status, count(*) AS vehicles, avg(v.capacity_kg) AS avg_capacity FROM {s}.vehicles v GROUP BY v.vehicle_type, v.status",
        ),
        (
            "v_warehouse_capacity",
            "SELECT w.name, w.city, w.sqft, w.dock_doors, count(wi.inventory_id) AS items FROM {s}.warehouses_lg w LEFT JOIN {s}.warehouse_inventory wi ON w.warehouse_id = wi.warehouse_id GROUP BY w.name, w.city, w.sqft, w.dock_doors",
        ),
        (
            "v_route_efficiency",
            "SELECT driver_id, count(*) AS routes, avg(total_distance_miles) AS avg_miles, avg(actual_hours/estimated_hours*100) AS efficiency_pct FROM {s}.route_plans WHERE status='completed' GROUP BY driver_id",
        ),
        (
            "v_exception_analysis",
            "SELECT exception_type, impact, count(*) AS exceptions FROM {s}.delivery_exceptions GROUP BY exception_type, impact",
        ),
        (
            "v_customs_processing",
            "SELECT origin_country, avg(datediff(clearance_date,filing_date)) AS avg_clearance_days, count(*) AS entries FROM {s}.customs_entries GROUP BY origin_country",
        ),
        (
            "v_fuel_costs",
            "SELECT v.vehicle_type, avg(fp.price_per_gallon) AS avg_price, sum(fp.total_cost) AS total_fuel_cost FROM {s}.fuel_purchases fp JOIN {s}.vehicles v ON fp.vehicle_id = v.vehicle_id GROUP BY v.vehicle_type",
        ),
        (
            "v_driver_safety",
            "SELECT d.first_name, d.last_name, d.safety_score, d.certifications, d.total_miles FROM {s}.drivers d WHERE d.status='active' ORDER BY d.safety_score DESC",
        ),
        (
            "v_rate_comparison",
            "SELECT origin, destination, mode, avg(rate_per_kg) AS avg_rate FROM {s}.freight_rates GROUP BY origin, destination, mode",
        ),
        (
            "v_tracking_timeline",
            "SELECT event_type, count(*) AS events, avg(CASE WHEN event_type='delivered' THEN 1 ELSE 0 END) AS delivery_rate FROM {s}.tracking_events GROUP BY event_type",
        ),
        (
            "v_dock_utilization",
            "SELECT warehouse_id, count(*) AS schedules, avg(duration_minutes) AS avg_duration FROM {s}.dock_schedules WHERE status='completed' GROUP BY warehouse_id",
        ),
        (
            "v_claims_overview",
            "SELECT claim_type, status, count(*) AS claims, sum(amount) AS total_amount FROM {s}.claims_lg GROUP BY claim_type, status",
        ),
        (
            "v_inventory_aging",
            "SELECT warehouse_id, status, count(*) AS items, avg(datediff(current_date(),received_date)) AS avg_age_days FROM {s}.warehouse_inventory GROUP BY warehouse_id, status",
        ),
        (
            "v_package_analysis",
            "SELECT package_type, count(*) AS packages, avg(weight_kg) AS avg_weight, sum(CASE WHEN is_fragile THEN 1 ELSE 0 END) AS fragile_count FROM {s}.packaging GROUP BY package_type",
        ),
        (
            "v_return_reasons",
            "SELECT reason, condition, count(*) AS returns, sum(refund_amount) AS total_refunded FROM {s}.returns_lg GROUP BY reason, condition",
        ),
        (
            "v_telemetry_alerts",
            "SELECT vehicle_id, count(*) AS readings, avg(speed_mph) AS avg_speed, avg(fuel_level_pct) AS avg_fuel FROM {s}.fleet_telemetry WHERE speed_mph > 70 OR fuel_level_pct < 20 GROUP BY vehicle_id",
        ),
        (
            "v_monthly_volume",
            "SELECT date_trunc('month',ship_date) AS month, carrier, count(*) AS shipments FROM {s}.shipments GROUP BY date_trunc('month',ship_date), carrier",
        ),
        (
            "v_lane_analysis",
            "SELECT origin, destination, count(*) AS shipments, avg(weight_kg) AS avg_weight FROM {s}.shipments GROUP BY origin, destination",
        ),
        (
            "v_fleet_maintenance",
            "SELECT vehicle_type, avg(datediff(current_date(),last_service_date)) AS avg_days_since_service FROM {s}.vehicles GROUP BY vehicle_type",
        ),
    ],
    "udfs": [
        (
            ...
            "STRING",
            "Format weight",
            "CASE WHEN kg >= 1000 THEN concat(round(kg/1000,2),' T') ELSE concat(round(kg,1),' kg') END",
        ),
        (
            "format_distance",
            "miles DOUBLE",
            "STRING",
            "Format distance",
            "CASE WHEN miles >= 1000 THEN concat(round(miles/1000,1),'K mi') ELSE concat(round(miles,0),' mi') END",
        ),
        (
        ...
        (
            "transit_time_sla",
            "ship_date DATE, delivery_date DATE, sla_days INT",
            "STRING",
            "SLA compliance",
            "CASE WHEN datediff(delivery_date,ship_date) <= sla_days THEN 'Met' ELSE 'Breached' END",
        ),
        (
            "dim_weight",
        ...
        (
            "fuel_efficiency",
            "miles DOUBLE, gallons DECIMAL(8,2)",
            "DECIMAL(5,2)",
            "Miles per gallon",
            "round(miles/GREATEST(gallons,0.01), 2)",
        ),
        (
        ...
        (
            ...
            "STRING",
            "Masks license plate",
            "concat('***',right(plate,3))",
        ),
        (
            "is_overweight",
            "weight_kg DECIMAL(10,2), capacity_kg DECIMAL(10,2)",
            "BOOLEAN",
            "Overweight check",
            "weight_kg > capacity_kg",
        ),
        (
            "customs_duty_rate",
            "hs_code STRING",
            "DECIMAL(5,2)",
            "Estimated duty rate",
            "CASE WHEN left(hs_code,2) IN ('01','02','03','04') THEN 5.0 WHEN left(hs_code,2) IN ('84','85') THEN 2.5 ELSE 3.5 END",
        ),
        (
            "exception_priority",
            "impact STRING",
            "INT",
            "Exception priority",
            "CASE WHEN impact = 'critical' THEN 1 WHEN impact = 'severe' THEN 2 WHEN impact = 'moderate' THEN 3 ELSE 4 END",
        ),
        (
            "warehouse_zone",
            "location_code STRING",
            "STRING",
            "Warehouse zone from location",
            "left(location_code,1)",
        ),
        (
            "cargo_class",
            "is_fragile BOOLEAN, is_hazmat BOOLEAN",
            "STRING",
            "Cargo classification",
            "CASE WHEN is_hazmat THEN 'Hazmat' WHEN is_fragile THEN 'Fragile' ELSE 'Standard' END",
        ),
        (
            "rate_per_mile",
            "total_cost DECIMAL(8,2), miles DOUBLE",
            "DECIMAL(6,4)",
            "Cost per mile",
            "round(total_cost/GREATEST(miles,1), 4)",
        ),
        (
            "anonymize_shipment",
            "shipment_id BIGINT",
            "STRING",
            "Anonymized shipment ref",
            "concat('SHP-',substring(sha2(cast(shipment_id as STRING),256),1,8))",
        ),
        (
            "claim_severity",
            "amount DECIMAL(10,2)",
            "STRING",
            "Claim severity",
            "CASE WHEN amount > 10000 THEN 'Major' WHEN amount > 1000 THEN 'Moderate' ELSE 'Minor' END",
        ),
        (
            "dock_utilization_pct",
            "scheduled INT, capacity INT",
            "DECIMAL(5,2)",
            "Dock utilization %",
            "round(scheduled*100.0/GREATEST(capacity,1), 2)",
        ),
        (
            "package_volume_m3",
            "l DOUBLE, w DOUBLE, h DOUBLE",
            ...
            "round(l*w*h/1000000, 4)",
        ),
        (
            "eta_status",
            "estimated TIMESTAMP, actual TIMESTAMP",
            "STRING",
            "ETA vs actual",
            "CASE WHEN actual IS NULL THEN 'Pending' WHEN actual <= estimated THEN 'On Time' ELSE 'Late' END",
        ),
        (
            "format_tracking_id",
            "id BIGINT",
            ...
            "concat('TRK-',lpad(cast(id as STRING),12,'0'))",
        ),
    ],
}
...
INDUSTRIES["insurance"] = {
    "tables": [
        _quick_table(
            "policies",
            100_000_000,
            "policy_id BIGINT, customer_id BIGINT, policy_type STRING, effective_date DATE, expiry_date DATE, premium DECIMAL(10,2), deductible DECIMAL(10,2), coverage_limit DECIMAL(14,2), status STRING, agent_id BIGINT",
        ...
        _quick_table(
            "claims_ins",
            50_000_000,
            "claim_id BIGINT, policy_id BIGINT, claim_date DATE, loss_date DATE, claim_type STRING, amount_claimed DECIMAL(12,2), amount_paid DECIMAL(12,2), status STRING, adjuster_id BIGINT, description STRING",
        ...
        _quick_table(
            "underwriting",
            30_000_000,
            "underwriting_id BIGINT, policy_id BIGINT, risk_score DOUBLE, risk_class STRING, factors STRING, decision STRING, premium_modifier DECIMAL(5,2), underwriter_id BIGINT, decision_date DATE, model_version STRING",
        ...
        _quick_table(
            "policyholders",
            1_000_000,
            "customer_id BIGINT, first_name STRING, last_name STRING, date_of_birth DATE, gender STRING, email STRING, phone STRING, address STRING, city STRING, state STRING",
        ...
        _quick_table(
            "agents_ins",
            1_000_000,
            "agent_id BIGINT, first_name STRING, last_name STRING, agency STRING, license_state STRING, commission_rate DECIMAL(5,2), policies_sold INT, premium_volume DECIMAL(14,2), hire_date DATE, status STRING",
        ...
        _quick_table(
            "payments_ins",
            1_000_000,
            "payment_id BIGINT, policy_id BIGINT, amount DECIMAL(10,2), payment_date DATE, payment_method STRING, payment_type STRING, status STRING, period_start DATE, period_end DATE",
        ...
        _quick_table(
            "loss_runs",
            1_000_000,
            "loss_id BIGINT, policy_id BIGINT, loss_date DATE, loss_type STRING, amount DECIMAL(12,2), reserve DECIMAL(12,2), paid DECIMAL(12,2), incurred DECIMAL(12,2), status STRING, catastrophe_code STRING",
        ...
        _quick_table(
            "reinsurance",
            500_000,
            "treaty_id BIGINT, reinsurer STRING, treaty_type STRING, retention DECIMAL(14,2), limit_amount DECIMAL(14,2), premium DECIMAL(12,2), effective_date DATE, expiry_date DATE, line_of_business STRING",
        ...
        _quick_table(
            "fraud_detection",
            500_000,
            "detection_id BIGINT, claim_id BIGINT, fraud_score DOUBLE, indicators STRING, investigation_status STRING, detected_date DATE, investigator STRING, outcome STRING, savings DECIMAL(10,2)",
        ...
        _quick_table(
            "actuarial_tables",
            500_000,
            "table_id BIGINT, table_type STRING, age_band STRING, gender STRING, risk_class STRING, mortality_rate DECIMAL(8,6), morbidity_rate DECIMAL(8,6), loss_ratio DECIMAL(5,2), effective_date DATE",
        ...
        _quick_table(
            "endorsements",
            300_000,
            "endorsement_id BIGINT, policy_id BIGINT, endorsement_type STRING, effective_date DATE, premium_change DECIMAL(8,2), description STRING, approved_by STRING",
        ...
        _quick_table(
            "quotes",
            300_000,
            "quote_id BIGINT, customer_id BIGINT, policy_type STRING, quoted_premium DECIMAL(10,2), quoted_date DATE, expiry_date DATE, converted BOOLEAN, agent_id BIGINT, channel STRING",
        ...
        _quick_table(
            "renewals",
            200_000,
            "renewal_id BIGINT, policy_id BIGINT, old_premium DECIMAL(10,2), new_premium DECIMAL(10,2), change_pct DECIMAL(5,2), renewal_date DATE, status STRING, reason STRING",
        ...
        _quick_table(
            "compliance_ins",
            200_000,
            "record_id BIGINT, state STRING, regulation STRING, filing_type STRING, filing_date DATE, approval_date DATE, status STRING, department STRING",
        ...
        _quick_table(
            "catastrophe_events",
            100_000,
            "event_id BIGINT, event_name STRING, event_type STRING, start_date DATE, end_date DATE, affected_states STRING, estimated_insured_losses DECIMAL(14,2), claims_count INT, pcs_number STRING",
    ...
    "views": [
        (
            "v_policy_portfolio",
            "SELECT policy_type, status, count(*) AS policies, sum(premium) AS total_premium, avg(coverage_limit) AS avg_limit FROM {s}.policies GROUP BY policy_type, status",
        ),
        (
            "v_claims_summary",
            "SELECT claim_type, status, count(*) AS claims, sum(amount_claimed) AS total_claimed, sum(amount_paid) AS total_paid FROM {s}.claims_ins GROUP BY claim_type, status",
        ),
        (
            "v_loss_ratio",
            "SELECT p.policy_type, sum(l.paid) AS total_losses, sum(p.premium) AS total_premium, round(sum(l.paid)/GREATEST(sum(p.premium),1)*100,2) AS loss_ratio FROM {s}.policies p LEFT JOIN {s}.loss_runs l ON p.policy_id = l.policy_id GROUP BY p.policy_type",
        ),
        (
            "v_underwriting_results",
            "SELECT risk_class, decision, count(*) AS applications, avg(risk_score) AS avg_score FROM {s}.underwriting GROUP BY risk_class, decision",
        ),
        (
            "v_agent_production",
            "SELECT a.agency, count(DISTINCT a.agent_id) AS agents, sum(a.policies_sold) AS total_sold, sum(a.premium_volume) AS total_premium FROM {s}.agents_ins a WHERE a.status='active' GROUP BY a.agency",
        ),
        (
            "v_fraud_savings",
            "SELECT outcome, count(*) AS cases, sum(savings) AS total_savings FROM {s}.fraud_detection GROUP BY outcome",
        ),
        (
            "v_renewal_retention",
            "SELECT status, count(*) AS renewals, avg(change_pct) AS avg_premium_change FROM {s}.renewals GROUP BY status",
        ),
        (
            "v_catastrophe_exposure",
            "SELECT event_type, count(*) AS events, sum(estimated_insured_losses) AS total_losses, sum(claims_count) AS total_claims FROM {s}.catastrophe_events GROUP BY event_type",
        ),
        (
            "v_reinsurance_coverage",
            "SELECT reinsurer, treaty_type, sum(limit_amount) AS total_limit, sum(premium) AS total_premium FROM {s}.reinsurance GROUP BY reinsurer, treaty_type",
        ),
        (
            "v_quote_conversion",
            "SELECT policy_type, channel, count(*) AS quotes, sum(CASE WHEN converted THEN 1 ELSE 0 END) AS converted, round(sum(CASE WHEN converted THEN 1 ELSE 0 END)*100.0/count(*),2) AS conversion_pct FROM {s}.quotes GROUP BY policy_type, channel",
        ),
        (
            "v_payment_collection",
            "SELECT payment_method, status, count(*) AS payments, sum(amount) AS total FROM {s}.payments_ins GROUP BY payment_method, status",
        ),
        (
            "v_actuarial_rates",
            "SELECT table_type, risk_class, age_band, avg(loss_ratio) AS avg_loss_ratio FROM {s}.actuarial_tables GROUP BY table_type, risk_class, age_band",
        ),
        (
            "v_endorsement_activity",
            "SELECT endorsement_type, count(*) AS endorsements, avg(premium_change) AS avg_change FROM {s}.endorsements GROUP BY endorsement_type",
        ),
        (
            "v_compliance_status",
            "SELECT state, regulation, status, count(*) AS filings FROM {s}.compliance_ins GROUP BY state, regulation, status",
        ),
        (
            "v_claims_aging",
            "SELECT status, avg(datediff(current_date(),claim_date)) AS avg_age_days, count(*) AS claims FROM {s}.claims_ins WHERE status NOT IN ('closed','denied') GROUP BY status",
        ),
        (
            "v_premium_by_state",
            "SELECT ph.state, p.policy_type, count(*) AS policies, sum(p.premium) AS total_premium FROM {s}.policies p JOIN {s}.policyholders ph ON p.customer_id = ph.customer_id GROUP BY ph.state, p.policy_type",
        ),
        (
            "v_monthly_written_premium",
            "SELECT date_trunc('month',effective_date) AS month, policy_type, sum(premium) AS written_premium FROM {s}.policies GROUP BY date_trunc('month',effective_date), policy_type",
        ),
        (
            "v_combined_ratio",
            "SELECT p.policy_type, round((sum(l.paid) + sum(p.premium)*0.3)/GREATEST(sum(p.premium),1)*100,2) AS combined_ratio FROM {s}.policies p LEFT JOIN {s}.loss_runs l ON p.policy_id = l.policy_id GROUP BY p.policy_type",
        ),
        (
            "v_policyholder_demographics",
            "SELECT gender, floor(datediff(current_date(),date_of_birth)/365) AS age, count(*) AS customers FROM {s}.policyholders GROUP BY gender, floor(datediff(current_date(),date_of_birth)/365)",
        ),
        (
            "v_fraud_indicators",
            "SELECT indicators, avg(fraud_score) AS avg_score, count(*) AS detections FROM {s}.fraud_detection WHERE investigation_status IN ('confirmed','investigating') GROUP BY indicators",
        ),
    ],
    "udfs": [
        (
            "loss_ratio_calc",
            "losses DECIMAL(14,2), premium DECIMAL(14,2)",
            "DECIMAL(5,2)",
            "Loss ratio",
            "round(losses / GREATEST(premium, 1) * 100, 2)",
        ),
        (
            "combined_ratio_calc",
            "loss_ratio DECIMAL(5,2), expense_ratio DECIMAL(5,2)",
            "DECIMAL(5,2)",
            "Combined ratio",
            "round(loss_ratio + expense_ratio, 2)",
        ),
        (
            "risk_class_label",
            "risk_class STRING",
            "STRING",
            "Risk class display",
            "initcap(replace(risk_class,'_',' '))",
        ),
        (
            "premium_rate_change",
            "old_premium DECIMAL(10,2), new_premium DECIMAL(10,2)",
            "STRING",
            "Premium change display",
            "CASE WHEN new_premium > old_premium THEN concat('+',cast(round((new_premium-old_premium)/old_premium*100,1) as STRING),'%') WHEN new_premium < old_premium THEN concat('-',cast(round((old_premium-new_premium)/old_premium*100,1) as STRING),'%') ELSE 'No Change' END",
        ),
        (
            "claim_severity_ins",
            "amount DECIMAL(12,2)",
            "STRING",
            "Claim severity",
            "CASE WHEN amount > 100000 THEN 'Catastrophic' WHEN amount > 50000 THEN 'Major' WHEN amount > 10000 THEN 'Moderate' WHEN amount > 1000 THEN 'Minor' ELSE 'Minimal' END",
        ),
        (
            "policy_status_label",
            "status STRING",
        ...
        (
            ...
            "STRING",
            "Fraud risk level",
            "CASE WHEN score >= 80 THEN 'Very High' WHEN score >= 60 THEN 'High' WHEN score >= 40 THEN 'Medium' WHEN score >= 20 THEN 'Low' ELSE 'Minimal' END",
        ),
        (
            "mask_policy_id",
            "id BIGINT",
            "STRING",
            "Masked policy ID",
            "concat('POL-***',right(cast(id as STRING),4))",
        ),
        (
            "coverage_adequate",
            "limit_val DECIMAL(14,2), recommended DECIMAL(14,2)",
            "STRING",
            "Coverage adequacy",
            "CASE WHEN limit_val >= recommended THEN 'Adequate' WHEN limit_val >= recommended*0.75 THEN 'Borderline' ELSE 'Underinsured' END",
        ),
        (
            "renewal_eligibility",
            "claims_count INT, premium_paid BOOLEAN",
            "BOOLEAN",
            "Eligible for renewal",
            "claims_count <= 3 AND premium_paid",
        ),
        (
            "deductible_impact",
            "claim DECIMAL(12,2), deductible DECIMAL(10,2)",
            "DECIMAL(12,2)",
            "Net claim after deductible",
            "GREATEST(claim - deductible, 0)",
        ),
        (
            "annualized_premium",
            "premium DECIMAL(10,2), months INT",
            "DECIMAL(10,2)",
            "Annualized premium",
            "round(premium * 12.0 / GREATEST(months, 1), 2)",
        ),
        (
            "catastrophe_category",
            "losses DECIMAL(14,2)",
            "STRING",
            "Catastrophe category",
            "CASE WHEN losses >= 25000000000 THEN 'Mega-Cat' WHEN losses >= 5000000000 THEN 'Major-Cat' WHEN losses >= 1000000000 THEN 'Significant' ELSE 'Minor' END",
        ),
        (
            "treaty_retention_pct",
            "retention DECIMAL(14,2), limit_val DECIMAL(14,2)",
            "DECIMAL(5,2)",
            "Retention percentage",
            "round(retention / GREATEST(limit_val, 1) * 100, 2)",
        ),
        (
            "quote_validity",
            "expiry_date DATE",
            ...
            "CASE WHEN expiry_date >= current_date() THEN 'Valid' WHEN datediff(current_date(),expiry_date) <= 7 THEN 'Recently Expired' ELSE 'Expired' END",
        ),
        (
            "filing_priority",
            "regulation STRING",
            "INT",
            "Filing priority",
            "CASE WHEN regulation = 'rate_filing' THEN 1 WHEN regulation = 'form_filing' THEN 2 WHEN regulation = 'financial_statement' THEN 3 ELSE 4 END",
        ),
        (
            "is_high_risk_policy",
            "risk_score DOUBLE, claims_count INT",
            "BOOLEAN",
            "High risk check",
            "risk_score > 70 OR claims_count > 3",
        ),
        (
            "age_band",
            "dob DATE",
            "STRING",
            "Age band for actuarial",
            "CASE WHEN datediff(current_date(),dob)/365 < 25 THEN '18-24' WHEN datediff(current_date(),dob)/365 < 35 THEN '25-34' WHEN datediff(current_date(),dob)/365 < 45 THEN '35-44' WHEN datediff(current_date(),dob)/365 < 55 THEN '45-54' WHEN datediff(current_date(),dob)/365 < 65 THEN '55-64' ELSE '65+' END",
        ),
        (
            "commission_amount",
            "premium DECIMAL(10,2), rate DECIMAL(5,2)",
            "DECIMAL(10,2)",
            "Agent commission",
            "round(premium * rate / 100, 2)",
        ),
        (
            "anonymize_policyholder",
            "first_name STRING, customer_id BIGINT",
            "STRING",
            "Anonymized policyholder",
            "concat('PH-',substring(sha2(concat(first_name,cast(customer_id as STRING)),256),1,8))",
        ),
    ],
}
...
PARTITION_COLS = {
    ...
    "card_events": "event_timestamp",
    "loan_payments": "payment_date",
    "order_items": "shipped_date",
    "clickstream": "event_timestamp",
    "reviews": "review_date",
    "cdr_records": "start_time",
    "data_usage": "session_start",
    "billing": "billing_period",
    "sensor_readings": "reading_timestamp",
    "production_events": "event_timestamp",
    "quality_checks": "check_timestamp",
    "meter_readings": "reading_timestamp",
    "grid_events": "event_timestamp",
    "generation_output": "generation_date",
    "enrollments": "enrollment_date",
    "learning_events": "event_timestamp",
    "assessments": "submitted_at",
    "listings": "list_date",
    "transactions_re": "closing_date",
    "property_views": "view_date",
    "shipments": "ship_date",
    "tracking_events": "event_timestamp",
    "fleet_telemetry": "timestamp_utc",
    "policies": "effective_date",
    "claims_ins": "claim_date",
    "underwriting": "decision_date",
}
...
def generate_demo_catalog(
    ...
    if not re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", catalog_name):
        raise ValueError(
            f"Invalid catalog_name '{catalog_name}' — must be alphanumeric with underscores"
        )
    ...
    try:
        ...
        if "must be before" in str(ve):
            raise
        raise ValueError(
            f"Invalid date format — use YYYY-MM-DD. start_date={start_date}, end_date={end_date}"
        )
    ...
    try:
        if drop_existing:
            try:
                execute_sql(
                    client, warehouse_id, f"DROP CATALOG IF EXISTS `{catalog_name}` CASCADE"
                )
        ...
        execute_sql(
            client, warehouse_id, f"CREATE CATALOG IF NOT EXISTS `{catalog_name}`{loc_clause}"
        )

        if owner:
            try:
                execute_sql(
                    client, warehouse_id, f"ALTER CATALOG `{catalog_name}` SET OWNER TO `{owner}`"
                )
    ...
    if medallion:
        ...
        if bronze_table_queries:
            try:
                execute_sql_parallel(
                    client, warehouse_id, bronze_table_queries, max_workers=max_workers
                )
            except Exception:
        ...
        result["schemas_created"] += len(
            set(q.split("`.`")[1].split("`")[0] for q in bronze_schema_queries)
        )
        result["tables_created"] += len(bronze_meta)
        result["total_rows"] += sum(r for _, _, r in bronze_meta)
        logger.info(
            f"    Bronze: {len(bronze_meta)} tables created across {len(industries)} industries"
        )
        ...
        if silver_view_queries:
            try:
                execute_sql_parallel(
                    client, warehouse_id, silver_view_queries, max_workers=max_workers
                )
            except Exception:
        ...
        result["schemas_created"] += len(
            set(q.split("`.`")[1].split("`")[0] for q in silver_schema_queries)
        )
        ...
        if gold_view_queries:
            try:
                execute_sql_parallel(
                    client, warehouse_id, gold_view_queries, max_workers=max_workers
                )
            except Exception:
        ...
        result["schemas_created"] += len(
            set(q.split("`.`")[1].split("`")[0] for q in gold_schema_queries)
        )
    ...
    if progress_dict is not None:
        progress_dict.update(
            {
                "current_industry": "enrichment",
                "current_phase": "enrichment",
                "industry_index": len(industries),
                "total_industries": len(industries),
            }
        )
    enrichment_queries = []
    ...
    for industry in industries:
        ...
        for tbl in tables[:5]:  # Top 5 tables per industry
            ...
            ddl_cols = {
                c.strip().split()[0] for c in tbl.get("ddl_cols", "").split(",") if c.strip()
            }
    ...
    if create_volumes:
        if schema_only:
            ...
            for industry in industries:
                try:
                    _create_volumes(
                        client, warehouse_id, catalog_name, industry, write_samples=False
                    )
                except Exception as e:
    ...
    if validate_referential_integrity and not schema_only:
        try:
            ...
            logger.info(
                ...
                + (
                    f", {ri_report['with_orphans']} with orphans"
                    if ri_report["with_orphans"]
                    else ""
                )
            )
        except Exception as e:
...
def _generate_industry(
            ...
            progress_dict.update(
                {
                    "current_phase": "tables",
                    "tables_done": tbl_idx,
                    "tables_total": len(industry_def["tables"]),
                }
            )
        try:
    ...
    if progress_dict is not None:
        progress_dict.update(
            {
                "current_phase": "views",
                "tables_done": len(industry_def["tables"]),
                "tables_total": len(industry_def["tables"]),
            }
        )
...
def _create_and_populate_table(
    ...
    sla_tier = (
        "gold"
        if tbl_def["rows"] >= 50_000_000
        else ("silver" if tbl_def["rows"] >= 1_000_000 else "bronze")
    )

    logger.info(
        f"  Creating table {fqn} (target: {target_rows:,} rows{', partitioned by ' + part_col if partition_clause else ''})"
    )
    execute_sql(
        client,
        warehouse_id,
    ...
    insert_expr = _fix_fk_ranges(
        tbl_def["insert_expr"], industry_name, scale_factor, start_date=start_date
    )
...
def _generate_medallion_schemas(
    ...
    logger.info(
        f"  Medallion done for {industry_name}: {counts['schemas']} schemas, {counts['tables']} tables, {counts['views']} views"
    )
    return counts
...
def _get_gold_views(industry_name, silver_schema, source_schema):
    ...
    views = {
        "healthcare": [
            (
                "daily_claims_summary",
                f"SELECT date_trunc('day', submitted_date) AS day, status, count(*) AS claims, sum(claim_amount) AS total_amount, avg(claim_amount) AS avg_amount FROM {source_schema}.claims GROUP BY date_trunc('day', submitted_date), status",
            ),
            (
                "provider_performance",
                f"SELECT provider_id, count(DISTINCT patient_id) AS patients, count(*) AS encounters, avg(duration_minutes) AS avg_visit_minutes FROM {source_schema}.encounters GROUP BY provider_id",
            ),
            (
                "top_diagnoses",
                f"SELECT diagnosis_code, count(*) AS frequency, sum(claim_amount) AS total_cost FROM {source_schema}.claims GROUP BY diagnosis_code ORDER BY frequency DESC",
            ),
            (
                "patient_risk_cohort",
        ...
        "financial": [
            (
                "daily_txn_volume",
                f"SELECT txn_date, txn_type, count(*) AS txns, sum(amount) AS total FROM {source_schema}.transactions GROUP BY txn_date, txn_type",
            ),
            (
                "fraud_summary",
                f"SELECT alert_type, resolution, count(*) AS alerts, avg(risk_score) AS avg_score FROM {source_schema}.fraud_alerts GROUP BY alert_type, resolution",
            ),
            (
                "portfolio_health",
                f"SELECT loan_type, status, count(*) AS loans, sum(principal) AS total_principal FROM {source_schema}.loans GROUP BY loan_type, status",
            ),
            (
                "customer_value_segments",
                f"SELECT CASE WHEN balance > 100000 THEN 'HNW' WHEN balance > 25000 THEN 'Affluent' WHEN balance > 5000 THEN 'Mass Affluent' ELSE 'Mass Market' END AS segment, count(*) AS accounts FROM {source_schema}.accounts GROUP BY CASE WHEN balance > 100000 THEN 'HNW' WHEN balance > 25000 THEN 'Affluent' WHEN balance > 5000 THEN 'Mass Affluent' ELSE 'Mass Market' END",
            ),
        ],
        "retail": [
            (
                "daily_revenue",
                f"SELECT order_date, count(*) AS orders, sum(total_amount) AS revenue, avg(total_amount) AS aov FROM {source_schema}.orders GROUP BY order_date",
            ),
            (
                "category_performance",
                f"SELECT p.category, count(oi.item_id) AS units, sum(oi.total) AS revenue FROM {source_schema}.order_items oi JOIN {source_schema}.products p ON oi.product_id = p.product_id GROUP BY p.category",
            ),
            (
                "customer_segments",
                f"SELECT loyalty_tier, count(*) AS customers FROM {source_schema}.customers GROUP BY loyalty_tier",
            ),
            (
                "product_ratings",
                f"SELECT p.category, avg(r.rating) AS avg_rating, count(*) AS reviews FROM {source_schema}.reviews r JOIN {source_schema}.products p ON r.product_id = p.product_id GROUP BY p.category",
            ),
        ],
        "telecom": [
            (
                "daily_network_kpi",
                f"SELECT cast(start_time as DATE) AS day, status, count(*) AS calls, avg(duration_seconds) AS avg_duration FROM {source_schema}.cdr_records GROUP BY cast(start_time as DATE), status",
            ),
            (
                "arpu_by_plan",
                f"SELECT p.name, avg(b.total_amount) AS arpu FROM {source_schema}.billing b JOIN {source_schema}.subscribers s ON b.subscriber_id = s.subscriber_id JOIN {source_schema}.plans p ON s.plan_id = p.plan_id GROUP BY p.name",
            ),
            (
                "churn_risk_summary",
                f"SELECT CASE WHEN churn_probability >= 0.8 THEN 'Critical' WHEN churn_probability >= 0.5 THEN 'High' WHEN churn_probability >= 0.3 THEN 'Medium' ELSE 'Low' END AS risk, count(*) AS subscribers FROM {source_schema}.churn_predictions GROUP BY CASE WHEN churn_probability >= 0.8 THEN 'Critical' WHEN churn_probability >= 0.5 THEN 'High' WHEN churn_probability >= 0.3 THEN 'Medium' ELSE 'Low' END",
            ),
            (
                "data_consumption_trends",
                f"SELECT app_category, network_type, sum(bytes_downloaded)/1073741824 AS total_gb FROM {source_schema}.data_usage GROUP BY app_category, network_type",
            ),
        ],
        "manufacturing": [
            (
                "oee_daily",
                f"SELECT cast(event_timestamp as DATE) AS day, line_id, sum(quantity) AS produced, count(*) AS events FROM {source_schema}.production_events WHERE event_type = 'complete' GROUP BY cast(event_timestamp as DATE), line_id",
            ),
            (
                "quality_pass_rate",
                f"SELECT product_code, count(*) AS checks, sum(CASE WHEN result='pass' THEN 1 ELSE 0 END)*100.0/count(*) AS pass_pct FROM {source_schema}.quality_checks GROUP BY product_code",
            ),
            (
                "downtime_analysis",
                f"SELECT reason, category, count(*) AS events, sum(duration_minutes) AS total_minutes, sum(cost_impact) AS total_cost FROM {source_schema}.downtime_events GROUP BY reason, category",
            ),
            (
                "safety_trends",
                f"SELECT date_trunc('month',incident_date) AS month, severity, count(*) AS incidents FROM {source_schema}.safety_incidents GROUP BY date_trunc('month',incident_date), severity",
            ),
        ],
    }
    ...
    return views.get(
        ...
        [
            (
                "daily_summary",
                f"SELECT current_date() AS report_date, '{industry_name}' AS industry, 'summary' AS metric_type",
            ),
        ],
    )
...
def _get_pii_tables(industry: str) -> dict[str, str]:
    ...
    base = {
        "healthcare": {
            "patients": "pii_high",
            "providers": "pii_medium",
            "claims": "confidential",
            "prescriptions": "pii_high",
        },
        "financial": {
            "customers": "pii_high",
        ...
        "telecom": {
            "subscribers": "pii_high",
            "cdr_records": "confidential",
            "devices": "pii_medium",
        },
        ...
        "education": {
            "students": "pii_high",
            "financial_aid": "confidential",
            "instructors": "pii_medium",
        },
        "real_estate": {
            "agents": "pii_medium",
            "properties": "public",
            "mortgages": "confidential",
        },
        "logistics": {"drivers": "pii_medium", "customs_entries": "confidential"},
        "insurance": {
            "policyholders": "pii_high",
            "claims_ins": "confidential",
            "fraud_detection": "confidential",
        },
    }
        ...
        if outlier_rate > 0 and ("amount" in cols.lower() or "price" in cols.lower()):
            for col_part in cols.split(","):
                ...
                if "amount" in col_part.lower() and ("DECIMAL" in col_part or "DOUBLE" in col_part):
                    ...
                    queries.append(
                        f"UPDATE {fqn} SET {col_name} = {col_name} * 1000 WHERE rand() < {outlier_rate}"
                    )
                    break
...
def _create_cross_industry_views(client, warehouse_id, catalog, industries):
    ...
    if "energy" in industries and "manufacturing" in industries:
        views.append(
            (
                "energy_manufacturing_consumption",
...
def _create_volumes(client, warehouse_id, catalog, industry, write_samples: bool = True):
    ...
    for vol_name, comment in [
        ("sample_data", f"Sample data files for {industry}"),
        ("exports", f"Export destination for {industry} reports"),
    ]:
        try:
            execute_sql(
                client,
        ...
        fake_ops.append(
            {
                ...
                "user_name": random.choice(
                    ["admin@company.com", "data-eng@company.com", "platform@company.com"]
                ),
            }
        )
...
def _apply_column_masks(client, warehouse_id, catalog, industry, industry_def):
    ...
    mask_functions = {
        ...
        "mask_demo_phone": ("phone STRING", "STRING", "concat('***-***-', right(phone, 4))"),
        "mask_demo_name": (
            "name STRING",
            "STRING",
    ...
    pii_cols = {
        "healthcare": [
            ("patients", "email", "mask_demo_email"),
            ("patients", "phone", "mask_demo_phone"),
        ],
        "financial": [
            ("customers", "email", "mask_demo_email"),
            ("customers", "phone", "mask_demo_phone"),
        ],
        "retail": [
            ("customers", "email", "mask_demo_email"),
            ("customers", "phone", "mask_demo_phone"),
        ...
        "telecom": [
            ("subscribers", "email", "mask_demo_email"),
            ("subscribers", "phone", "mask_demo_phone"),
        ],
        "education": [("students", "email", "mask_demo_email")],
        "insurance": [
            ("policyholders", "email", "mask_demo_email"),
            ("policyholders", "phone", "mask_demo_phone"),
        ],
        "energy": [("customers_energy", "name", "mask_demo_name")],
        "real_estate": [
            ("agents", "email", "mask_demo_email"),
            ("agents", "phone", "mask_demo_phone"),
        ],
    ...
    for tbl in dim_tables:
        ...
        try:
            ...
            execute_sql(
                client,
                warehouse_id,
                f"INSERT OVERWRITE TABLE {fqn} SELECT * EXCEPT(valid_from, valid_to, is_current) FROM {tmp_fqn}",
            )
            ...
            execute_sql(
                client, warehouse_id, f"CREATE OR REPLACE TABLE {fqn} AS SELECT * FROM {tmp_fqn}"
            )
...
def cleanup_demo_catalog(client, warehouse_id, catalog_name):
    ...
    try:
        ...
        logger.info(
            f"  Dropped catalog {catalog_name} ({result['schemas_dropped']} schemas, {result['tables_dropped']} objects)"
        )
    except Exception as e:
        result["status"] = "error"
...
_FK_DIM_ROWS = {
    ...
    "retail": {
        ...
        "store_id": 1_000_000,
        "order_id": 5_000_000,
        "warehouse_id": 500_000,
        "supplier_id": 200_000,
    },
    "telecom": {
        "subscriber_id": 1_000_000,
        "plan_id": 1_000_000,
        "tower_id": 1_000_000,
        "device_id": 1_000_000,
        "cell_tower_id": 50_000,
    },
    "manufacturing": {
        "equipment_id": 1_000_000,
        "line_id": 1_000_000,
        "material_id": 1_000_000,
        "supplier_id": 1_000_000,
        "order_id": 5_000_000,
        "operator_id": 500,
    },
    "energy": {
    ...
    "education": {
        "student_id": 1_000_000,
        "course_id": 1_000_000,
        "instructor_id": 500_000,
        "content_id": 100_000,
        "grader_id": 5_000,
        "venue_id": 300,
    },
    "real_estate": {
        "property_id": 1_000_000,
        "agent_id": 1_000_000,
        "listing_id": 100_000_000,
        "buyer_id": 2_000_000,
        "seller_id": 2_000_000,
        "appraiser_id": 10_000,
        "inspector_id": 5_000,
    ...
    "logistics": {
        "shipment_id": 100_000_000,
        "vehicle_id": 1_000_000,
        "driver_id": 1_000_000,
        "warehouse_id": 1_000_000,
        "facility_id": 5_000,
    },
    "insurance": {
        "policy_id": 100_000_000,
        "customer_id": 1_000_000,
        "agent_id": 1_000_000,
        "claim_id": 50_000_000,
        "adjuster_id": 5_000,
...
def _fix_fk_ranges(
    insert_expr: str, industry_name: str, scale_factor: float, start_date: str = "2020-01-01"
) -> str:
    ...
    if start_date != "2020-01-01":
        ...
        result = result.replace(
            "'2015-01-01'", f"'{start_date}'"
        )  # Some dim tables use older dates
...
TABLE_COMMENTS = {
    (
        "healthcare",
        "claims",
    ): "Insurance claims submitted by healthcare providers. Contains diagnosis codes (ICD-10), procedure codes (CPT), reimbursement amounts, and adjudication status. Primary fact table for revenue cycle analytics.",
    (
        "healthcare",
        "encounters",
    ): "Patient encounters including inpatient, outpatient, emergency, and telehealth visits. Tracks duration, discharge status, and admission source for utilization analysis.",
    (
        "healthcare",
        "prescriptions",
    ): "Prescription records including drug codes (NDC), quantities, days supply, and pharmacy information. Key dataset for formulary management and drug utilization review.",
    (
        "healthcare",
        "patients",
    ): "Patient demographics including name, date of birth, gender, and insurance information. Contains PII requiring column masking and access controls.",
    (
        "healthcare",
        "providers",
    ): "Healthcare provider directory with NPI numbers, specialties, and facility affiliations. Reference table for provider network analysis.",
    (
        "financial",
        "transactions",
    ): "Financial transactions across all channels including debit, credit, transfers, and payments. Primary fact table for fraud detection and spending analytics.",
    (
        "financial",
        "accounts",
    ): "Customer account master data including checking, savings, money market, and brokerage accounts. Contains balance, interest rate, and overdraft information.",
    (
        "financial",
        "customers",
    ): "Customer profiles with credit scores, income brackets, and risk ratings. Contains PII (SSN hash, DOB, email) requiring masking.",
    (
        "financial",
        "loans",
    ): "Loan portfolio including mortgage, auto, personal, and student loans. Tracks principal, interest rate, term, and delinquency status.",
    (
        "retail",
        "order_items",
    ): "Individual line items within customer orders. Links products to orders with quantity, pricing, discount, and fulfillment status. Largest fact table for sales analytics.",
    (
        "retail",
        "customers",
    ): "Customer profiles with loyalty tier, contact information, and registration date. Foundation for customer lifetime value and segmentation analysis.",
    (
        "retail",
        "products",
    ): "Product catalog with category, brand, pricing, cost, and active status. Used for margin analysis, assortment planning, and recommendation engines.",
    (
        "telecom",
        "cdr_records",
    ): "Call Detail Records capturing every voice call, SMS, and MMS. Includes duration, cell tower, cost, and completion status. Primary dataset for network analytics.",
    (
        "telecom",
        "subscribers",
    ): "Subscriber profiles with plan, activation date, credit class, and contract terms. Foundation for churn prediction and customer lifecycle management.",
    (
        "manufacturing",
        "sensor_readings",
    ): "Real-time IoT sensor data from production equipment. Captures temperature, pressure, vibration, and other measurements for predictive maintenance.",
    (
        "manufacturing",
        "production_events",
    ): "Production floor events including starts, completions, pauses, scrap, and changeovers. Key dataset for OEE (Overall Equipment Effectiveness) analysis.",
    (
        "energy",
        "meter_readings",
    ): "Smart meter readings for electric, gas, water, and renewable sources. Foundation for billing, demand forecasting, and conservation programs.",
    (
        "energy",
        "generation_output",
    ): "Power generation output by plant and fuel type. Includes capacity factor, CO2 emissions, and dispatch economics for energy portfolio optimization.",
    (
        "education",
        "enrollments",
    ): "Student course enrollments with grades, credits, and completion status. Primary dataset for academic analytics, retention analysis, and degree auditing.",
    (
        "education",
        "students",
    ): "Student profiles with major, enrollment year, GPA, and financial aid status. Contains PII requiring FERPA compliance controls.",
    (
        "real_estate",
        "listings",
    ): "Property listings with asking price, status, property type, and features. Primary dataset for market analysis and comparative market assessments.",
    (
        "real_estate",
        "properties",
    ): "Property master data with address, type, year built, lot size, and assessed value. Foundation for appraisal and tax analysis.",
    (
        "logistics",
        "shipments",
    ): "Shipment records with origin, destination, carrier, weight, and delivery status. Primary fact table for logistics performance and cost analytics.",
    (
        "logistics",
        "tracking_events",
    ): "Package tracking events with location, timestamp, and handler. Provides shipment visibility and SLA compliance monitoring.",
    (
        "insurance",
        "policies",
    ): "Insurance policy records across auto, home, life, health, and commercial lines. Contains premium, deductible, coverage limit, and renewal information.",
    (
        "insurance",
        "claims_ins",
    ): "Insurance claims with loss details, claimed/paid amounts, and adjuster assignments. Primary dataset for loss ratio analysis and fraud detection.",
}
...
def _apply_seasonal_patterns(
    client, warehouse_id, catalog, industry, industry_def, scale_factor=1.0
):
    ...
    for tbl in tables[:2]:
        ...
        try:
            ...
            select_parts = [f"{shift_expr} AS {date_col}" if c == date_col else c for c in all_cols]
...
def _grant_permissions(client, warehouse_id, catalog, industries):
    ...
    for industry in industries:
        ...
        grants.append(
            f"GRANT ALL PRIVILEGES ON SCHEMA `{catalog}`.`{industry}` TO `data_engineers`"
        )
