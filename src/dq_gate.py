...
def _evaluate_table_gate(
    client, warehouse_id, config, table_fqn: str, min_pass_rate: float
) -> dict:
    ...
    try:
        ...
        if result.get("error"):
            ...
            return {
                "passed": True,
                "reason": f"Checks could not run: {result['error']}",
                "warning": True,
            }
...
def _evaluate_suite_gate(client, warehouse_id, config, suite_id: str, min_pass_rate: float) -> dict:
