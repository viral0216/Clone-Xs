...
def create_environment(
    ...
    if not tables_to_clone:
        ...
        try:
            ...
            tables_to_clone = [
                f"{source_catalog}.{t['table_schema']}.{t['table_name']}" for t in all_tables
            ]
        except Exception:
            pass
    ...
    for tbl in tables_to_clone:
        ...
        if len(parts) == 3:
            ...
            try:
                _run_sql(
                    f"CREATE SCHEMA IF NOT EXISTS {target_catalog}.{src_schema}",
                    client,
                    warehouse_id,
                )
                _run_sql(
                    f"CREATE TABLE {target_catalog}.{src_schema}.{src_table} {clone_type} CLONE {tbl}",
                    client,
                    warehouse_id,
                )
                cloned += 1
...
def _update_env_status(env_id, status, schema, client, warehouse_id):
    ...
    try:
        _run_sql(
            f"UPDATE {schema}.environments SET status = '{status}'{extra} WHERE env_id = '{_esc(env_id)}'",
            client,
            warehouse_id,
        )
    except Exception:
...
def list_environments(
    status: str = None, client=None, warehouse_id: str = "", config: dict = None
) -> list[dict]:
    config = config or {}
    schema = _get_schema(config)
    ...
    try:
        return (
            _query_sql(
                f"SELECT * FROM {schema}.environments {where} ORDER BY created_at DESC",
                limit=100,
                client=client,
                warehouse_id=warehouse_id,
            )
...
def get_environment(env_id: str, client=None, warehouse_id: str = "", config: dict = None) -> dict:
    ...
    try:
        rows = _query_sql(
            f"SELECT * FROM {schema}.environments WHERE env_id = '{_esc(env_id)}'",
            limit=1,
            client=client,
            warehouse_id=warehouse_id,
        )
...
def destroy_environment(
    env_id: str, client=None, warehouse_id: str = "", config: dict = None
) -> dict:
...
def list_templates(client=None, warehouse_id: str = "", config: dict = None) -> list[dict]:
    ...
    try:
        return (
            _query_sql(
                f"SELECT * FROM {schema}.environment_templates ORDER BY created_at DESC",
                limit=50,
                client=client,
                warehouse_id=warehouse_id,
            )
...
def delete_template(template_id: str, client=None, warehouse_id: str = "", config: dict = None):
    ...
    try:
        _run_sql(
            f"DELETE FROM {schema}.environment_templates WHERE template_id = '{_esc(template_id)}'",
            client,
            warehouse_id,
        )
    except Exception:
