...
def ensure_governance_tables(client, warehouse_id, config):
    ...
    try:
        ...
        ensure_catalog_and_schema(
            client, warehouse_id, cat, sch, config.get("catalog_location", "")
        )
...
def create_glossary_term(client, warehouse_id, config, term: dict, user: str = "") -> dict:
    ...
    _track_change(
        client, warehouse_id, config, "glossary", term_id, "created", {"name": term["name"]}, user
    )
...
def list_glossary_terms(client, warehouse_id, config) -> list[dict]:
    ...
    try:
        rows = execute_sql(
            client, warehouse_id, f"SELECT * FROM {schema}.business_glossary ORDER BY name"
        )
...
def delete_glossary_term(client, warehouse_id, config, term_id: str, user: str = ""):
    ...
    execute_sql(
        client,
        warehouse_id,
        f"DELETE FROM {schema}.glossary_links WHERE term_id = '{_esc(term_id)}'",
    )
    execute_sql(
        client,
        warehouse_id,
        f"DELETE FROM {schema}.business_glossary WHERE term_id = '{_esc(term_id)}'",
    )
    _track_change(client, warehouse_id, config, "glossary", term_id, "deleted", {}, user)


def link_term_to_columns(
    client, warehouse_id, config, term_id: str, column_fqns: list[str], user: str = ""
):
...
def certify_table(client, warehouse_id, config, cert: dict, user: str = "") -> dict:
    ...
    _track_change(
        ...
        cert_id,
        "created",
        {"table": cert["table_fqn"], "status": cert["status"]},
        user,
    )
...
def list_certifications(client, warehouse_id, config) -> list[dict]:
    ...
    try:
        rows = execute_sql(
            client, warehouse_id, f"SELECT * FROM {schema}.certifications ORDER BY table_fqn"
        )
    ...
    if not catalogs:
        try:
            ...
            catalogs = [
                c
                for c in catalogs
                if c and c not in ("system", "hive_metastore", "__databricks_internal")
            ]
        except Exception:
            catalogs = []
...
def get_change_history(
    client, warehouse_id, config, entity_type: str = "", limit: int = 100
) -> list[dict]:
