...
class PipelineStore:
    ...
    def save_run(
        self, run_id: str, pipeline_id: str, pipeline_name: str, total_steps: int, triggered_by: str
    ) -> None:
        now = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
        execute_sql(
            self.client,
    ...
    def update_run(
        self, run_id: str, status: str, completed_steps: int = 0, error: str | None = None
    ) -> None:
        now = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
