...
def check_connectivity(client: WorkspaceClient) -> dict:
    ...
    try:
        ...
        return {
            "check": "connectivity",
            "status": "OK",
            "detail": f"{len(catalogs)} catalogs accessible",
        }
...
def check_catalog_access(
    ...
    try:
        ...
        return {
            "check": f"{label}_access ({catalog})",
            "status": "OK",
            "detail": f"Accessible (owner: {getattr(info, 'owner', 'unknown')})",
        }
    except Exception as sdk_err:
        sdk_detail = str(sdk_err)
        if label == "destination" and (
            "cannot be found" in sdk_detail.lower()
            or "not found" in sdk_detail.lower()
            or "NO_SUCH_CATALOG" in sdk_detail
            or "does not exist" in sdk_detail.lower()
        ):
            return {
                "check": f"{label}_access ({catalog})",
                "status": "WARN",
    ...
    try:
        ...
        detail = str(e)
        if label == "destination" and (
            "cannot be found" in detail.lower()
            or "not found" in detail.lower()
            or "NO_SUCH_CATALOG" in detail
        ):
            return {
                "check": f"{label}_access ({catalog})",
                "status": "WARN",
...
def check_permissions(
    ...
    try:
        ...
        return {
            ...
            "status": "OK",
            "detail": "Can create/drop schemas (SDK)",
        }
    except Exception as sdk_err:
        sdk_detail = str(sdk_err)
        if (
            "not found" in sdk_detail.lower()
            or "NO_SUCH_CATALOG" in sdk_detail
            or "does not exist" in sdk_detail.lower()
        ):
            return {
...
def check_uc_permissions(
    ...
    try:
        ...
        if is_owner:
            ...
            has_manage = any(
                priv in str(g.get("privilege", "")).upper() for g in grants for priv in MANAGE_PRIVS
            )
    ...
    manage_ok = any(
        c.get("check") == "dest_manage_permission" and c.get("status") == "OK" for c in results
    )
