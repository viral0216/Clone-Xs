...
def _stats_row(
    *, schema="default", table="t1", table_type="MANAGED",
    size_bytes=1_000_000, row_count=1000, last_modified="2025-01-01T00:00:00",
):
    """Build a stats-row dict shaped like `catalog_stats_fast` produces."""
    return {
        "schema": schema, "table": table, "table_type": table_type,
        "size_bytes": size_bytes, "row_count": row_count,
        "last_modified": last_modified, "num_columns": 5, "comment": None,
    }


def _usage_row(*, fqn="main.default.t1", last_accessed=None, query_count=0, distinct_users=0):
    return {
        "table_name": fqn, "last_accessed": last_accessed,
        "query_count": query_count, "distinct_users": distinct_users,
    }
...
class TestRiskAndActionRules:
    ...
    def test_high_risk_threshold_is_inclusive_at_10gb(self):
        ...
        assert _risk_level(
            is_stale=True, has_stats=True, never_accessed=True,
            table_type="MANAGED", size_bytes=_HIGH_RISK_SIZE_BYTES, row_count=1,
        ) == "HIGH"
    ...
    def test_high_risk_just_below_threshold_is_medium(self):
        ...
        assert _risk_level(
            is_stale=True, has_stats=True, never_accessed=True,
            table_type="MANAGED", size_bytes=_HIGH_RISK_SIZE_BYTES - 1, row_count=1,
        ) == "MEDIUM"
    ...
    def test_no_stats_action_takes_priority(self):
        ...
        action = _suggested_action(
            has_stats=False, is_stale=True,
            table_type="MANAGED", never_accessed=True,
        )
...
class TestDetectStaleTables:
    ...
    def test_findings_include_classification_block(self, mock_stats, mock_usage):
        ...
        mock_stats.return_value = {"tables": [
    ...
    def test_min_size_bytes_drops_tiny_findings(self, mock_stats, mock_usage):
        ...
        result = detect_stale_tables(
            MagicMock(), "wh", "main",
            days_threshold=90, min_size_bytes=1_000_000,
        )
    ...
    def test_total_reclaimable_only_counts_stale_managed(self, mock_stats, mock_usage):
        ...
        mock_stats.return_value = {"tables": [
            _stats_row(schema="s", table="m_stale", size_bytes=5_000_000_000, table_type="MANAGED"),
            _stats_row(schema="s", table="ext_stale", size_bytes=99_000_000_000, table_type="EXTERNAL"),
            _stats_row(schema="s", table="no_stats", size_bytes=None, row_count=1, table_type="MANAGED"),
        ]}
        mock_usage.return_value = []
        result = detect_stale_tables(MagicMock(), "wh", "main", days_threshold=90)
...
class TestSmallFilesEnrichment:
    ...
    @patch("src.stale_detection.query_table_access_patterns")
    @patch("src.stale_detection.catalog_stats_fast")
    def test_small_files_overwrites_default_optimize_then_vacuum(self, mock_stats, mock_usage, mock_sql):
        ...
        mock_stats.return_value = {"tables": [
            {"schema": "s", "table": "compactable", "table_type": "MANAGED",
             "size_bytes": 200 * 32 * 1024 * 1024, "row_count": 1000,
             "last_modified": "2024-01-01T00:00:00", "num_columns": 5, "comment": None},
        ]}
        # Accessed once 200 days ago → stale, but accessed at least
        # once → not "never_accessed" → action is "OPTIMIZE then VACUUM".
        from datetime import timedelta as _td
        mock_usage.return_value = [_usage_row(
    ...
    @patch("src.stale_detection.query_table_access_patterns")
    @patch("src.stale_detection.catalog_stats_fast")
    def test_small_files_describe_detail_failure_is_swallowed(self, mock_stats, mock_usage, mock_sql):
...
class TestEndpointDispatch:
    ...
    def test_source_catalog_routes_to_single(self, client):
        with patch("src.stale_detection.detect_stale_tables") as mock_single, \
             patch("src.stale_detection_multi.detect_stale_tables_multi") as mock_multi:
            mock_single.return_value = {
                "catalog": "main", "findings": [], "summary": {
                    "by_risk_level": {}, "by_suggested_action": {},
                    "total_reclaimable_bytes": 0, "never_accessed_count": 0, "no_stats_count": 0,
                },
                "errors": [],
            }
    ...
    def test_source_catalogs_routes_to_multi(self, client):
        with patch("src.stale_detection.detect_stale_tables") as mock_single, \
            ...
            mock_multi.return_value = {
                "catalogs": ["main", "samples"], "findings": [],
                "summary": {"by_risk_level": {}, "by_suggested_action": {},
                            "total_reclaimable_bytes": 0, "never_accessed_count": 0, "no_stats_count": 0},
                "per_catalog": {}, "errors": [],
            }
            resp = client.post("/api/stale-scan", json={
    ...
    def test_days_threshold_clamped_at_request_binding(self, client):
        ...
        resp = client.post("/api/stale-scan", json={
            "source_catalog": "main", "days_threshold": 500,
        })
        assert resp.status_code == 422
