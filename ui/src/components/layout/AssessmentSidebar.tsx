...
const NAV_SECTIONS = [
  {
    title: "Assessment",
    items: [
      { href: "/assessment", label: "Overview", icon: LayoutDashboard },
      { href: "/assessment/run", label: "Run Scan", icon: Play },
    ],
  },
  {
    title: "Results",
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/assessment/inventory/topology", label: "Infrastructure", icon: Layers },
    ],
  },
  {
...
const STORAGE_KEY = "clxs-assessment-sidebar-collapsed";
...
export default function AssessmentSidebar() {
  ...
  const isActive = (href: string) => {
    ...
    // Only use prefix match for routes that have no children in NAV_SECTIONS
    const allHrefs = NAV_SECTIONS.flatMap(s => s.items.map(i => i.href));
    const hasChildren = allHrefs.some(h => h !== href && h.startsWith(href + "/"));
    return !hasChildren && location.pathname.startsWith(href + "/");
  };
    ...
    return (
      <aside className="w-11 shrink-0 border-r border-border bg-background hidden lg:flex flex-col">
        <div className="flex flex-col items-center h-full py-2">
          <nav className="flex-1 overflow-y-auto w-full flex flex-col items-center gap-0.5 pt-1" aria-label="Assessment navigation">
  ...
  return (
    <aside className="w-56 shrink-0 border-r border-border bg-background h-full flex flex-col">
      <div className="px-4 py-3 border-b border-border">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Assessment Portal</p>
      </div>
      <nav className="py-2 flex-1 overflow-y-auto" aria-label="Assessment navigation">
