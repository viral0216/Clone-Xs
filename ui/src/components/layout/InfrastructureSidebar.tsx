...
import {
  LayoutDashboard, Server, Globe, Share2, Radio,
...
const NAV_SECTIONS = [
  {
    ...
    items: [
      { href: "/infrastructure", label: "Overview", icon: LayoutDashboard },
    ],
  },
  {
    title: "Compute",
    items: [
      { href: "/infrastructure/warehouse", label: "Warehouse", icon: Server },
      { href: "/infrastructure/lakehouse-monitor", label: "Lakehouse Monitor", icon: Radio },
    ],
  },
  {
    title: "Connectivity",
    items: [
      { href: "/infrastructure/federation", label: "Federation", icon: Globe },
      { href: "/infrastructure/delta-sharing", label: "Delta Sharing", icon: Share2 },
    ],
  },
];

const STORAGE_KEY = "clxs-infra-sidebar-collapsed";
    ...
    return (
      <aside className="w-11 shrink-0 border-r border-border bg-background hidden lg:flex flex-col">
        <div className="flex flex-col items-center h-full py-2">
          <nav className="flex-1 overflow-y-auto w-full flex flex-col items-center gap-0.5 pt-1" aria-label="Infrastructure navigation">
            {NAV_SECTIONS.flatMap((section) =>
              section.items.map((item) => {
                const Icon = item.icon;
                const active = location.pathname === item.href ||
                  (item.href !== "/infrastructure" && location.pathname.startsWith(item.href));
                return (
  ...
  return (
    <aside className="w-56 shrink-0 border-r border-border bg-background h-full flex flex-col">
      <div className="px-4 py-3 border-b border-border">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Infrastructure Portal</p>
      </div>
      <nav className="py-2 flex-1 overflow-y-auto" aria-label="Infrastructure navigation">
