...
import {
  LayoutDashboard, Fingerprint, Shield, CheckSquare,
...
const NAV_SECTIONS = [
  {
    title: "Security",
    items: [
      { href: "/security", label: "Overview", icon: LayoutDashboard },
    ],
  },
  {
    title: "Data Protection",
    items: [
      { href: "/security/pii", label: "PII Scanner", icon: Fingerprint },
      { href: "/security/compliance", label: "Compliance", icon: Shield },
    ],
  },
  {
    title: "Validation",
    items: [
      { href: "/security/preflight", label: "Preflight Checks", icon: CheckSquare },
    ],
  },
];

const STORAGE_KEY = "clxs-security-sidebar-collapsed";
    ...
    return (
      <aside className="w-11 shrink-0 border-r border-border bg-background hidden lg:flex flex-col">
        <div className="flex flex-col items-center h-full py-2">
          <nav className="flex-1 overflow-y-auto w-full flex flex-col items-center gap-0.5 pt-1" aria-label="Security navigation">
            {NAV_SECTIONS.flatMap((section) =>
              section.items.map((item) => {
                const Icon = item.icon;
                const active = location.pathname === item.href ||
                  (item.href !== "/security" && location.pathname.startsWith(item.href));
                return (
  ...
  return (
    <aside className="w-56 shrink-0 border-r border-border bg-background h-full flex flex-col">
      <div className="px-4 py-3 border-b border-border">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Security Portal</p>
      </div>
      <nav className="py-2 flex-1 overflow-y-auto" aria-label="Security navigation">
